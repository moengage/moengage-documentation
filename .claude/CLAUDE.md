...
- **API endpoint links** do not follow the folder structure. Their slugs are generated by Mintlify from the OpenAPI spec: `/{directory}/{tag-kebab}/{summary-kebab}`. For example, `POST /customer/{app_id}` with tag `User` and summary `Track User` in `api/data/data.yaml` links to `/api/user/track-user`. To find the correct slug, check the endpoint's `tags[0]` and `summary` in the YAML file and kebab-case both.
