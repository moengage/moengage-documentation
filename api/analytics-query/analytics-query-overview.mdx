...
<Steps>
  ...
  <Step title="Poll for Status">
    Call [Get Query Status](/api/analytics-queries/get-query-status) with that `request_id`. The query is still running while `status` is `PENDING` or `PROCESSING`, so keep polling until it reaches a final status of `SUCCESSFUL` or `FAILED`. When a query fails, the response also returns a `failure_reason`.
  </Step>
