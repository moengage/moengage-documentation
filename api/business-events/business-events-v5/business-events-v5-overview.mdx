...
<AccordionGroup>
  ...
  <Accordion title="How does the Idempotency-Key header work?">
    Send a UUID v4 in the `Idempotency-Key` header to safely retry a trigger call. Replays within a 24-hour window return the original cached response instead of firing the event again.
  </Accordion>
...
A dedicated Postman collection for the Business Events API (V5) is not yet published. In the meantime, you can refer to the existing collection covering the legacy Business Events API: [View MoEngage Business Events Collection](https://www.postman.com/moengage-dev/workspace/api-docs/collection/3182294-38587f83-f039-46f3-b86e-10af2c918053)
