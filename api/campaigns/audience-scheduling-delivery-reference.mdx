...
| `advanced.expiration_settings`'s four fields (`expire_notification_after_value`, `expire_notification_after_type`, `remove_from_inbox_after_value`, `remove_from_inbox_after_type`) are all **required together** for any campaign whose `platforms` is not `["WEB"]`. For `WEB`-only campaigns, `remove_from_inbox_after_value` and `remove_from_inbox_after_type` must be **omitted**. | `AdvancedDetails.expiration_settings` |
...
All four `expiration_settings` fields are required together unless the campaign's `platforms` is `["WEB"]`, in which case `remove_from_inbox_after_value` and `remove_from_inbox_after_type` must be omitted entirely.
