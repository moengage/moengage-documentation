...
keywords: ["trigger_condition", "segmentation_details", "scheduling_details", "delivery_controls", "conversion_goal_details", "control_group_details", "utm_params", "campaign_audience_limit", "geofences", "filters", "periodic", "BTS", "user timezone", "frequency capping", "SMS delivery controls", "MMS delivery controls"]
...
<Note>
**SMS/MMS:** This schema is reused as-is. The PRD marks SMS "Segmentation" as unresolved ("- ?"), but no SMS-specific shape has been specified anywhere — this repo documents SMS/MMS as reusing this existing channel-agnostic schema, matching Push/Email. Confirm with Engineering.
</Note>
...
<Note>
**SMS/MMS:** This schema is reused as-is. The PRD's "Delivery Type: ASAP / Scheduled Time" maps directly onto the existing `ASAP` / `AT_FIXED_TIME` values above. SMS/MMS adds one additional constraint: a scheduled `start_time` must be at least 1 hour ahead of the current time when the request is made — see [Validation rules](#validation-rules). This constraint is SMS/MMS-specific per the PRD; it is not assumed to apply to Push/Email.
</Note>
...
Reuses the same field names as Push/Email `delivery_controls` wherever the PRD concept already has one. Applies to both SMS and MMS campaigns.

| Field | Type | Notes |
| :--- | :--- | :--- |
| `bypass_dnd` | boolean | Whether to bypass Do Not Disturb. PRD: "Ignore DND." |
| `campaign_throttle_rpm` | integer | The throttle in requests per minute. PRD: "RPM." **Maximum:** 200,000. <Note>The minimum value is not confirmed — the PRD references an unincluded "low throttling file." Confirm with Engineering.</Note> |
| `count_for_frequency_capping` | boolean | Whether the campaign counts toward frequency caps. PRD: "Count for FC." **Conditionally required** when `ignore_frequency_capping` is `true` — see [Validation rules](#validation-rules). This conditional requirement is SMS/MMS-specific and is not documented for Push/Email today. |
| `ignore_frequency_capping` | boolean | Whether to bypass frequency capping. PRD: "Ignore FC." |
| `minimum_delay_between_two_notification_in_hour` | integer | Minimum hours between two messages from this campaign. PRD: "Minimum Delay b/w message." |
...
```json
{
  "delivery_controls": {
    ...
    "count_for_frequency_capping": true,
    "ignore_frequency_capping": false,
    "minimum_delay_between_two_notification_in_hour": 4
  }
}
```
...
<Note>
**SMS/MMS:** This schema is reused as-is. The PRD's conversion-goals story (goal name, event name, attribute filter, one primary goal, tracking window in days) maps directly onto the existing schema below.
</Note>
...
<Note>
**SMS/MMS:** This schema is reused as-is — see [campaign_content (SMS)](/api/campaigns/campaign-content-reference#updating-an-existing-campaign) for an update example.
</Note>
...
<Note>
**SMS/MMS:** The PRD's "Limit Audience" story maps conceptually onto this feature, but this schema's supported-channels list (below) currently documents Email and Push only — SMS/MMS support is not yet confirmed. This is intentionally **not** extended to SMS/MMS here pending Engineering confirmation; it is not included in the `SmsCampaignCreateV5Request`/`SmsComponentPatchRequest` schemas in this PR.
</Note>
...
| For SMS/MMS, `scheduling_details.start_time` must be at least 1 hour ahead of the current time when scheduling a send. This is SMS/MMS-specific per the PRD and is not assumed to apply to Push/Email. | [Campaign delivery schedule](#campaign-delivery-schedule) |
| For SMS/MMS, `delivery_controls.count_for_frequency_capping` is **required** when `delivery_controls.ignore_frequency_capping` is `true`. This conditional requirement is SMS/MMS-specific per the PRD and is not documented for Push/Email today. | `SmsDeliveryControls.count_for_frequency_capping` |
...
<Tab title="delivery_controls (SMS)">
