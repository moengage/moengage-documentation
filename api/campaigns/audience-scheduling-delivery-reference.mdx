...
| `limit_send_config` | object | Configuration for limiting how many times a user can receive this campaign within a rolling time window. Applies to periodic and event-triggered campaigns. Not applicable to transactional campaigns. |
...
- [Create Campaign](/api/create-draft-campaign-v5) — required fields, happy-path cURLs, error responses.
- [Update Campaign](/api/patch-draft-campaign-v5) — per-state edit restrictions.
- [Update Campaign Status](/api/change-campaign-status-v5) — `STOP`, `PAUSE`, `RESUME` transitions.
- [Validate Campaign](/api/validate-draft-campaign-v5) — publish-time validation check.
