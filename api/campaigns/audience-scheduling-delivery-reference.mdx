...
| :--- | :--- |
| [API-triggered campaigns](#api-triggered-campaigns) | Component restrictions for `API_TRIGGERED` |
...
An `API_TRIGGERED` campaign is a reusable shell for the [Messaging API](/api/messaging/messaging-api-overview). It has no audience and no schedule: each trigger request specifies the recipient and the content values for that send. This changes which components the campaign accepts.
