...
An `API_TRIGGERED` campaign is a reusable shell for the Messaging API. It has no audience and no schedule of its own: each trigger request names the recipient and supplies the content values for that send. This changes which components the campaign accepts.
...
| `control_group_details` | Supported | Control group evaluation for individual triggers is planned for a later release. |
