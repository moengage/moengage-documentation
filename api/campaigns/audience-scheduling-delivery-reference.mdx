...
| Component or field | Behavior |
| :--- | :--- |
| `segmentation_details` | Not supported. The server forces an All Users audience on the shell, and each trigger request names the actual recipients. |
| `scheduling_details` | Not supported, except that a redundant `delivery_type: ASAP` is tolerated and ignored. No other field applies. |
| `trigger_condition` | Not supported. The trigger request initiates the send. |
| `campaign_audience_limit` | Not supported when `is_campaign_audience_limit_enabled` is `true`. |
| `basic_details.business_event` | Not supported. |
| `delivery_controls.campaign_throttle_rpm` | Not supported, and no default RPM is applied. The per-request rate limit on the trigger endpoint governs delivery pacing rather than audience size. |
| `delivery_controls.bypass_dnd` | Not supported when `true`. DND always applies, and MoEngage drops the message for a recipient in DND rather than deferring or queueing it. |
| `delivery_controls.minimum_delay_between_two_notification_in_hour` | Not supported when non-zero. |
| `delivery_controls.limit_send_config` | Not supported when `enabled` is `true`. |
...
| `API_TRIGGERED` (Email) | Not permitted. See [API-triggered campaigns](#api-triggered-campaigns). | — | — |
