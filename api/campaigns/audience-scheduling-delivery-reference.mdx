...
An `API_TRIGGERED` shell supports `PUBLISH`, `PAUSE`, and `RESUME`. It does not support `STOP`. Use `PAUSE` to make a published shell stop accepting trigger requests, and `RESUME` to make it triggerable again. `PUBLISH` on `PATCH /v5/campaigns/{campaign_id}/status` applies to `API_TRIGGERED` campaigns only; publishing every other delivery type through V5 is not yet supported.
