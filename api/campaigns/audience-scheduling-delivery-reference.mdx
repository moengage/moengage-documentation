...
| `delivery_controls.minimum_delay_between_two_notification_in_hour` | When non-zero. The field is measured in hours, so `0` or omitting it are the only accepted values. | |
...
| Idempotent trigger retries | Not enforced in this release. The trigger endpoint accepts the `Idempotency-Key` header and returns it as `response_id`, but does not deduplicate on it. |
