...
keywords: ["trigger_condition", "segmentation_details", "scheduling_details", "delivery_controls", "conversion_goal_details", "control_group_details", "utm_params", "campaign_audience_limit", "geofences", "filters", "periodic", "BTS", "user timezone", "frequency capping", "API_TRIGGERED", "Messaging API"]
...
The minimum audience configuration is either `segmentation_details.is_all_user_campaign: true` or a single filter under `segmentation_details.included_filters`. The minimum schedule is `scheduling_details.delivery_type: ASAP`. Event-triggered campaigns also require `trigger_condition`. Neither component applies to `API_TRIGGERED` campaigns; see [API-triggered campaigns](#api-triggered-campaigns).
...
| :--- | :--- |
| [API-triggered campaigns](#api-triggered-campaigns) | Field support and status actions for `API_TRIGGERED` |
...
An `API_TRIGGERED` campaign is a reusable shell for the [Messaging API](/api/messaging/messaging-api-overview). It has no audience and no schedule: each trigger request specifies one recipient and the content values for that send. This changes which components the campaign accepts.

### Supported components

| Component | Supported fields |
| :--- | :--- |
| Top level | `request_id`, `channel` (`EMAIL` only), `created_by`, `updated_by`, `campaign_delivery_type`, `version_note` (update), `approvers` |
| `basic_details` | `name`, `team`, `tags`, `user_attribute_identifier`, `content_type` (`PROMOTIONAL` or `TRANSACTIONAL`), `subscription_category` |
| `campaign_content` | `locales`, `variation_details`, `content` |
| `connector` | `connector_type`, `connector_name` |
| `control_group_details` | `is_global_control_group_enabled`, `is_campaign_control_group_enabled`, `campaign_control_group_percentage` |
| `conversion_goal_details` | `attribution_window_in_hours`, `goals[]` |
| `delivery_controls` | `ignore_frequency_capping`, `count_for_frequency_capping` |
| `utm_params` | All fields |

`locales` and `variation_details` behave as they do for every other delivery type. See [Locale-keyed and variation-keyed shape](/api/campaigns/campaign-content-reference#locale-keyed-and-variation-keyed-shape).

### Audience and scheduling

A shell has no audience of its own. Do not send `segmentation_details`: MoEngage rejects a create request that includes it, and stores an All Users audience on the shell instead. Each trigger request names its own recipient.

`scheduling_details` is optional. When you send it, `delivery_type` must be `ASAP`, which is accepted and has no effect. MoEngage rejects any other value, and no other field within `scheduling_details` applies.

### Unsupported components

The following are not supported for API-triggered email campaigns. MoEngage rejects a create request that includes them with `400 VALIDATION_FAILED`.

| Component or field | Condition | Error message |
| :--- | :--- | :--- |
| `channel` other than `EMAIL` | Always | `API_TRIGGERED delivery type is supported only for the EMAIL channel` |
| `segmentation_details` | Always | `segmentation_details is not supported for API_TRIGGERED campaigns` |
| `scheduling_details.delivery_type` other than `ASAP` | Any other value | `Only ASAP delivery_type is supported for API_TRIGGERED campaigns` |
| `campaign_audience_limit` | When `is_campaign_audience_limit_enabled` is `true` | `campaign_audience_limit is not allowed for API_TRIGGERED campaigns` |
| `delivery_controls.campaign_throttle_rpm` | Always, and no default RPM is applied | `campaign_throttle_rpm is not valid for this campaign type` |
| `send_only_double_opt_in_users` | Always. Double opt-in does not apply. | `['send_only_double_opt_in_users'] are not expected in this request.` |
| `deduplication_attribute` | Always. Brand deduplication does not apply. | `['deduplication_attribute'] are not expected in this request.` |
| `trigger_condition` | Always. The trigger request initiates the send. | |
| `basic_details.business_event` | Always | |
| `delivery_controls.bypass_dnd` | When `true`. DND is not evaluated for an API-triggered campaign, so the flag has no effect. | |
| `delivery_controls.minimum_delay_between_two_notification_in_hour` | When non-zero | |
| `delivery_controls.limit_send_config` | When `enabled` is `true` | |

The per-request rate limit on the trigger endpoint governs delivery pacing rather than audience size.

### Unsupported email features

Beyond the components above, the following email capabilities do not apply to an `API_TRIGGERED` campaign.
...
| From address personalization | Not supported. `from_address` accepts a fixed value only. |
| Reply-to address personalization | Not supported. `reply_to_address` accepts a fixed value only. |
| Reply-to unsubscribe and list unsubscribe | Not supported. |
| Idempotent trigger retries | Not enforced in this early-access release. The trigger endpoint accepts the `Idempotency-Key` header and returns it as `response_id`, but does not deduplicate on it. |

Campaign reports behave as for any other delivery type: delivery and engagement events and campaign stats are reported. Messages dropped during dispatch are not reported.

### Test sends

Test Campaign is not supported for `API_TRIGGERED` campaigns.

| Mode | Behavior |
| :--- | :--- |
| Draft mode | A `draft_id` that points to an `API_TRIGGERED` shell returns an error. |
| Inline mode | The request returns no error, because the endpoint does not accept `campaign_delivery_type` as input. A successful inline test does not confirm that the shell works. |

To verify a shell, publish it and send a trigger request to a recipient you control.

### Status actions

An `API_TRIGGERED` shell supports `PUBLISH` and `STOP`. It does not support `PAUSE` or `RESUME`, so a shell has no paused state. After you stop a shell, MoEngage rejects trigger requests against it. `PUBLISH` on `PATCH /v5/campaigns/{campaign_id}/status` applies to `API_TRIGGERED` campaigns only; publishing every other delivery type through V5 is not yet supported.

<Note>
`API_TRIGGERED` is available for Email only and is in early access. Contact your Customer Success Manager to enable it for your workspace.
</Note>
...
| `API_TRIGGERED` (Email) | Not permitted. See [API-triggered campaigns](#api-triggered-campaigns). | Not permitted. | — |
...
The segment ID and name are visible in the MoEngage dashboard under **Segments**. For full segment construction details, refer to [Custom Segments](/user-guide/segment/create-segments/manage-segments).
...
`queuing_enabled` and `queue_duration` are flag-gated. Including them on a workspace where the feature is not enabled returns a validation error. For detailed behavior of queued messages, see [Message Queuing](/user-guide/settings/channels/delivery-controls/message-queuing).
...
For `API_TRIGGERED` campaigns, only `ignore_frequency_capping` and `count_for_frequency_capping` are supported. See [API-triggered campaigns](#api-triggered-campaigns).
...
- [Update Campaign](/api/update-campaigns/update-campaign-v5) — per-state edit restrictions.
- [Update Campaign Status](/api/update-campaigns/update-campaign-status-v5) — `STOP`, `PAUSE`, `RESUME` transitions.
- [Validate Campaign](/api/create-campaigns/validate-campaign-v5) — publish-time validation check.
