...
| Component | Status for `API_TRIGGERED` | Reason |
| :--- | :--- | :--- |
| `segmentation_details` | **Not permitted** | Each trigger request specifies the recipient identifier. The shell stores a user-level audience rather than a materialized segment, so there is no segment to define at creation time. |
| `scheduling_details` | **Not permitted** | The campaign is triggered on demand. |
| `trigger_condition` | **Not permitted** | The trigger request initiates the send. |
| `campaign_audience_limit` | **Not permitted** | Audience caps are not supported. |
| `delivery_controls` | **Partially supported** | See the field list below. |
| `conversion_goal_details` | Supported | Behaves as for other delivery types. |
| `control_group_details` | Supported | MoEngage does not evaluate control groups for individual trigger requests. |
| `utm_params` | Supported | Behaves as for other delivery types. |

Within `delivery_controls`, only three fields apply:

| Field | Behavior |
| :--- | :--- |
| `ignore_frequency_capping` | MoEngage evaluates frequency capping per recipient at send time. |
| `bypass_dnd` | MoEngage drops the message for a recipient in DND and does not defer or queue it. |
| `minimum_delay_between_two_notification_in_hour` | MoEngage enforces the delay per recipient. |

`campaign_throttle_rpm` and `limit_send_config` are not permitted. Throttling and maximum-send limits do not apply, because the per-request rate limit on the trigger endpoint governs delivery pacing rather than audience size.
...
For `API_TRIGGERED` campaigns, only `ignore_frequency_capping`, `bypass_dnd`, and `minimum_delay_between_two_notification_in_hour` are permitted. See [API-triggered campaigns](#api-triggered-campaigns).
