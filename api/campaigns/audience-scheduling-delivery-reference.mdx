...
An `API_TRIGGERED` campaign is a reusable shell for the [Messaging API](/api/messaging/messaging-api-overview). It has no audience and no schedule of its own: each trigger request names the recipient and supplies the content values for that send. This changes which components the campaign accepts.

| Component | Status for `API_TRIGGERED` | Reason |
| :--- | :--- | :--- |
| `segmentation_details` | **Not permitted** | The recipient identifier arrives with each trigger request. The shell stores a user-level audience rather than a materialized segment, so there is no segment to define at creation time. |
| `scheduling_details` | **Not permitted** | The campaign is fired on demand. |
| `trigger_condition` | **Not permitted** | The trigger request is the trigger. |
...
| `conversion_goal_details` | Supported | Unchanged. |
| `control_group_details` | Supported | MoEngage does not evaluate control groups for individual trigger requests. |
| `utm_params` | Supported | Unchanged. |

Within `delivery_controls`, only three fields apply:

| Field | Behavior |
| :--- | :--- |
| `ignore_frequency_capping` | Frequency capping is evaluated per recipient at send time. |
| `bypass_dnd` | A recipient in DND is dropped. The message is not deferred or queued for later. |
| `minimum_delay_between_two_notification_in_hour` | Enforced per recipient. |

`campaign_throttle_rpm` and `limit_send_config` are not permitted. Throttling and maximum-send limits do not apply, because delivery pacing is governed by the per-request rate limit on the trigger endpoint rather than by the size of a known audience.
