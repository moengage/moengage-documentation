...
### Overridden and ignored components

Two components behave differently from the rest. MoEngage accepts them without an error and then disregards what you sent, so a request that appears to succeed does not configure what the payload describes.

| Component | Behavior |
| :--- | :--- |
| `segmentation_details` | **Overridden.** MoEngage replaces any audience you send with All Users, and the shell stores All Users regardless of the payload. The audience you send has no effect, and each trigger request names the actual recipients. |
| `scheduling_details` | **Ignored.** A redundant `delivery_type: ASAP` is accepted and has no effect. No other value or field within `scheduling_details` is accepted. |

### Unsupported components

The following are not supported for API-triggered email campaigns.

| Component or field | Condition |
| :--- | :--- |
| `trigger_condition` | Always. The trigger request initiates the send. |
| `campaign_audience_limit` | When `is_campaign_audience_limit_enabled` is `true`. |
| `basic_details.business_event` | Always. |
| `delivery_controls.campaign_throttle_rpm` | Always, and no default RPM is applied. The per-request rate limit on the trigger endpoint governs delivery pacing rather than audience size. |
...
| Idempotent trigger retries | Not supported. The trigger endpoint accepts the `Idempotency-Key` header but does not deduplicate on it. |
