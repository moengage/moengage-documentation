...
An `API_TRIGGERED` campaign is a reusable shell for the [Messaging API](/api/messaging/messaging-api-overview). It has no audience and no schedule: each trigger request specifies the recipients and the content values for that send. This changes which components the campaign accepts.
...
A shell has no audience of its own. Do not send `segmentation_details`: MoEngage rejects a create request that includes it, and stores an All Users audience on the shell instead. Each trigger request names the actual recipients.
...
| `delivery_controls.bypass_dnd` | When `true`. DND always applies, and MoEngage drops the message for a recipient in DND rather than deferring or queueing it. | |
...
| Idempotent trigger retries | Not supported. The trigger endpoint accepts an optional `Idempotency-Key` header but does not deduplicate on it. |
