...
An `API_TRIGGERED` campaign is a reusable shell for the [Messaging API](/api/messaging/messaging-api-overview) and stores a message skeleton rather than a finished message. The fields in `campaign_content.content.email` behave as they do for any other Email campaign. The difference is what you put in them: a standard campaign carries final copy, and a shell carries placeholders that each trigger request populates.
...
- **The trigger request supplies the values.** Each request carries the values for the shell's placeholders and the recipients.

A value in a trigger request takes one of two forms, and a single request can combine both:

| Form | Behavior |
| :--- | :--- |
| Fully resolved | The request carries the final value, and MoEngage passes it through unchanged. Use this form when your application has already rendered the content. |
| Personalization token | The request carries a value that contains personalization tokens, and MoEngage resolves them for each recipient at send time. |
