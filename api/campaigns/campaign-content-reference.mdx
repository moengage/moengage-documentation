...
keywords: ["campaign content", "basic_details", "Android push template", "iOS push template", "web push template", "email content", "html_content", "custom_template_id", "A/B variation", "locale", "SMS content", "MMS content", "mms_media_data"]
...
| [SMS delivery connector](#sms-delivery-connector) | `connector`, `sender_name` (SMS/MMS requests only) |
...
## SMS delivery connector

The `connector` object is part of the SMS/MMS request body (not `campaign_content`) and identifies the delivery provider configured in the workspace. `sender_name` is a sibling field on the request body, not nested inside `connector`.

<Note>
`connector` and `sender_name` are **already confirmed, live fields** — [Get Campaign (V5)](/api/get-campaign-details/get-campaign-v5) already documents them on the SMS read path today, in this same shape (`connector` and `sender_name` as siblings). Per the `patch/mcp-sms` engineering source: on create/update requests, only `connector.connector_name` and `sender_name` are sent — `connector.connector_type` is **not** part of the SMS/MMS request (unlike Email's `connector`, which requires `connector_type`). `connector_type` still appears in the `GET /v5/campaigns/{campaign_id}` response, inferred server-side from `connector_name`.
</Note>
