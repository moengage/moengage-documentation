...
keywords: ["campaign content", "basic_details", "Android push template", "iOS push template", "web push template", "email content", "html_content", "custom_template_id", "A/B variation", "locale", "SMS content"]
...
| [SMS campaign metadata](#sms-campaign-metadata) | `basic_details` on an SMS request |
...
| [SMS delivery connector](#sms-delivery-connector) | `connector`, `sender_name` (SMS requests only) |
...
The `basic_details` object on an SMS campaign carries identifying metadata and SMS-specific compliance fields (`SmsBasicDetailsV5`).
...
<Note>
The PRD describes `team` as a "List of Team Name," which could mean a multi-value field specific to SMS. Documented here as the existing single-string `team` field (matching Push/Email), since neither the PRD nor the `patch/mcp-sms` engineering source confirms a plural form. Confirm with Engineering whether SMS genuinely needs a multi-value team field, and the maximum number of teams per campaign (unresolved in the PRD: "Min - 1, Max - ?").
</Note>
...
<Note>
Applies to SMS campaigns as-is — no new section needed. The PRD's Distribution Type (Manual/Sherpa), percentage split, and number of variations map directly onto the existing `VariationDetails` schema and the `MANUAL`/`SHERPA` tabs below.
</Note>
...
The `connector` object is part of the SMS request body (not `campaign_content`) and identifies the delivery provider configured in the workspace. `sender_name` is a sibling field on the request body, not nested inside `connector`.

<Note>
`connector` and `sender_name` are **already confirmed, live fields** — [Get Campaign (V5)](/api/get-campaign-details/get-campaign-v5) already documents them on the SMS read path today, in this same shape (`connector` and `sender_name` as siblings). Per the `patch/mcp-sms` engineering source: on create/update requests, only `connector.connector_name` and `sender_name` are sent — `connector.connector_type` is **not** part of the SMS request (unlike Email's `connector`, which requires `connector_type`). `connector_type` still appears in the `GET /v5/campaigns/{campaign_id}` response, inferred server-side from `connector_name`.
</Note>
...
| For SMS, `basic_details.name` must be 5–50 characters. | `SmsBasicDetailsV5.name` |
| For SMS, `basic_details.tags` allows a maximum of 30 tags. | `SmsBasicDetailsV5.tags` |
| For SMS, `campaign_delivery_type` is restricted to `ONE_TIME`, `PERIODIC`, `EVENT_TRIGGERED`, `BUSINESS_EVENT_TRIGGERED`. `DEVICE_TRIGGERED`, `LOCATION_TRIGGERED`, and `BROADCAST_LIVE_ACTIVITY` are Push-only. | `SmsCampaignCreateV5Request.campaign_delivery_type` |
