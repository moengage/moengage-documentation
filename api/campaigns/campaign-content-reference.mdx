...
An `API_TRIGGERED` campaign stores the message skeleton rather than a finished message. The fields in `campaign_content.content.email` work exactly as they do for any other Email campaign, with one difference in how you fill them: where a normal campaign carries final copy, a shell carries placeholders that each trigger request fills in.

The division of responsibility is fixed:

- **The shell owns structure.** Subject, sender, HTML body, and template reference all live on the shell. A trigger request cannot change them.
- **The trigger owns values.** Each request supplies the values for the shell's placeholders, along with the recipient.

A value supplied by a trigger request can take either form, and a single request can mix the two:

| Form | Behavior |
| :--- | :--- |
| Fully resolved | The caller sends the final value, and MoEngage uses it as-is. Use this when your system has already rendered the content. |
| Personalization token | The caller sends a value that itself contains personalization tokens. MoEngage resolves them for the recipient at send time. |
...
Images and attachments are referenced by URL only. There is no file upload on the trigger path.
