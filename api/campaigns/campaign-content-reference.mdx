...
keywords: ["campaign content", "basic_details", "Android push template", "iOS push template", "web push template", "email content", "html_content", "custom_template_id", "A/B variation", "locale", "SMS content", "MMS content", "mms_media_data", "tracked URL shortening"]
...
| [SMS campaign metadata](#sms-campaign-metadata) | `basic_details` on an SMS or MMS request |
...
| [SMS content](#sms-content) | `campaign_content.content.sms` |
| [MMS content](#mms-content) | `campaign_content.content.mms` |
...
The `basic_details` object on an SMS or MMS campaign carries identifying metadata and SMS/MMS-specific compliance fields. This section applies to both `SMS` and `MMS` campaigns — they share the same `basic_details` schema (`SmsBasicDetailsV5`).

| Field | Type | Notes |
| :--- | :--- | :--- |
| `name` | string | The campaign name. **Length:** 5–50 characters, per the PRD. |
| `business_event` | string | The business event mapped to the campaign. **Required** when `campaign_delivery_type` is `BUSINESS_EVENT_TRIGGERED`. |
| `content_type` | string | The type of content. Enum: `PROMOTIONAL`, `TRANSACTIONAL`. Reuses the same enum as Email's `content_type`. |
| `tags` | array of string | Free-form context tags. **Maximum:** 30 tags, per the PRD. |
| `team` | string | The team collaborating on the campaign. |
| `user_attribute_identifier` | string | The user attribute that stores the recipient's mobile number. Default: `Mobile Number (Standard)`. |
| `segmentation_details` | object | The campaign audience. Reuses the existing channel-agnostic `SegmentationDetails` schema already used by Push and Email — see [Campaign audience](/api/campaigns/audience-scheduling-delivery-reference#campaign-audience). <Note>The PRD marks this field as unresolved ("Segmentation - ?"). Documented here as a reuse of the existing shared schema, since no SMS-specific shape has been specified anywhere. Confirm with Engineering.</Note> |
| `ignore_optin_preference` | boolean | <Warning>**Guessed field name — not confirmed.** Whether to bypass the recipient's SMS opt-in preference (PRD: "Ignore Opt-in Preference"). No Push/Email equivalent exists to confirm this name against, and the `patch/mcp-sms` engineering source does not mention it. Confirm with Engineering.</Warning> |
| `bypass_prescrub_check` | boolean | <Warning>**Guessed field name — not confirmed, and conditionally *forbidden*, not conditionally required.** Only valid to set when the account has the T-Mobile sentry flag enabled — setting it without that flag is expected to be rejected. This is the one field in the SMS/MMS schema that's conditionally forbidden rather than conditionally required — easy to miss in review. Confirm both the name and the exact validation behavior with Engineering.</Warning> |
| `delivery_strategy` | enum | <Warning>**Guessed field name — not confirmed, and not mentioned in the `patch/mcp-sms` engineering source.** `STANDARD` or `AI_CAMPAIGN_DECISIONING`, per the PRD. A genuinely new concept with no Push/Email equivalent. Confirm with Engineering.</Warning> |
| `agent_name` | string | <Warning>**Guessed field name — not confirmed.** Required if `delivery_strategy` is `AI_CAMPAIGN_DECISIONING`, per the PRD. Same caveat as `delivery_strategy`.</Warning> |

<Note>
The PRD describes `team` as a "List of Team Name," which could mean a multi-value field specific to SMS/MMS. Documented here as the existing single-string `team` field (matching Push/Email), since neither the PRD nor the `patch/mcp-sms` engineering source confirms a plural form. Confirm with Engineering whether SMS/MMS genuinely needs a multi-value team field, and the maximum number of teams per campaign (unresolved in the PRD: "Min - 1, Max - ?").
</Note>

```json
{
  "basic_details": {
    "name": "Order Shipped SMS Alert",
    "content_type": "PROMOTIONAL",
    "tags": ["activation", "order_updates"],
    "team": "marketing_team",
...
`campaign_content.content.sms` carries the SMS message. Two mutually compatible content sources are supported: raw text in `message`, or a saved template referenced by `custom_template_id` — reusing the exact same field Email uses for saved templates. At least one of these must be present.

| Field | Type | Notes |
| :--- | :--- | :--- |
| `message` | string | The SMS text body. **Maximum:** 1600 characters. Optional when `custom_template_id` is provided. |
| `custom_template_id` | string | A saved SMS template ID. Reuses the same field name as `EmailContent.custom_template_id`. Optional when `message` is provided. |
| `tracked_urls` | array of `TrackedUrl` | See [Tracked URLs and shortening](#tracked-urls-and-shortening) below. <Warning>Field name and shape are entirely unconfirmed — see the warning below.</Warning> |

### SMS connector and sender

`connector` and `sender_name` are part of the SMS/MMS request body (not `campaign_content`). These are **already confirmed, live fields** — [Get Campaign (V5)](/api/get-campaign-details/get-campaign-v5) already documents them on the SMS read path today.

<Note>
Per the `patch/mcp-sms` engineering source: on create/update requests, only `connector.connector_name` and `connector.sender_name` are sent — `connector.connector_type` is **not** part of the SMS/MMS request (unlike Email's `connector`, which requires `connector_type`). `connector_type` still appears in the `GET /v5/campaigns/{campaign_id}` response, inferred server-side from `connector_name`.
</Note>

| Field | Type | Notes |
| :--- | :--- | :--- |
| `connector.connector_name` | string | The SMS connector configuration name in the MoEngage workspace. |
| `sender_name` | string | The sender ID or name configured for this connector. |
...
```json
{
  ...
  "campaign_content": {
    "content": {
      "sms": {
        "message": "Your order has shipped. Track it: {{shortened_url}}"
      }
    }
  }
}
...
### Tracked URLs and shortening

<Warning>
**Entirely unconfirmed shape.** No Push/Email precedent exists for this, the PRD does not specify field names, and the `patch/mcp-sms` engineering source does not mention tracked-URL fields at all. Field names below follow this repo's snake_case convention as a guess only — confirm with Engineering before relying on this.
</Warning>

Each tracked URL specifies a `shortening_method`: `CUSTOM_DOMAIN`, `MOE_SHORTENING`, or `DEEPLINK`. These are mutually exclusive per campaign:

- **`CUSTOM_DOMAIN`** — only valid if a custom domain is configured for SMS on the account. Campaign creation fails otherwise.
- **`MOE_SHORTENING`** — only valid if a custom domain is **not** configured. Fails if one is.
- **`DEEPLINK`** — only valid if deeplinking is configured for the account. Requires `web_url`, `alias`, and `key_value_pairs` matching the account's configured dynamic keys. Mismatched keys fail campaign creation. Validation is expected to mirror the existing dashboard deeplink validation, not introduce new rules.
  ...
  "campaign_content": {
    "content": {
      "sms": {
        ...
        "tracked_urls": [
          {
            "url": "https://example.com/orders/{{UserAttribute['Order ID']}}",
            "shortening_method": "MOE_SHORTENING"
          }
        ]
      }
    }
...
`campaign_content.content.mms` carries the MMS message and media. In addition to `message` and `custom_template_id` (same as SMS), MMS uses `mms_media_data` for media attachments.

| Field | Type | Notes |
| :--- | :--- | :--- |
| `message` | string | The MMS text body accompanying the media. **Maximum:** 1600 characters. |
| `custom_template_id` | string | A saved template ID. |
| `mms_media_data` | array of `{ media_url, type }` | Media attachments. **Allowed extensions:** `jpg`, `jpeg`, `gif`, `png`, `mp4`, `webm`. **Maximum:** 10 items. At most one personalized media URL. |

<Note>
An MMS draft can be created without `mms_media_data` (a soft draft). It becomes required when the draft is validated via `POST /v5/campaigns/{campaign_id}/validate`, or when published from the Dashboard.
</Note>
  ...
  "campaign_content": {
    "content": {
      "mms": {
        ...
        "mms_media_data": [
          { "type": "image/jpeg", "media_url": "{{media_url}}" }
        ]
      }
    }
  }
...
<Note>
Applies to SMS and MMS campaigns as-is — no new section needed. The PRD's Distribution Type (Manual/Sherpa), percentage split, and number of variations map directly onto the existing `VariationDetails` schema and the `MANUAL`/`SHERPA` tabs below.
</Note>
...
| For SMS/MMS, `basic_details.name` must be 5–50 characters. | `SmsBasicDetailsV5.name` |
| For SMS/MMS, `basic_details.tags` allows a maximum of 30 tags. | `SmsBasicDetailsV5.tags` |
| For SMS/MMS, `campaign_delivery_type` is restricted to `ONE_TIME`, `PERIODIC`, `EVENT_TRIGGERED`, `BUSINESS_EVENT_TRIGGERED`. `DEVICE_TRIGGERED`, `LOCATION_TRIGGERED`, and `BROADCAST_LIVE_ACTIVITY` are Push-only. | `SmsCampaignCreateV5Request.campaign_delivery_type` |
| For SMS, at most one `shortening_method` per tracked URL: `CUSTOM_DOMAIN` requires a custom domain configured for SMS on the account; `MOE_SHORTENING` requires that no custom domain is configured; `DEEPLINK` requires deeplinking configured for the account, plus matching `web_url`, `alias`, and `key_value_pairs`. <Warning>Unconfirmed — see [Tracked URLs and shortening](#tracked-urls-and-shortening).</Warning> | `TrackedUrl` description |
| For SMS/MMS, `basic_details.bypass_prescrub_check` is only valid to set when the account has the T-Mobile sentry flag enabled — setting it without that flag is expected to be rejected. <Warning>Unconfirmed field name and behavior — see [SMS campaign metadata](#sms-campaign-metadata).</Warning> | `SmsBasicDetailsV5.bypass_prescrub_check` |
| For SMS/MMS, `basic_details.agent_name` is required when `basic_details.delivery_strategy` is `AI_CAMPAIGN_DECISIONING`. <Warning>Unconfirmed field names — see [SMS campaign metadata](#sms-campaign-metadata).</Warning> | `SmsBasicDetailsV5.agent_name` |
...
<Tab title="campaign_content (SMS)">
...
<Tab title="campaign_content (MMS)">
...
<Tab title="control_group_details (SMS)">
...
<Note>
Applies to MMS with `"channel": "MMS"` — the payload shape is identical.
</Note>
