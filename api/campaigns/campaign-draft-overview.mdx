...
<Note>
`API_TRIGGERED` creates a reusable campaign shell for the [Messaging API](/api/messaging/messaging-api-overview). The shell holds content structure and settings, and each trigger request supplies the recipient and the values for that send, so one shell serves many sends. A trigger never creates a campaign, and trigger traffic does not change the shell version. `segmentation_details`, `scheduling_details`, `trigger_condition`, and `campaign_audience_limit` are not permitted, and `delivery_controls` is limited to frequency capping, DND, and minimum delay. See [API-triggered campaigns](/api/campaigns/audience-scheduling-delivery-reference#api-triggered-campaigns).
