...
<Note>
`API_TRIGGERED` creates a reusable campaign shell for the [Messaging API](/api/messaging/messaging-api-overview). The shell holds the content structure and the campaign settings, and each trigger request supplies the recipients and the values for that send, so a single shell serves every message of a given type. A trigger request never creates a campaign, and trigger requests do not change the shell version. `segmentation_details`, `scheduling_details`, `trigger_condition`, and `campaign_audience_limit` are not permitted, and `delivery_controls` is limited to frequency capping, DND, and minimum delay. See [API-triggered campaigns](/api/campaigns/audience-scheduling-delivery-reference#api-triggered-campaigns).
...
<AccordionGroup>
  ...
  <Accordion title="When should I use API_TRIGGERED instead of EVENT_TRIGGERED?">
    Use `EVENT_TRIGGERED` when MoEngage should decide who receives a message, based on an event a user performs and the filters you configure. Use `API_TRIGGERED` when your system decides, and names the recipient in the request.

    An `API_TRIGGERED` shell has no audience and no schedule. It waits for trigger requests, each of which supplies one recipient identifier and the content values for that send.
  </Accordion>
  <Accordion title="Do the campaign creation rate limits apply to API_TRIGGERED shells?">
    Yes. Creating a shell is a campaign creation call and counts against the creation limits below. Firing messages against a published shell does not, because no campaign is created. Trigger traffic is governed by its own per-request rate limit.
  </Accordion>
