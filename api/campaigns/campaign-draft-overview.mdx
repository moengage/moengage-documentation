...
The publish action is currently in beta. Campaigns created as V5 drafts cannot be published yet via this API. Calling the publish action returns a `4xx` error. You can publish the v5 drafts via MoEngage dashboard. For more details, reach out to your MoEngage Customer Success Manager (CSM).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="What API scopes are required to update or publish a campaign?">
    Component-level updates require the `campaigns:create_manage` scope. Publishing a campaign (sending `{ "status": "PUBLISH" }`) requires the `campaigns:create_manage_publish` scope. The publish action is currently in beta and returns a `4xx` error regardless of scope.
  </Accordion>
  <Accordion title="Is there a separate publish endpoint?">
    No. Publishing is a status transition sent through the Update Campaign endpoint. Send `{ "status": "PUBLISH" }` as the request body in a `PATCH /v5/campaigns/{campaign_id}` request. The Update Campaign Status endpoint (`POST /v5/campaigns/{campaign_id}/status`) is a different endpoint that handles post-publish transitions, STOP, PAUSE, and RESUME, and does not accept `PUBLISH`. The publish action is currently in beta and not yet available.
  </Accordion>
