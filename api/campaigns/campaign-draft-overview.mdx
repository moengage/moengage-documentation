...
<AccordionGroup>
  ...
  <Accordion title="Do the campaign creation rate limits apply to API_TRIGGERED shells?">
    Yes. Creating a shell counts against the creation limits below. Triggering messages against a published shell does not, because the request creates no campaign. A separate per-request rate limit governs trigger traffic.
  </Accordion>
