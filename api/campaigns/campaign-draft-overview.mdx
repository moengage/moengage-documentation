...
* [Create Campaign](/api/create-campaigns/create-campaign-draft): Initializes a new Push or Email campaign in a `DRAFT` state using minimal required fields.
* [Get Campaign](/api/get-campaign-details/get-campaign): Retrieves a single campaign in its full deparsed form, displaying its current state and configuration.
* [Update Campaign](/api/update-campaigns/update-campaign): Updates specific components of an existing draft or triggers the formal `PUBLISH` action to launch the campaign.
* [Validate Campaign](/api/create-campaigns/validate-campaign): Safely runs a full publish-time validation check without modifying the draft or changing its state.
* [Update Campaign Status](/api/update-campaigns/update-campaign-status): Applies lifecycle transitions (STOP, PAUSE, RESUME) to campaigns that have already been published.
...
* [Get Campaign Meta](/api/get-campaign-details/get-campaign-meta): Fetches lightweight metadata, including daily cached reachability estimates for scheduled campaigns.
...
* [Get Child Campaigns](/api/get-campaign-details/get-child-campaigns): Retrieves the execution history of a Periodic or Business Event-triggered campaign, returning each child campaign's ID, send time, and status.
* [Personalized Preview](/api/personalized-preview/personalized-preview): Returns a resolved preview of personalized campaign content for a specific user without sending a message.
* [Update Global Control Group](/api/global-control-group/update-global-control-group): Adds or removes users from the Global Control Group in bulk via a CSV file.
...
<AccordionGroup>
  ...
  <Accordion title="How do I find the campaign ID to use in the update request?">
    The `campaign_id` is returned in the `data.id` field of the Create Campaign response. You can also retrieve it using `GET /v5/campaigns/{campaign_id}` or by using the [Search Campaigns](/api/get-campaign-details/search-campaigns) endpoint to look up campaigns by name, status, channel, or delivery type.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which fields are required for a specific Push template type?">
    ...
    For the `Custom` template type, `custom_template_id` is required. For all other types, refer to the `campaign_content` schema in the [Create Campaign](/api/create-campaigns/create-campaign-draft) reference for the full list of required and optional fields per template type.
  </Accordion>
</AccordionGroup>
...
We have made it easy for you to test the APIs in your own environment. Click [here](ADD_POSTMAN_URL) to view and download the Postman collection for the Campaigns API.
