...
keywords: ["Campaigns API", "V5", "campaign draft", "campaign_delivery_type", "ONE_TIME", "PERIODIC", "EVENT_TRIGGERED", "API_TRIGGERED", "Messaging API", "campaign lifecycle", "campaign versioning", "idempotency key"]
---
...
Include an `Idempotency-Key` (UUID v4) on all POST and PATCH requests to ensure safe retries. The exception is [Trigger Message](/api/trigger/trigger-message), which accepts the header but does not deduplicate on it, so a retry sends the message again.
...
- `API_TRIGGERED` (Email only)

<Note>
`API_TRIGGERED` creates a reusable campaign shell for the [Messaging API](/api/messaging/messaging-api-overview). The shell defines the content structure and the campaign settings, and each trigger request specifies one recipient and the values for that send, so a single shell supports repeated trigger requests for one message type. A trigger request never creates a campaign and does not change the shell version. `segmentation_details`, `scheduling_details`, `trigger_condition`, and `campaign_audience_limit` are not permitted, and within `delivery_controls` only `ignore_frequency_capping` and `count_for_frequency_capping` are supported. A shell supports the `PUBLISH` and `STOP` status actions and does not support `PAUSE` or `RESUME`. See [API-triggered campaigns](/api/campaigns/audience-scheduling-delivery-reference#api-triggered-campaigns).
</Note>
...
<Steps>
  ...
  <Step title="Manage">
    Pause, resume, or stop a live campaign. `API_TRIGGERED` campaigns support `STOP` only. Search your workspace and retrieve lightweight metadata across all campaigns.
  </Step>
</Steps>
...
<Warning>
**Campaign publishing is not yet supported in V5, except for `API_TRIGGERED` campaigns.** An `API_TRIGGERED` campaign publishes through the `PUBLISH` action on `PATCH /v5/campaigns/{campaign_id}/status`. To publish every other delivery type, use the V1 API (`PATCH /core-services/v1/campaigns/{campaign_id}`) in the interim.
</Warning>
...
* [Update Campaign](/api/update-campaigns/update-campaign-v5): Updates specific components of an existing draft.
...
* [Update Campaign Status](/api/update-campaigns/update-campaign-status-v5): Applies lifecycle transitions (STOP, PAUSE, RESUME) to campaigns that have already been published, and publishes `API_TRIGGERED` campaigns with the PUBLISH action.
* [Search Campaigns](/api/get-campaign-details/search-campaigns): Searches for campaigns using granular filters, with the ability to explicitly include or exclude campaigns in draft state.
...
* [Test Campaign](/api/test-campaigns/test-campaign): Sends a test push or email to up to 10 users. Supports **inline mode** (pass `channel` and `campaign_content` directly) and **draft mode** (pass `draft_id` to load content from a saved draft). Draft mode is new in V5.
...
<AccordionGroup>
  ...
  <Accordion title="What delivery types are supported for Push and Email campaigns?">
    ...
    - **Email:** `ONE_TIME`, `PERIODIC`, `EVENT_TRIGGERED`, `BUSINESS_EVENT_TRIGGERED`, `API_TRIGGERED`
  </Accordion>
  <Accordion title="When should I use API_TRIGGERED instead of EVENT_TRIGGERED?">
    Use `EVENT_TRIGGERED` when MoEngage determines the recipients, based on an event a user performs and the filters you configure. Use `API_TRIGGERED` when your application determines the recipient and names it in the request.

    An `API_TRIGGERED` shell has no audience and no schedule. It sends only in response to a trigger request, and each request specifies the recipient identifier and the content values for that send.
  </Accordion>
  <Accordion title="Do the campaign creation rate limits apply to API_TRIGGERED shells?">
    Yes. Creating a shell counts against the creation limits below. Triggering messages against a published shell does not, because the request creates no campaign. A separate per-request rate limit governs trigger requests.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="What API scopes are required to update a campaign?">
...
<AccordionGroup>
  ...
  <Accordion title="Can I use this endpoint to publish a campaign?">
    Only for `API_TRIGGERED` campaigns, which publish with the `PUBLISH` action. For every other delivery type, this endpoint applies lifecycle transitions (STOP, PAUSE, RESUME) to campaigns that are already live, and publishing is not yet supported in V5. Use the V1 API in the interim.
  </Accordion>
...
<Warning>
Test Campaign is not supported for `API_TRIGGERED` campaigns. In draft mode, a `draft_id` that points to an `API_TRIGGERED` shell returns an error. In inline mode, the request returns no error, because the endpoint does not accept `campaign_delivery_type` as input. A successful inline test therefore does not confirm that the shell works. To verify a shell, publish it and send a trigger request to a recipient you control. See [API-triggered campaigns](/api/campaigns/audience-scheduling-delivery-reference#api-triggered-campaigns).
</Warning>
