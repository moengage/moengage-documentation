...
<Warning>
  For the `user_time` and `current_time` fields (Track Event and Bulk Import), MoEngage accepts only:

  - **Epoch time in seconds** (10-digit) — for example, `1710740192`
  - **ISO-8601 string** — for example, `2020-05-31T16:33:35Z`

  **Epoch time in milliseconds (13-digit) is not accepted.** If `user_time` or `current_time` is sent in an unsupported or unparseable format, MoEngage does not reject the request; the event is ingested using the server-received (Kafka) time instead, which may cause the event's recorded time to differ from what you intended to send.
</Warning>
...
```
<img alt="monitor_your_usage.png"  src="/images/monitor_your_usage.png" width="50%" />
...
![Newuserprofile](/images/user_profile_new.png)
...
<AccordionGroup>
  ...
  <Accordion title="Can I use this API to delete users from MoEngage?">
    Please use the [Delete User API](/api/user/delete-users) to delete existing users in MoEngage.
  </Accordion>
</AccordionGroup>
