...
- **OAuth 2.0**: Streams supports OAuth 2.0 authentication using the Client Credentials grant type. First, create an OAuth 2.0 configuration in **Settings** \> **Advanced settings** \> **Authorization configuration** (see [Authorization Configuration with OAuth 2.0](/user-guide/settings/advanced-settings/authorization-configuration-with-oauth-20)). Then request OAuth 2.0 enablement for Streams through the support ticket described in [Enable Streams for Your Account](#enable-streams-for-your-account). A self-serve option to select OAuth 2.0 directly in Streams is not yet available. For the setup steps, refer to [Configure OAuth 2.0 for Streams](#configure-oauth-20-for-streams).

To support other authentication methods such as API Key, Bearer Token, and so on, you can communicate the same through the enablement ticket and provide us the Header key and static value to be passed. Apart from OAuth 2.0, where MoEngage fetches and refreshes the access token through your Authorization configuration, we cannot support dynamically refreshing API keys and tokens.
...
<Steps>
  ...
  <Step title="Note the OAuth configuration ID">
    Open the OAuth 2.0 configuration and note its configuration ID, which appears in the page URL. You will need this ID in the enablement ticket.
  </Step>
...
- We can send data to only one static endpoint. If the access token expires, use an OAuth 2.0 configuration so that MoEngage can fetch and refresh the token for you (see [Authentication Methods](#authentication-methods)). For any other authentication method, we cannot integrate with an endpoint where the access token or link expires or is refreshed every few hours.
...
4. If you need all campaign interaction events, we will export such events as mentioned in the [Campaign Interaction Events](#campaign-interaction-events) section.
