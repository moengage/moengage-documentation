...
Use the Flows endpoints to discover flows, view a single flow (including its versions and stages), change a flow's status, and read flow and channel analytics. These endpoints read and control flows; they do not create them.
...
* [Search Flows](/api/flows/search-flows): Lists flows with filters and keyset pagination.
* [Get a Single Flow](/api/flows/get-a-single-flow): Returns a single flow, and optionally a specific version with `version_no`.
* [Get a Specific Version of a Flow](/api/flows/get-a-specific-version-of-a-flow): Returns a flow as it was in a specific version, addressed by its version ID.
* [Update Flow Status](/api/flows/update-flow-status): Pauses, resumes, stops, retires, archives, or unarchives a flow.
* [Get Flow Analytics](/api/flow-analytics/get-flow-analytics): Returns trip, drop, split stage, per-stage, and conversion goal performance for a flow.
* [Get Flow Channel Analytics](/api/flow-analytics/get-flow-channel-analytics): Returns delivery and performance analytics for one channel in a flow.

## Authentication

Authenticate with Basic Auth using your MoEngage Workspace ID (App ID) as the username and your API Key as the password. You can find these in the MoEngage dashboard at **Settings** > **Account** > **APIs**.
...
The four flow endpoints resolve the workspace from the Basic Auth credentials in the `Authorization` header. The two analytics endpoints do not: they read the workspace from the `X-MOE-Tenant-ID` header, so send it together with `X-MOE-SCOPES`. Both headers are required on the analytics endpoints. You can send an optional `X-MOE-Request-Id` on any endpoint.
