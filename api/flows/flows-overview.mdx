...
* [Get Flow Analytics](/api/flow-analytics/get-flow-analytics): Returns trip, drop, split stage, and conversion goal performance for a flow.
...
The four flow endpoints resolve the workspace from the Basic Auth credentials in the `Authorization` header. The two analytics endpoints do not: they read the workspace from the `X-MOE-Tenant-ID` header, so send it together with `X-MOE-SCOPES`. You can send an optional `X-MOE-Request-Id` on any endpoint.
