...
Rate limits vary by API and apply at the workspace level unless stated otherwise. Requests that exceed a limit are rejected with an `HTTP 429` response. Where an API has multiple endpoints with different limits, the specific limit is documented on each endpoint's reference page.

| API | Rate limit |
| --- | --- |
| **Campaigns API** (create campaign) | 5/second, 25/minute, 100/hour per client |
| **Campaign Stats API** | 100/minute per workspace |
| **Catalog API** | 100/minute or 1,000/hour |
| **Custom Segments API — Filter (v3)** | 50/minute, 200/hour, 1,000/day |
| **Custom Segments API — File operations** | 10 operations/hour, 100 operations/day (2 million users/day) |
| **Cards API** (Fetch, Delete) | 50,000/minute per workspace |
| **Recommendations API** | 1,000/minute (up to 10,000/minute on select endpoints) |
| **Track User (Data) API** | 10,000 user updates/minute |
| **Merge Users API** | 1,000 user updates/minute (max 50 users per call) |
| **Personalize Fetch Experience API** | 10,000/minute per workspace (configurable on request) |
| **Business Events API** | Definitions: max 50 business events per workspace. Campaign triggers: 50/hour, 200/day |
| **Cohort Audience API** | 300/minute |
| **Coupons API** | Create: 100 coupon lists/day. Fetch: 10,000 coupon lists/day |
| **Email Subscription API** | Update: 10/minute (14,400/day). Read: 1,000/minute (360,000/day) |
| **Email Templates API** | 1,000/minute per workspace (max 10,000 templates per channel) |
| **Flows API** | 10/second, 100/minute, 6,000/hour per workspace |
| **Inform API** | 10,000/minute |
| **Live Activities API** | Start: 5/minute per workspace. Update: 500/minute per workspace (5/minute per live-activity ID) |
| **Message Archival API** | 1,000/minute |
| **Offerings API** | 150/minute, 500/hour, 1,000/day per consumer (varies by endpoint) |
| **OSM Templates API** | 100/minute (max 100 templates/minute) |
| **Push API** | 10,000/minute |
| **Push Templates API** | 100/minute |
| **SMS Templates API** | 100/minute (max 100 templates per channel) |
| **Subscription Categories API** | 100/minute (360,000/day) |
...
| --- | --- |
| Custom Segments API — File | 150 MB per file |
| Catalog API | 5 MB per request |
| Recommendations API | 1 MB per request |
| Track User (Data) API | 128 KB per request |
| Merge Users API | 128 KB per request |
| GDPR / CCPA API | 128 KB per request (100 KB per user record) |
| Cohort Audience API | 128 KB per request |
| Live Activities API | 5,120 bytes per request (iOS) |
| Email Subscription API | 100 recipient email addresses per request |
