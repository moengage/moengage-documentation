...
MoEngage has two API key experiences. Which one your workspace uses determines where you get your credentials:

- **API key dashboard** (**Settings** \> **Account** \> **API keys**) — create multiple keys, each scoped to the APIs you select.
- **APIs page** (**Settings** \> **Account** \> **APIs**) — one key per feature, with no scoping and no second active key.

Both use the same Workspace ID, and both produce a key you send as the Basic Auth password.
...
Each key is scoped to the APIs you select, so you can issue separate keys for different integrations or teams.
...
Regenerating a key instead of rotating it invalidates the old value immediately, so every caller using it starts failing.
...
Go to **Settings** \> **Account** \> **APIs**, copy your **Workspace ID**, and copy the key for the feature you are calling:
...
![The APIs page in the MoEngage dashboard showing the Workspace ID and the per-feature API key tiles](/images/api-dashboard.png)
...
You can authenticate from a client such as Postman by entering the Workspace ID as the username and the API key as the password under Basic Auth:
