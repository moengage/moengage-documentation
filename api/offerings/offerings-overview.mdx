...
- [Get Offering Meta](/api/public-offerings/get-offering-meta): Returns a single offering's full configuration by ID, including its content, targeting, scheduling, variation, and capping rules.
...
<AccordionGroup>
  ...
  <Accordion title="Can I fetch a single offering by ID?">
    Yes. Use [Get Offering Meta](/api/public-offerings/get-offering-meta) (`GET /v5/offers/{offer_id}`) to retrieve a single offering's full configuration by ID. The response mirrors the Create Offering request body, so you can use it as a reference to replicate or modify the offering.
  </Accordion>
</AccordionGroup>
### Create Offering
