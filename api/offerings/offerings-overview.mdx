...
| `response_id` | Trace identifier for this response. Always present on both success and error responses. Provide this to MoEngage support when reporting an issue. |
...
<AccordionGroup>
  ...
  <Accordion title="Can I include all fields in a single Create request, or do I build it incrementally?">
    You include all fields in a single Create request — there is no incremental or draft-building flow. The offering's initial status is set from the scheduling dates: `scheduled` when `start_datetime` is in the future.
  </Accordion>
