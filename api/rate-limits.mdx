...
Endpoints are grouped by product area, matching the API reference navigation. Where several endpoints share a single pool, the limit is applied across all of them combined.
...
| Endpoint | Rate limit |
| --- | --- |
| `POST /opengdpr_requests/{appId}` — Submit Data Request | No request-rate limit documented; payload capped (see [Payload size limits](#payload-size-limits)) |
...
| `POST /business_event/trigger` — Trigger Business Event | 50 per hour, 200 per day (campaign triggers) |
...
| Endpoint | Rate limit |
| --- | --- |
| `POST /v5/campaigns` — Create Campaign Draft | 5 per second, 25 per minute per client |
...
| Endpoint | Rate limit |
| --- | --- |
| `POST /campaigns` — Create Campaign | 5 per second, 25 per minute per client |
