...
keywords: ["rate limit", "API limits", "429", "RPM", "payload size", "throttling", "Messaging API"]
---
...
| `POST /business_event/trigger` — Trigger Business Event | Campaign triggers: 10 per 5 minutes, 50 per hour, 200 per day. Flow triggers: 3 per hour, 10 per day |
...
| Endpoint | Rate limit |
| --- | --- |
| `POST /v5/campaigns/{campaign_id}/send` — Trigger Message | 5,000 requests per minute per workspace |

<Note>
  MoEngage counts this limit per API request, not per recipient. A single request can carry several recipients, so the effective send rate is the request rate multiplied by the number of recipients in each request. MoEngage rejects requests above the limit and does not queue them.

  Creating a campaign shell is a Campaigns API call and counts against the `POST /v5/campaigns` limit under [Campaigns (V5)](#campaigns-v5). Triggering a message against a published shell does not, because the request creates no campaign. See the [Messaging API Overview](/api/messaging/messaging-api-overview).
</Note>
