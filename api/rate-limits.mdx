...
Some endpoints cap the number of objects a workspace can hold rather than the rate at which you can call them. These caps are not request-rate limits, and exceeding one does not return `HTTP 429`.

| API / endpoint | Maximum |
| --- | --- |
| Create Business Event (`/business_event`) | 50 business events per workspace |
