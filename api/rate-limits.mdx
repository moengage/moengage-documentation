...
<Note>
  This limit is counted per API request, not per recipient. A single request can carry several recipients, so your effective send rate is the request rate multiplied by the number of recipients in each request. Over-limit requests are rejected and are never queued.

  Creating a campaign shell is a Campaigns API call and counts against the `POST /v5/campaigns` limit under [Campaigns (V5)](#campaigns-v5). Triggering messages against a published shell does not, because no campaign is created.
</Note>
