...
The `listener` is notified when a cards sync completes and indicates whether new cards are available. It does not deliver card data directly — call `getCardsForCategory()` or `getCardInfo()` after receiving this callback to fetch the current card list.
