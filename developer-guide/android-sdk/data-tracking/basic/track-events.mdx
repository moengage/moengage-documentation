...
The MoEngage SDK batches events and syncs them to the server on a WorkManager schedule (approximately every 15 minutes when the device is idle and connected). Events tracked during a session may take up to 15 minutes to appear in the MoEngage dashboard — this is expected behaviour.

<AccordionGroup>
<Accordion title="How do I force an immediate sync?">
  If you need to force an immediate sync after a critical event (such as a purchase or logout), call [`MoECoreHelper.syncInteractionData(context)`](https://moengage.github.io/android-api-reference/core/com.moengage.core/-mo-e-core-helper/sync-interaction-data.html?query=fun%20syncInteractionData(context:%20Context,%20appId:%20String)). This flushes all pending events to the server immediately.
</Accordion>
</AccordionGroup>

# Session Management

MoEngage automatically tracks user sessions. A **new session** starts when:
- The app is opened after being in the background for more than **30 minutes**.
- The user explicitly calls `logoutUser()`.

<Info>
  Starting a new session fires an **App Opened** (or **Site Opened**) event. Simply backgrounding and foregrounding the app within 30 minutes does **not** start a new session and does **not** fire App Opened again.
</Info>

| Action | Effect |
|--------|--------|
| App foregrounded within 30 min | Session continues, no new App Opened event |
| App foregrounded after >30 min | New session, App Opened event fires |
| `logoutUser()` called | Session ends immediately regardless of timeout |

<Warning>
  Calling `logoutUser()` is the correct way to end a session when a user signs out of your app. Do not rely on the 30-minute timeout for logout flows — pending events may be lost if the user signs in with a different identity before the timeout expires.
</Warning>

# SDK vs Server-to-Server API Tracking

| | Android SDK | Server-to-Server (S2S) API |
|--|-------------|---------------------------|
| **Device context** | Automatically attached (OS version, device model, app version, locale) | Not attached — must be provided manually |
| **Session tracking** | Automatic | Not automatic — must be implemented separately |
| **User identity** | Resolved via `identifyUser()` at the SDK level | Must be provided in every API request |
| **Event batching** | Yes — up to 15 min delay | No — immediate on request |
| **Use when** | User actions in the app (clicks, views, purchases) | Backend actions not tied to a user session (order shipped, payment processed) |

<Info>
  Use the Android SDK for in-app user actions and the S2S API for backend-originated events. Mixing both for the same event causes duplicate entries in MoEngage analytics.
</Info>
