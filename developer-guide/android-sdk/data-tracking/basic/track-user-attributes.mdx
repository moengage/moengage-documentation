...
keywords: "getUserIdentities, MoEAnalyticsHelper, identifyUser, user identities, multiple identifiers"
---
...
To read the identifiers currently set for the user, call `getUserIdentities()`. It returns a map of identifier names to their values, or `null` when no identifiers are set.

<CodeGroup>

```kotlin Kotlin wrap
val identities: Map<String, String>? = MoEAnalyticsHelper.getUserIdentities(context)
```


```java Java
Map<String, String> identities = MoEAnalyticsHelper.INSTANCE.getUserIdentities(context, null);
```

</CodeGroup>

Call this method from a worker thread. It is annotated `@WorkerThread` and throws `SdkNotInitializedException` if the SDK is not initialized.
