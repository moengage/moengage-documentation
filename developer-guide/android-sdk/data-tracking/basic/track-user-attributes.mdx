...
description: "Track user attributes and set unique identifiers for user identification in the MoEngage Android SDK."
---
...
Before going to production, verify your identity management implementation:

- [ ] Call `identifyUser()` immediately after a user logs in with their unique identifier (email, phone, or internal user ID)
- [ ] Call `identifyUser()` on every app launch if the user is already logged in — the SDK does not persist the identifier across cold starts unless it is re-set
- [ ] Call `MoECoreHelper.logoutUser()` when the user logs out — do not rely on session timeout for logout flows
- [ ] Do not call `identifyUser()` before SDK initialization is complete
...
MoEAnalyticsHelper.setFirstName(context, "John")
MoEAnalyticsHelper.setLastName(context, "Doe")
MoEAnalyticsHelper.setUserName(context, "johndoe")
MoEAnalyticsHelper.setLocation(context, GeoLocation(40.77, 73.98))
MoEAnalyticsHelper.setGender(context, "male")
MoEAnalyticsHelper.setMobileNumber(context, "+14155552671")
MoEAnalyticsHelper.setBirthDate(context, Date())
MoEAnalyticsHelper.setEmailId(context, "john@example.com")
```
...
MoEAnalyticsHelper.INSTANCE.setFirstName(context, "John");
MoEAnalyticsHelper.INSTANCE.setLastName(context, "Doe");
MoEAnalyticsHelper.INSTANCE.setUserName(context, "johndoe");
MoEAnalyticsHelper.INSTANCE.setLocation(context, new GeoLocation(40.77, 73.98));
MoEAnalyticsHelper.INSTANCE.setGender(context, "male");
MoEAnalyticsHelper.INSTANCE.setMobileNumber(context, "+14155552671");
MoEAnalyticsHelper.INSTANCE.setBirthDate(context, new Date());
MoEAnalyticsHelper.INSTANCE.setEmailId(context, "john@example.com");
```

</CodeGroup>

For setting other User Attributes, use the generic method **setUserAttribute()** which accepts multiple overloads. You can pass attributes either individually (key-value pairs) or as a Map. The SDK handles type conversion internally. Supported types are `String`, `Int`, `Long`, `Double`, `Float`, `Boolean`, `Date`, `GeoLocation`, `JSONObject`, and `JSONArray`.
...
// Tracking Array Attributes
MoEAnalyticsHelper.INSTANCE.setUserAttribute(context, "int_array", new Integer[]{1, 2, 3});
MoEAnalyticsHelper.INSTANCE.setUserAttribute(context, "string_array", new String[]{"English", "French"});
MoEAnalyticsHelper.INSTANCE.setUserAttribute(context, "double_array", new Double[]{40.0, 20.0});
MoEAnalyticsHelper.INSTANCE.setUserAttribute(context, "json_array", new JSONArray(Arrays.asList(1, 2, 3)));
MoEAnalyticsHelper.INSTANCE.setUserAttribute(context, "json_object", new JSONObject().put("key", "value"));
// Map-based overload
Map<String, Object> attributes = new HashMap<>();
attributes.put("locality", "SF");
attributes.put("json_object", new JSONObject().put("key", "value"));
MoEAnalyticsHelper.INSTANCE.setUserAttribute(context, attributes);
```
