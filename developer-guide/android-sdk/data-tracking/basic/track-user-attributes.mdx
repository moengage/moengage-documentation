...
For setting other User Attributes, use the generic method `setUserAttribute(key, value)`. Supported value types are `String`, `int`, `long`, `float`, `double`, `boolean`, `Date`, `GeoLocation`, `JSONObject`, `JSONArray`, and arrays of `int`, `long`, `float`, `double`, and `String`.
