...
For setting other User Attributes, use the generic method **setUserAttribute(key, value).** This method accepts a single signature — there are no type-specific overloads. Pass the value directly and the SDK handles the type internally. Supported types for the value are `String`, `Int`, `Long`, `Double`, `Float`, `Boolean`, `Date`, `GeoLocation`, `JSONObject`, and `JSONArray`.
