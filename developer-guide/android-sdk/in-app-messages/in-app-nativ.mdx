...
The MoEngage SDK supports two rendering mechanisms, each available in two display formats:

- **Native In-App** — a popup or full-screen message rendered using standard Android UI elements.
- **Native Nudge** — a non-intrusive banner rendered using standard Android UI elements.
- **HTML In-App** — a popup or full-screen message rendered inside a WebView (HTML/CSS/JS).
- **HTML Nudge** — a non-intrusive banner rendered inside a WebView (HTML/CSS/JS).

<Info>
  The MoEngage SDK determines the correct renderer (Native or HTML) based on your dashboard campaign configuration. Calling `showInApp()` or `showNudge()` triggers this rendering process automatically. Use `showInApp()` for in-app messages and `showNudge()` for nudge messages — you do not need to call both.
</Info>
...
To display a nudge at a specific screen position, pass an `InAppPosition` value to `showNudge()`. Available positions: `ANY` (default), `TOP`, `BOTTOM`, `BOTTOM_LEFT`, `BOTTOM_RIGHT`.
...
<Info>
  The [`data`](https://moengage.github.io/android-api-reference/inapp/com.moengage.inapp.model/-self-handled-campaign-data/index.html) parameter in the `onSuccess` callback can be `null` when no campaign is scheduled for the current user. Always null-check before accessing campaign properties or passing `data` to the tracking APIs (`selfHandledShown`, `selfHandledClicked`, `selfHandledDismissed`).
</Info>

Call `getSelfHandledInApp()` in `onResume()` of your Fragment or `onStart()` of your activity. The method is asynchronous — the callback is invoked once the payload is available.
