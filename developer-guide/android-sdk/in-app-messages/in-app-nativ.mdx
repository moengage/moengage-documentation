...
The SDK determines which renderer to use based on the campaign configuration in the MoEngage dashboard. The same `showInApp()` and `showNudge()` calls trigger the correct renderer automatically.
...
This method triggers rendering for both Native and HTML in-app campaigns. The SDK selects the appropriate renderer based on the campaign configuration.

# Display Nudges

Nudges are non-intrusive banners displayed at the edge of the screen without interrupting the user's flow. Like in-apps, nudges support both Native and HTML rendering — the SDK selects the renderer from the campaign configuration.
