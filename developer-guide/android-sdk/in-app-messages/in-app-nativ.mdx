...
<Info>
  The MoEngage SDK determines which renderer to use based on the campaign configuration in the MoEngage dashboard. The `showInApp()` and `showNudge()` calls trigger the correct renderer automatically.
</Info>
...
<Info>
  The `data` parameter in the `onSuccess` callback can be `null` when no campaign is scheduled for the current user. Always null-check before accessing campaign properties or passing `data` to the tracking APIs (`selfHandledShown`, `selfHandledClicked`, `selfHandledDismissed`).
</Info>
