...
{/* SDK doesn't have initialize() function, consider replacing it with `Ensure you have initialised the SDK within your application entry point`. We are adding initialisation doc link */}
...
<Tip>
  **Prerequisites**
  ...
  - Initialization: Ensure you have initialised the SDK within your application entry point ( `onCreate()` on Android. For more information on initialization, refer to [SDK Initialization](https://www.moengage.com/docs/developer-guide/android-sdk/sdk-integration/basic-integration/sdk-initialization).
</Tip>
...
{/* Consider adding a new section, "How It All Fits Together" — the Flutter and React Native has this: */}
...
{/*Link MoEPersonalizeHelper with API Reference doc and reformatting of the code. Also the 2nd snippet is java not the javascript */}
...
{/* We can consider adding note what will happen when client doesn't call this api or called with different status but expecting payload from different status. SDK will return InValid Key in failure during payload call if the key was not part of last meta fetch */}
{/*Link fetchExperiencesMeta, ExperienceCampaignsMetadata with API Reference doc */}
...
{/* Link ExperienceCampaignsResult with API Ref doc */}
...
{/* Consider adding help or product doc or any backend doc for the exact usage and how this will be helpful */}
Use Cases:
...
{/* Code Formatting, also consider adding comment for experiencesShown vs experienceShown. Currently it is placed under same snippet which might be confusing */}
...
{/* Consider adding a section or sub-section on how to differentiate between normal experience campaign vs offering experience campaign. We can add the details around the payload structure in common to all platform. Anirudh has the added the payload details on ios page (we can move it to common page and link it to all doc) */}

</CodeGroup>

### 3b. Track Impressions for Offering Campaigns

[Offerings](/user-guide/decisioning/new-ui-experience/offerings) are specific, personalized items (such as a product recommendation or a coupon) contained within a campaign.
