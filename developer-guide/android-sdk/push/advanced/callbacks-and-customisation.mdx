...
<Warning>
  **Custom redirection and impression tracking**
  ...
  MoEngage tracks a notification impression only when the push payload reaches the MoEngage SDK. If your application intercepts the push and handles redirection without passing the payload to the SDK — for example, when another service resolves the deep link, such as a Firebase Dynamic Link — MoEngage records no impression for that notification and loses the click attribution that depends on it.

  To customize redirection and keep tracking intact, handle the redirection inside `onNotificationClick()` in your `PushMessageListener` and return `true`. The SDK still receives the payload and tracks both the impression and the click.

  If your application handles the display itself, track impressions and clicks manually as described in [Push Display Handled by Application](/developer-guide/android-sdk/push/advanced/push-display-handled-by-application).
</Warning>
