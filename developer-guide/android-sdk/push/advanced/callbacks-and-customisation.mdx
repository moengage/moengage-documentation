...
<Warning>
  **Impression Tracking**
  ...
  MoEngage tracks a notification impression only when the push payload reaches the SDK. If the application intercepts the push and handles the redirection without passing the payload to the SDK, the impression is not tracked. Click attribution also fails, because it depends on the impression. This happens, for example, when another service resolves the deep link, such as a Firebase Dynamic Link.

  To customize the redirection and keep tracking accurate, handle the redirection inside `onNotificationClick()` in your `PushMessageListener` and return `true`. The SDK receives the payload and tracks both the impression and the click.

  If the application handles the display itself, track impressions and clicks manually. For more information, refer to [Push Display Handled by Application](/developer-guide/android-sdk/push/advanced/push-display-handled-by-application).
</Warning>
