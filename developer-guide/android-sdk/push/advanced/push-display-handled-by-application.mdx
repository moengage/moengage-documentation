...
<Warning>
  MoEngage tracks a notification impression only when the push payload reaches the MoEngage SDK. If your application handles the push itself and never passes the payload to the SDK — including custom redirection where another service resolves the deep link, such as a Firebase Dynamic Link — MoEngage records no impression for that notification and loses the click attribution that depends on it.

  To keep tracking accurate, either call `logNotificationReceived()` and `logNotificationClick()` as described below, or hand the payload back to the SDK with `MoEFireBaseHelper.getInstance().passPushPayload()`. If you customize only the redirection, override `onNotificationClick()` in a `PushMessageListener` instead. For more information, refer to [Callbacks and Customisation](/developer-guide/android-sdk/push/advanced/callbacks-and-customisation).
</Warning>
