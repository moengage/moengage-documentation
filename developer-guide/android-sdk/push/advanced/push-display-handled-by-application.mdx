...
<Warning>
  MoEngage tracks a notification impression only when the push payload reaches the SDK. If the application handles the push itself and does not pass the payload to the SDK, the impression is not tracked. Click attribution also fails, because it depends on the impression. This includes custom redirection where another service resolves the deep link, such as a Firebase Dynamic Link.

  To keep tracking accurate, call `logNotificationReceived()` and `logNotificationClick()` as described below, or pass the payload to the SDK using `MoEFireBaseHelper.getInstance().passPushPayload()`. To customize only the redirection, override `onNotificationClick()` in a `PushMessageListener`. For more information, refer to [Callbacks and Customisation](/developer-guide/android-sdk/push/advanced/callbacks-and-customisation).
</Warning>
