...
## Dismissing Sticky (Ongoing) Notifications

If you create an ongoing (sticky) notification using `setOngoing(true)` on the `NotificationCompat.Builder`, Android does not auto-dismiss it when the user taps it. You must dismiss it explicitly in the click-handler Activity:
...
// In the Activity opened by the notification click PendingIntent
override fun onCreate(savedInstanceState: Bundle?) {
    super.onCreate(savedInstanceState)
    // Log the click for MoEngage analytics
    MoEPushHelper.getInstance().logNotificationClick(applicationContext, intent)
    // Dismiss the sticky notification explicitly
...
<Warning>
  Sticky notifications that are not explicitly cancelled remain visible in the notification drawer indefinitely, even after the user taps them. Always call `NotificationManager.cancel(notificationId)` in the click handler for ongoing notifications.
</Warning>
