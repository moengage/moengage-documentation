...
description: "Generate and upload an FCM private key to the MoEngage Dashboard for Android push notification delivery."
---
...
3. Select **Project settings**.
   ...
   <Frame>
     ![FCM Step1](/images/FCM_Step1.png)
   </Frame>
4. On the **Project Settings** page, navigate to the **Service accounts** tab.

   <Frame>
     ![Firebase Admin SDK](/images/FirebaseAdminSDK.png)
   </Frame>
...
8. After the JSON is configured, enable FCM in the [cloud console](https://console.cloud.google.com/apis/library/fcm.googleapis.com).
   ...
   <Frame>
     ![Enable FCM](/images/EnableFCM.png)
   </Frame>
...
If push notifications are not being delivered after uploading the private key, verify the following:

| Check | What to look for |
|-------|-----------------|
| **Sender ID match** | The Sender ID in `google-services.json` must match the project the service-account JSON was generated from. A mismatch is the most common cause of delivery failure after key rotation. For details on push campaign setup, see [Push Campaign Creation](https://www.moengage.com/docs/user-guide/campaigns-and-channels/mobile-push/create). |
| **FCM API enabled** | The Firebase Cloud Messaging API must be enabled in the [Google Cloud Console](https://console.cloud.google.com/apis/library/fcm.googleapis.com). A disabled API results in `403 Forbidden` errors on all push requests. |
| **Correct JSON file** | Upload the service-account JSON, not the `google-services.json` client config file. They are different files. The service-account JSON contains a `"type": "service_account"` field. |
| **Test and Live environments** | Upload the private key to both the **Test** and **Live** environments in the MoEngage dashboard. For more information on managing push credentials, refer to [Push Configuration](/developer-guide/android-sdk/push/basic/push-configuration). |
| **Single service account per project** | If you regenerated the private key, the old key is immediately invalidated. Verify the dashboard is using the latest downloaded JSON file. |

<Warning>
  A mismatch between the Sender ID in `google-services.json` and the service-account JSON file causes a `MismatchSenderId` error from FCM. All push notifications will silently fail. Re-download both files from the same Firebase project to resolve this.
</Warning>
