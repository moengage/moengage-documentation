...
### Notification Small Icon Rendering

Android renders the small icon as a monochrome silhouette. The system uses the drawable's alpha channel as a mask, ignores the colors inside the drawable, and tints the shape with the color set in `notificationColor` in `NotificationConfig`. If `notificationColor` is not set, Android applies a system default. The icon appears in the tint color, not in the colors present in the drawable.

Android also adjusts the icon's contrast for the device theme, so the same icon appears differently in light mode and dark mode. This rendering is handled by the Android framework. You control the result through the icon artwork and `notificationColor`, not through a MoEngage SDK or dashboard setting.

Design the small icon as a single-color silhouette on a transparent background, and set `notificationColor` to control the color Android applies. For more information, refer to [Notification header](https://developer.android.com/design/ui/mobile/guides/home-screen/notifications#notification-header) in the Android notification design guidelines.
