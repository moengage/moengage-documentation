...
### Why the Small Icon Is White on Transparent

Android renders the notification small icon as a monochrome silhouette. The system treats the drawable's alpha channel as a mask, discards the colors within it, and tints the resulting shape with the notification color you set through `notificationColor` in `NotificationConfig`. When you leave `notificationColor` unset, Android applies a system default. The icon therefore appears in the tint color rather than in the colors you ship inside the drawable.

Android also adjusts the icon's contrast to suit the device theme, so the same icon appears differently in light mode and dark mode. Android owns this rendering step, so you shape the outcome through the icon artwork and `notificationColor` rather than through a MoEngage SDK or dashboard setting.

Design the small icon as a single-color silhouette on a transparent background, following the [Android notification guidelines](https://developer.android.com/develop/ui/views/notifications#Templates). Set `notificationColor` to control the color Android applies.
