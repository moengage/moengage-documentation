...
Your app must also declare `com.google.android.gms:play-services-location` version 21.3.0 or later. The Geofence module builds against version 21.3.0 and does not ship it as a transitive dependency. The MoEngage BOM manages MoEngage module versions only and does not manage the `play-services-location` version.
