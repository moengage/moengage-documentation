...
<Warning>
  **Breaking Change (SDK version X.XX.XX)** The SDK now throws an `IllegalStateException` if any API is invoked before the SDK is initialized. Ensure `MoEngage.initialiseDefaultInstance()` is called in `Application.onCreate()` before invoking any other SDK APIs. For details, see the [release notes](/release-notes/sdks/android#7th-july-2026).
</Warning>
