...
## Override data environment

To override the default behavior and explicitly set the environment the SDK routes data to, use `configureMoEngageEnvironment()` in the builder:
...
val moEngage = MoEngage.Builder(this, "YOUR_Workspace_ID", DataCenter.DATA_CENTER_X)
                     .configureMoEngageEnvironment(
                         MoEngageEnvironmentConfig(MoEngageEnvironment.TEST)
                     )
                     .build()
MoEngage.initialiseDefaultInstance(moEngage)
```
...
MoEngage moEngage = new MoEngage.Builder(this, "YOUR_Workspace_ID", DataCenter.DATA_CENTER_X)
                     .configureMoEngageEnvironment(
                         new MoEngageEnvironmentConfig(MoEngageEnvironment.TEST)
                     )
                     .build();
MoEngage.initialiseDefaultInstance(moEngage);
```
...
`MoEngageEnvironment.DEFAULT` follows the automatic detection described above. Use `TEST` or `LIVE` to explicitly route data regardless of build type.

<Warning>
  Ensure this is not set to `MoEngageEnvironment.TEST` in production builds. Doing so will prevent live data from reaching the MoEngage platform.
</Warning>