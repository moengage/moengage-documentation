...
To enable logs for **native Android**, complete the below steps: 

## Step 1: Enable SDK logs globally at runtime

Use the following API to enable MoEngage SDK logs globally at runtime, without modifying initialization code.
...
<Warning>
  `enableAllLogs()` only captures logs generated after it is called — it does not replay earlier logs. Call `disableAllLogs()` to turn off all logs, including instance-level ones. Remove both calls before pushing to production.
</Warning>

## Step 2: Enable instance-level verbose logs

Add `configureLogs` to your MoEngage builder in `Application.kt` (or `Application.java`) before calling `MoEngage.initialise()`:
...
If you are using React Native, complete [Step 1](#step-1-enable-sdk-logs-globally-at-runtime) and [Step 2](#step-2-enable-instance-level-verbose-logs), then follow [Step 3](#step-3-enable-debug-logs-for-react-native) to enable TypeScript-level log control.

## Step 3: Enable debug logs for React Native

React Native also provides log control at the TypeScript level. Update your MoEngage initialization code in `App.tsx` (or your app entry file) to pass a log configuration:
