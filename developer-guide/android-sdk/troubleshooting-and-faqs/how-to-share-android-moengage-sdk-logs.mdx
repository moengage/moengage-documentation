...
Use the following API to enable workspace-agnostic SDK logs globally at runtime.
...
<Warning>
  `enableAllLogs()` only captures logs generated after it is called — it does not replay earlier logs. To turn off all logs, including instance-level ones, call `disableAllLogs()`. Remove this call before pushing to production.
</Warning>
...
Follow [Step 1](#step-1-enable-sdk-logs-globally-at-runtime) and [Step 2](#step-2-enable-instance-level-verbose-logs) the same as native Android. In addition, React Native provides log control at the TypeScript level. Update your MoEngage initialization code in `App.tsx` (or your app entry file) to pass a log configuration:
