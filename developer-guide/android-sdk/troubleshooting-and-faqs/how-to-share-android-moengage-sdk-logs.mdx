...
Use the following API to enable MoEngage SDK logs globally at runtime, without modifying initialization code. This applies to native Android, React Native (Android side), and Flutter apps.
...
<Warning>
  `enableAllLogs()` only captures logs generated after it is called — it does not replay earlier logs. To capture workspace instance-level logs, you must also add `configureLogs` to your initialization code (see the platform-specific sections below). Call `disableAllLogs()` to turn off all logs, including instance-level ones. Remove both calls before pushing to production.
</Warning>

## Enable debug logs for Android

MoEngage SDK logs are best read in verbose format, which prints almost all the steps the SDK takes at any given time. Add `configureLogs` to your MoEngage builder in `Application.kt` (or `Application.java`) before calling `MoEngage.initialise()`:
...
Flutter does not expose a Dart-level log configuration API. To enable verbose logging, open the Android initialization file at `android/app/src/main/java/<your-package>/MainApplication.kt` (or `MainApplication.java`) and add `configureLogs` to the MoEngage builder:
