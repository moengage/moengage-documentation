...
Use the following API to enable MoEngage SDK logs globally at runtime. This activates logging across SDK components, but does not cover workspace instance-level logs. To capture instance-level logs, add `configureLogs(new LogConfig(LogLevel.VERBOSE, true))` to your initialization code and rebuild the application.
...
React Native provides log control at the TypeScript level. Update your MoEngage initialization code (typically in `App.tsx` or your app entry file) to pass a log configuration:
