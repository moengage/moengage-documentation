...
description: "Enable and share verbose MoEngage Android SDK logs with the support team to resolve issues faster."
---
...
## Enable SDK logs globally at runtime

Use the following API to enable MoEngage SDK logs globally at runtime. This activates logging across SDK components. To capture workspace instance-level logs, add `configureLogs(new LogConfig(LogLevel.VERBOSE, true))` to your initialization code and rebuild the application.
...
<Warning>
  `enableAllLogs()` only captures logs generated after it is called, it does not replay previous logs. To disable all logs, including instance-level logs, call `disableAllLogs()`. Remove both methods before pushing the app to production.
</Warning>
...
React Native provides log control at the TypeScript level. Update your MoEngage initialization code (in `App.tsx` or your app entry file) to pass a log configuration:

```typescript TypeScript
import { MoEInitConfig, MoEPushConfig, MoEngageLogConfig, MoEngageLogLevel } from "react-native-moengage";
import ReactMoE from 'react-native-moengage';

const logConfig = new MoEngageLogConfig(
  MoEngageLogLevel.VERBOSE,
  true /* isEnabledForReleaseBuild — set to false before releasing to production */
);
...
<Warning>
  Set `isEnabledForReleaseBuild` to `false` and remove verbose logging before pushing the app to production.
</Warning>

## Enable debug logs for Flutter

Flutter uses the same Android native initialization for log configuration. Open the Android initialization file, `android/app/src/main/java/<your-package>/MainApplication.kt` (or `MainApplication.java`), and add `configureLogs` to the MoEngage builder:

<CodeGroup>

```kotlin Kotlin
MoEngage moEngage = MoEngage.Builder(this, "YOUR_WORKSPACE_ID", DataCenter.DATA_CENTER_X)
    .configureLogs(LogConfig(logLevel = LogLevel.VERBOSE, isEnabledForReleaseBuilds = true /* set to false before releasing to production */))
    .build()
```

```java Java
MoEngage moEngage = new MoEngage.Builder(this, "YOUR_WORKSPACE_ID", DataCenter.DATA_CENTER_X)
    .configureLogs(new LogConfig(LogLevel.VERBOSE, true /* isEnabledForReleaseBuilds — set to false before releasing to production */))
    .build();
```
...
<Warning>
  Set `isEnabledForReleaseBuilds` to `false` and remove verbose logging before pushing the app to production.
</Warning>
