...
<Warning>
  `enableAllLogs()` only captures logs generated after it is called — it does not replay earlier logs. To turn off all logs, call `disableAllLogs()`. Set `isEnabledForReleaseBuilds` to `false` and remove these calls before pushing the app to production.
</Warning>
