...
Call `enableAllLogs()` to capture workspace-agnostic logs globally at runtime, then add `configureLogs` to your MoEngage builder to enable instance-level verbose logs.
...
// Enable workspace-agnostic logs
enableAllLogs()

// Enable instance-level verbose logs
...
<Warning>
  `enableAllLogs()` only captures logs generated after it is called — it does not replay earlier logs. Set `isEnabledForReleaseBuilds` to `false` and remove both calls before pushing the app to production.
</Warning>
