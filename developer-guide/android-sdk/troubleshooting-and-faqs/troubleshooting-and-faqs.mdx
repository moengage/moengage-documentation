...
`MoEDebuggerActivity` powers on-device debugging for the MoEngage SDK — it lets anyone with access to your MoEngage dashboard capture live SDK logs from a specific test device after your app is already published, without shipping a new build. This helps diagnose issues across any SDK feature (push, in-app messages, data tracking, initialization, and so on). Opening a debug link on that device, generated from the MoEngage dashboard, launches this activity to enable, extend, or stop a logging session for that device.
...
**How to remove it going forward:** If this doesn't fit your organization's security or compliance policies, exclude the `sdk-debugger` module from future builds:
...
This removes the debugger entirely from your app going forward, along with the ability to use this debugging capability for that app.
