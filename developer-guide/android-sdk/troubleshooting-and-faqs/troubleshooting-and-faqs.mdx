...
Push impression tracking can fail for several reasons:

- **When MoEngage handles push by default:**
  - Ensure the SDK isn't initialized with a delay — initialize it in the `onCreate()` method of the Application class, on the main thread (not in a callback). See [SDK Initialization](/developer-guide/android-sdk/sdk-integration/basic-integration/sdk-initialization).
  - Check for network issues (such as Private DNS or an ad blocker) that prevent the SDK from syncing to the MoEngage backend.
- **When MoEngage handles push with customizations:**
  - Confirm the SDK isn't initialized with a delay.
  - Verify that your `PushMessageListener` / `PluginPushCallback` doesn't interfere with the SDK's callbacks.
- **When the app handles push:**
  - Confirm the SDK isn't initialized with a delay.
  - Ensure the app calls `logNotificationReceived()` in the `onMessageReceived()` callback. See [Push Display Handled by Application](/developer-guide/android-sdk/push/advanced/push-display-handled-by-application).
