---
title: "Compliance"
description: "Enable or disable data tracking and the MoEngage Capacitor SDK from the JavaScript layer."
...
<Note>
  While the SDK or data tracking is disabled, the SDK drops `identifyUser()` calls instead of caching them. Call `identifyUser()` again after you re-enable the SDK or data tracking. For more information, refer to [Tracking User Attributes and User Identity](/developer-guide/capacitor-sdk/data-tracking/tracking-user-attributes).
</Note>

## Delete User Data

To delete the current user's profile from the MoEngage server, refer to [Delete User From MoEngage Server](/developer-guide/capacitor-sdk/data-tracking/delete-user-from-moengage-server).
