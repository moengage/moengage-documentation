...
<Warning>
  **Behavior change in SDK 6.0.0:** The SDK no longer automatically logs out the previous user when a new user is detected. Call `logoutUser()` explicitly before identifying a new user to avoid data corruption. Wait for the `logoutCompleted` callback before tracking any events or setting user attributes for the new user.
</Warning>
...
Register a listener for the `logoutCompleted` event to detect successful logout.
