...
To read the identifiers currently set for the user, call `getUserIdentities()`. Available from SDK 7.0.0, it resolves to an object of identifier names and their values, an empty object when no identifiers are set, or `null` if the identifiers could not be read.

<CodeGroup>

```javascript TypeScript
import { MoECapacitorCore } from 'capacitor-moengage-core'
const identities = await MoECapacitorCore.getUserIdentities({ appId: "YOUR_WORKSPACE_ID" })
```
...
-  [Push configuration for Android](/developer-guide/capacitor-sdk/push/basic/android-push-configuration)
-  [Push configuration for iOS](/developer-guide/capacitor-sdk/push/basic/ios-push-configuration)
