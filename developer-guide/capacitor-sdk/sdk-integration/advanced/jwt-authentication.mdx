...
`MoEngage.Builder moEngage = new MoEngage.Builder(this, "YOUR_WORKSPACE_ID", DataCenter.DATA_CENTER_X) //existing configurations
    .configureNetworkRequest(new NetworkRequestConfig(new NetworkAuthorizationConfig(true, true)));`
```
...
The payload for authentication error data is returned via a callback. Register a callback as shown below.

```typescript TypeScript wrap
import { MoECapacitorCore, MoEAuthenticationErrorData } from 'capacitor-moengage-core'; 

MoECapacitorCore.addListener('authenticationError' (data: MoEAuthenticationErrorData) => { 
  // handle in step 4 
});
