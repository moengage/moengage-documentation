...
<Warning>
Do not construct the context argument manually, for example `{id: <landing_page_id>, name: <landing_page_name>}`. Events tracked this way are missing the `moe_lp_formatted_id` attribute. The event still appears on the user profile, but the click is not counted in the landing page campaign statistics. Always pass the `lp_context` object as-is.
</Warning>
