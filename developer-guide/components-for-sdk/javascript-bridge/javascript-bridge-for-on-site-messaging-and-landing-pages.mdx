...
<Warning>
Always pass the `lp_context` object as-is. Do not build the context argument yourself. An event tracked with a manually constructed object is missing the `moe_lp_formatted_id` attribute, so it still appears on the user profile but the click is not counted in the landing page campaign statistics.
</Warning>

```javascript ✅ Correct — pass lp_context as-is
onclick="Moengage.landingPages.trackClick(lp_context, 'Register_Now')"
```

```javascript ❌ Incorrect — do not build the context object yourself
onclick="Moengage.landingPages.trackClick({id: <landing_page_id>, name: <landing_page_name>}, 'Register_Now')"
```
