---
title: "Compliance"
description: "Opt out of data tracking and enable or disable the MoEngage Cordova SDK from the JavaScript layer."
keywords: "consent, opt out, data tracking, disable SDK, GDPR"
---

Use the APIs below to control what the MoEngage SDK tracks, based on the consent a user has given.

## Enable or Disable Data Tracking

To stop the SDK from tracking custom events and user attributes, opt out of data tracking. The SDK rejects all events and user attributes until you opt back in. Data tracking is enabled by default.

<CodeGroup>

```javascript JavaScript
var moe = MoECordova.init(YOUR_WORKSPACE_ID);
moe.optOutDataTracking(true);  // Stop tracking events and user attributes
moe.optOutDataTracking(false); // Resume tracking events and user attributes
```

</CodeGroup>

## Enable or Disable the SDK

To stop the SDK from tracking any user information or sending any data to MoEngage, call `disableSdk()`.

<CodeGroup>

```javascript JavaScript
var moe = MoECordova.init(YOUR_WORKSPACE_ID);
moe.disableSdk();
```

</CodeGroup>

All SDK APIs are non-operational until you call `enableSdk()`. The SDK is enabled by default, so call `enableSdk()` only if you disabled it earlier.

<CodeGroup>

```javascript JavaScript
var moe = MoECordova.init(YOUR_WORKSPACE_ID);
moe.enableSdk();
```

</CodeGroup>

<Note>
  While the SDK or data tracking is disabled, the SDK drops `identifyUser()` calls instead of caching them. Call `identifyUser()` again after you re-enable the SDK or data tracking. For more information, refer to [Tracking User Attributes and User Identity](/developer-guide/cordova-sdk/data-tracking/tracking-user-attributes).
</Note>

## Delete User Data

To delete the current user's profile from the MoEngage server, refer to [Delete User From MoEngage Server](/developer-guide/cordova-sdk/data-tracking/delete-user-from-moengage-server).
