...
keywords: "identifyUser, getUserIdentities, user identities, multiple identifiers, setUniqueId"
---
...
Call `identifyUser()` as soon as the user logs in. The method accepts either a single identifier or a set of identifiers. It is available from Core 10.0.0.

### Login With a Single Identifier

When you pass a string, the SDK sets it as the `uid` identifier.
...
<Info>
  This method replaces the deprecated `setUniqueId()`. If you are using `setUniqueId()`, replace it with `identifyUser()`. Do not use both methods in the same project, because that leads to inconsistent user profile creation and merging in your MoEngage account.
</Info>
...
If your application has multiple identifiers for a user, pass all identifiers as an object of key-value pairs.

<CodeGroup>
```javascript JavaScript
var moe = MoECordova.init(YOUR_WORKSPACE_ID);
moe.identifyUser({ "u_em": "user@example.com", "u_mb": "9999999999" });
```
...
Before you use multiple identifiers, activate the identifiers on the MoEngage Dashboard. For configuration steps, see [Configure multiple identifiers](/user-guide/data/user-data/unified-identity-identity-resolution#configure-multiple-identifiers).
...
- If you call `identifyUser()` multiple times with different identifier names, the SDK appends the new identifier to the already set identifiers.
- If you call `identifyUser()` with an identifier key that is already set, the new value replaces the existing one.

The SDK validates the argument before it sends anything. It logs an error and drops the call when the identity is null, an array, or a type other than a string or an object, when an object key is numeric or is not a string, or when an object value is not a non-null string.

## Retrieving User Identities

To read the identifiers currently set for the user, call `getUserIdentities()`. It is available from Core 10.0.0 and returns a promise that resolves to an object of identifier names and their values.

<CodeGroup>
```javascript JavaScript
var moe = MoECordova.init(YOUR_WORKSPACE_ID);
moe.getUserIdentities().then(function (identities) {
...
When no identifiers are set, the resolved value is an empty object on Android and `null` on iOS. Check the resolved value before you read keys from it.
