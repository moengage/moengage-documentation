...
To enable or disable the SDK and data tracking, refer to [Compliance](/developer-guide/cordova-sdk/compliance/compliance).
...
Logout clears the data and identity of the current user on the device and resets the SDK state. Call `identifyUser()` again when the next user logs in. For how identifiers map to user profiles, refer to [Identity resolution](/user-guide/data/user-data/unified-identity-identity-resolution).
