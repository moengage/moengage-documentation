...
<AccordionGroup>
  ...
  <Accordion title="Why are webhook events not flowing after I reinstalled the Shopify app?">
...
<AccordionGroup>
  ...
  <Accordion title="How often does the catalog sync?">
    ...
    <Note>
      Real-time catalog sync is in early access. The rest of catalog sync is generally available.
    </Note>
  </Accordion>

  <Accordion title="Which products sync to the catalog?">
  ...
  <Accordion title="What product details sync?">
    You define the catalog schema yourself on the **Sync product catalog** tab. MoEngage prefills a default mapping that covers Variation ID, Product ID, Link, Image Link, Vendor Name, SKU, Variation Title, Currency, Price, Quantity, Description, Created At, and Updated At. **Status** is available but is not mapped by default.

    You can change the data type and Shopify source of any prefilled attribute, remove the ones you do not need, and map your own attributes, including your Shopify metafields. For more information, refer to [Sync Your Product Catalog](/developer-guide/ecommerce-platforms/shopify/sync-product-catalog).
  </Accordion>
  ...
  <Accordion title="What are the limitations of automatic catalog synchronization?">
    The attributes, their data types, their Shopify sources, and the catalog currency are fixed once the first sync completes successfully. You cannot rename, edit, or delete those attributes afterward, but you can add new custom attributes at any time.

    Only the products whose Shopify status is **Active** sync to the catalog.
  </Accordion>
</AccordionGroup>
