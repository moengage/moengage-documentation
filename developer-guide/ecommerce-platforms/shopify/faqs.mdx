...
<AccordionGroup>
  ...
  <Accordion title="When are an anonymous user's email and mobile number captured?">
    For an anonymous shopper, email and mobile number are captured at specific points, not on the checkout page:

    - **Email** is captured on the **Thank-You page** (shown after the purchase completes), not on the checkout page.
    - **Mobile number** is captured only if the buyer provides it in the **shipping details** and the store has enabled the option to track the shipping mobile number.
    - The MoEngage SDK is **not available on Shopify Checkout pages**, so no data is captured there. The **Checkout Started** event is dispatched before the shopper reaches the checkout page.

    This is why an anonymous profile may have no email or mobile number until the purchase reaches the Thank-You page.
  </Accordion>
</AccordionGroup>
