...
**Status** is the only attribute in this list that MoEngage does not map by default. Map it yourself if you need it in your catalog.
...
| The SKU does not serve as the unique product identifier. | Select the **Mark unique** checkbox on the **SKU** row before the first sync. MoEngage marks **Variation ID** as unique by default, and you cannot change the mapping after the first sync completes. |
| A product is missing from the catalog. | MoEngage syncs only the products whose Shopify status is **Active**. Check the product's status in Shopify. |
