...
Catalog sync is not enabled by default. To start it, define your catalog schema on the **Sync product catalog** tab and request the first sync. For more information, refer to [Configure Catalog Sync](#configure-catalog-sync).

<Warning>
  **One time activation required**

  Before catalog sync functions, the MoEngage team must activate them for your workspace. If products do not appear after you request the sync, you can contact your Customer Success Manager or raise a support ticket.
</Warning>
...
After the first sync completes, the catalog refreshes automatically once daily between 8–10 AM IST. You can stop the daily sync at any time.
