...
2. Select the **Sync product catalog** tab.
3. In **Catalog name**, enter a name for your catalog.
4. For each attribute in the table, select a **Data type** and a **Shopify source**.
5. Select **Mark unique** for the attribute that uniquely identifies each item in your catalog.
...
![PLACEHOLDER: Sync product catalog tab showing the catalog name, attribute mapping table, and currency selection](/images/PLACEHOLDER-shopify-sync-product-catalog.png)
...
![PLACEHOLDER: Add custom attributes field with a name entered, a data type selected, and the Add attribute button](/images/PLACEHOLDER-shopify-add-custom-attribute.png)
...
Every catalog needs one attribute that identifies each item uniquely. Select **Mark unique** on that attribute's row. You can mark **Variation ID**, **SKU**, or a custom attribute as the unique identifier.
...
![PLACEHOLDER: Catalog sync confirmation dialog stating that attributes, Shopify source, and currency can be configured only once](/images/PLACEHOLDER-shopify-catalog-sync-dialog.png)
