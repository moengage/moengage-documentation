...
keywords: "getUserIdentities, identifyUser, user identities, multiple identifiers"
---

import GetUserIdentitiesScopeNote from '/snippets/get-user-identities-scope-note.mdx';
...
To read the identifiers currently set for the user, call `getUserIdentities()`. Available from plugin version 9.2.0, it returns a map of identifier names to their values, or `null` when no identifiers are set for the user.
...
final Map<String, String>? identities = await _moengagePlugin.getUserIdentities();
print(identities);
```

</CodeGroup>

<GetUserIdentitiesScopeNote />
