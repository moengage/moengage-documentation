...
{/* SDK-TEAM-SUGGESTION (cosmetic):
The `DataSource`, `ExperienceStatus`, and `ExperienceFailureReason` enum
snippets below use `/** ... */` Javadoc-style doc comments. Dart's style
guide recommends `///` triple-slash comments instead (which the class
snippets in this same file already use). Please normalise the enum doc
comments to `///` for consistency with the rest of the file and idiomatic
Dart. Example:
