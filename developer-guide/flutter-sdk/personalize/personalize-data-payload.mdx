...
enum ExperienceFailureReason {
  /// Maps to `USER_IN_CAMPAIGN_CONTROL_GROUP`.
  userInCampaignControlGroup,
  /// Maps to `USER_IN_GLOBAL_CONTROL_GROUP`.
  userInGlobalControlGroup,
  /// Maps to `USER_NOT_IN_SEGMENT`.
  userNotInSegment,
  /// Maps to `INVALID_EXPERIENCE_KEY`.
  invalidExperienceKey,
  /// Maps to `MAX_LIMIT_BREACHED`.
  maxLimitBreached,
  /// Maps to `EXPERIENCE_NOT_ACTIVE`.
  experienceNotActive,
  /// Maps to `EXPERIENCE_EXPIRED`.
  experienceExpired,
  /// Maps to `PERSONALIZATION_FAILED`.
