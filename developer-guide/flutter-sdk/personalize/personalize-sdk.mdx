...
{/* SDK-TEAM-SUGGESTION:
"Flutter SDK Initialization" in the line above is plain text — no hyperlink.
iOS / Android / React Native all link their initialization doc. Please link
this to the canonical Flutter SDK initialization page (the same target the
moengage_flutter installation guide uses).
*/}
...
{/* SDK-TEAM-SUGGESTION (low priority):
The `final personalize = MoEngagePersonalize('YOUR_APP_ID');` line is
repeated in every subsequent code block (§1, §2, §3a, §3b, §4a, §4b — 7
occurrences total). Consider adding a one-line note here that the same
`personalize` instance is reused throughout the rest of the guide, then
omit the re-declaration from the later blocks. Each block becomes shorter
and the reader isn't led to think they should construct a new instance
per call. Trade-off: each block is no longer fully self-contained.
*/}
...
{/* SDK-TEAM-SUGGESTION:
Inconsistent linking — `ExperienceCampaignsResult` and `ExperienceCampaign`
in this paragraph (and at lines 170, 217, 243 in tracking sections) are
plain text/code, but their siblings `ExperienceCampaignFailure`,
`ExperienceCampaignsMetadata`, and `PersonalizeError` ARE linked to the
data-payload page elsewhere in this doc. Please link these two types to
their data-payload entries for consistency.
*/}
...
{/* SDK-TEAM-SUGGESTION:
The sentence above is an orphan lead-in that sits at the end of §2 but
belongs to §3. It duplicates the §3 intro just below (which already states
the same thing more precisely with the impression definition). Please remove
this sentence — §3's own intro covers it.
*/}

## 3. Notify SDK on Showing the Experience (Track Impressions)

{/* SDK-TEAM-SUGGESTION:
Heading style is verbose ("Notify SDK on Showing the Experience (Track
Impressions)") and inconsistent with §4 just below ("Track Clicks"). The iOS
and Android docs use the terser "Track Impressions". React Native matches
Flutter's verbose form. This is a cross-platform consistency item — pick one
heading style and apply across all four docs. Suggested: "Track Impressions"
(matches §4 within this file).
*/}
...
// Or track a single campaign:
personalize.experienceShown(campaigns.first);
```
...
{/* SDK-TEAM-SUGGESTION:
The "Sample Payload" example above is too vague for a client to act on — it
just shows a placeholder key with a comment. The actual on-the-wire shape
(verified against the iOS test fixture and parser) is:
...
— with `<your_offering_key>` configurable per campaign, `value` being a
**stringified JSON array** the client must decode, and each element being
an offering dict with `offering_content` (UI data) and `offering_context`
(SDK attribution data).

The same gap exists in §3b (impressions). The Android dev (Abhishek, PR #124)
has proposed moving this payload-shape sample to a **shared cross-platform
page** linked from iOS / Android / RN / Flutter, rather than duplicating per
platform. See DOC-2009 (owner Shubhra Pati). When that common page lands,
link from here and §3b. Until then, please defer.
*/}

{/* SDK-TEAM-SUGGESTION:
This "Sample Payload" sits only under §4b (clicks), but the offering payload
shape is equally relevant to §3b (impressions). Once the common-payload page
is created, both §3b and §4b should reference it. Until then, consider
either moving the sample up to §3b (where offerings are introduced) or
cross-referencing it from §3b.
*/}
