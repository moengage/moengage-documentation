...
- `failures` — `ExperienceCampaignFailure` objects for keys that could not be resolved, each with a reason and `experienceKeys`.

Throws `PersonalizeError` (with code and message) for system-level failures such as network errors or SDK not initialised."
