...
<Note>
  `fetchExperience` and `fetchExperiences` resolve keys against the result of the most recent `fetchExperiencesMeta` call. If you skip the metadata call, or call it with a status filter that does not include the keys you later request, the subsequent fetch returns `INVALID_EXPERIENCE_KEY` for those keys.
</Note>
...
- `payload` is the campaign's content map. Offerings are nested one level below it.
...
- `value` is a **stringified JSON array**. Parse it with `jsonDecode(value)` to obtain the list of offering maps.
- Each offering map contains:
