...
<Warning>
  **Impression Tracking**
  ...
  MoEngage tracks a notification impression only when the push payload reaches the SDK. If the native layer intercepts the push and handles the redirection without passing the payload to the SDK, the impression is not tracked. Click attribution also fails, because it depends on the impression. This happens, for example, when another service resolves the deep link, such as a Firebase Dynamic Link. The same applies when the application handles foreground clicks outside the plugin.

  Use the plugin push click callback described on this page to customize the redirection. The payload reaches the SDK, so impressions and clicks continue to be tracked.
</Warning>
