...
<Note>
  Call `startGeofenceMonitoring()` only after `MoEngageFlutter.initialise()` has run and location permission has been granted. If the SDK is not yet initialized, the call is ignored silently.
</Note>
