...
<Note>
  Starting with iOS SDK 11.0.0, `presentCardsViewController` and `pushCardsViewController` are annotated `@MainActor`. Objective-C callers are unaffected. Swift code inside a `UIViewController` (as shown above) already runs on the main actor, so these calls compile unchanged. If you call them from a non-main-actor Swift context, hop to the main actor first, for example `await MainActor.run { ... }`. See the [iOS SDK release notes](/release-notes/sdks/ios#22nd-july-2026) for details.
</Note>
...
<Note>
  Starting with iOS SDK 11.0.0, `getCardsViewController` is annotated `@MainActor`. Objective-C callers are unaffected. Swift code inside a `UIViewController` (as shown above) already runs on the main actor, so this call compiles unchanged. If you call it from a non-main-actor Swift context, hop to the main actor first, for example `await MainActor.run { ... }`.
</Note>
