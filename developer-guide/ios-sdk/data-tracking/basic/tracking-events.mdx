...
<Note>
  ...
  - `trackEvent(_:withProperties:)` returns a typed task object ([`MoEngageTrackEventTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageTrackEventTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the one above compile unchanged.
</Note>

Attach `onSuccess` and `onFailure` to the returned task to observe whether the SDK accepted the event:
...
Success means the SDK accepted the event into its batch, not that it reached MoEngage. The SDK syncs batched events on its own schedule and retries failed syncs. To branch on the cause of a failure, switch on `reason.code`:
...
MoEngageSDKAnalytics.sharedInstance.trackEvent("Successful Purchase", withProperties: eventProperties)
    .onFailure { failure in
        ...
        case .invalidParameters:
            // An attribute value failed validation. Refer to Validations and restrictions.
            break
        default:
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `flush()` returns a typed task object ([`MoEngageFlushTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageFlushTask.html)) instead of `Void`, and the `flush(withCompletionBlock:)` variant is deprecated in favour of it. The return value is annotated `@discardableResult`, so direct calls like the ones above compile unchanged.
</Note>

Unlike `trackEvent`, a `flush()` outcome does describe the network sync, so `onFailure` here tells you the batch did not reach MoEngage:
...
MoEngageSDKAnalytics.sharedInstance.flush()
    .onSuccess { result in
        // The batch reached MoEngage.
    }
    .onFailure { failure in
        print("Flush failed: \(failure.reason) - \(failure.message)")
    }
```

```objective-c Objective C wrap
MoEngageFlushTask *task = [[MoEngageSDKAnalytics sharedInstance] flush];
...
[task onFailure:^(MoEngageRequestFailure * _Nonnull failure) {
    NSLog(@"Flush failed: %@ - %@", failure.reason, failure.message);
}];
```

</CodeGroup>

A failed flush does not lose data. The SDK keeps the tracked events and retries them on the next sync attempt.

Handlers run on the main thread, and registering a second handler of the same kind replaces the first. `onFailure` receives a [`MoEngageRequestFailure`](https://moengage.github.io/ios-api-reference/Classes/MoEngageRequestFailure.html): `reason.code` carries a [shared failure code](https://moengage.github.io/ios-api-reference/Classes/MoEngageRequestFailureReason/Code.html) — `sdkNotInitialized`, `networkError`, and `serverError` are the most likely here — and `message` describes the failure. Swift callers can `await` the task's `result()` method instead of attaching handlers.
