...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `flush()` returns a typed task object (`MoEngageFlushTask`) instead of `Void`, and the `flush(withCompletionBlock:)` variant is deprecated. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe whether the sync succeeded. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
