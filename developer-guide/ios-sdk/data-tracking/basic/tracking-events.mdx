...
<Note>
  ...
  - `trackEvent(_:withProperties:)` returns a typed task object (`MoEngageTrackEventTask`) instead of `Void`, so you can observe per-call success or failure through `.onSuccess` / `.onFailure` or the async `result()` method. Direct calls like the one above compile unchanged. Refer to [Observing the Outcome of a Call](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#observing-the-outcome-of-a-call).
</Note>
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `flush()` returns a typed task object (`MoEngageFlushTask`) instead of `Void`, and the `flush(withCompletionBlock:)` variant is deprecated. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe whether the sync succeeded. Refer to [Observing the Outcome of a Call](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#observing-the-outcome-of-a-call).
</Note>
