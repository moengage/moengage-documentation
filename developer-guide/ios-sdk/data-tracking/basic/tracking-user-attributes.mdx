...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `identifyUser` returns a typed task object ([`MoEngageIdentifyUserTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageIdentifyUserTask.html)) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` to read the resulting identities, or `.onFailure` to observe the failure. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
...
<Note>
  ...
  - Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), these methods return a typed task object (`MoEngageSetUserAttributeTask`) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe the outcome. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
...
<Note>
  ...
  - `setUserAttribute` returns a typed task object (`MoEngageSetUserAttributeTask`) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe the outcome. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
