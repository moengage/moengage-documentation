...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `identifyUser` returns a typed task object ([`MoEngageIdentifyUserTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageIdentifyUserTask.html)) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` to read the resulting identities, or `.onFailure` to observe the failure. Refer to [Observing the Outcome of a Call](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#observing-the-outcome-of-a-call).
</Note>
...
<Note>
  ...
  - Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), these methods return a typed task object (`MoEngageSetUserAttributeTask`) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe the outcome. Refer to [Observing the Outcome of a Call](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#observing-the-outcome-of-a-call).
</Note>
...
<Note>
  ...
  - `setUserAttribute` returns a typed task object (`MoEngageSetUserAttributeTask`) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe the outcome. Refer to [Observing the Outcome of a Call](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#observing-the-outcome-of-a-call).
</Note>
