...
keywords: "UserAttributeLevel, MoEngageUserAttributeLevelProject, MoEngageUserAttributeLevelPortfolio, project-level attribute, portfolio-level attribute, attribute level, setUserAttribute level, getUserIdentities, MoEngageGetUserIdentitiesTask, user identities"
---
...
To read the identifiers currently set for the user, call `getUserIdentities()`. It returns a `MoEngageGetUserIdentitiesTask`. Chain `.onSuccess { ... }` to receive a `MoEngageGetUserIdentitiesResult`, whose `identities` property holds the identifier names and their values. The property is an empty dictionary when no identifiers are set.

<CodeGroup>

```swift Swift wrap
MoEngageSDKAnalytics.sharedInstance.getUserIdentities()
    .onSuccess { result in
...
```objective-c Objective C wrap
[[[MoEngageSDKAnalytics sharedInstance] getUserIdentitiesWithWorkspaceId:nil] onSuccess:^(id result) {
    MoEngageGetUserIdentitiesResult *identitiesResult = (MoEngageGetUserIdentitiesResult *)result;
    NSLog(@"%@", identitiesResult.identities);
}];
```

</CodeGroup>

Handlers are dispatched on the main thread. `onFailure` reports `.sdkNotInitialized` when no SDK instance is registered for the workspace. As an alternative to `.onSuccess`, you can await the task's `result()` method. Use one approach per task instance, not both.

<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `getUserIdentities()` returns a typed task object. The earlier completion-block variant, `getUserIdentities(workspaceId:completion:)`, is deprecated.
</Note>
