...
keywords: "UserAttributeLevel, MoEngageUserAttributeLevelProject, MoEngageUserAttributeLevelPortfolio, project-level attribute, portfolio-level attribute, attribute level, setUserAttribute level"
---
...
Refer to our help [document](/user-guide/data/user-data/unified-identity-identity-resolution) to learn more about the feature.

<IdentifyUserNetworkFailure />

<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `identifyUser` returns a typed task object ([`MoEngageIdentifyUserTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageIdentifyUserTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the ones above compile unchanged.
</Note>

Attach `onSuccess` to read the resulting identities, or `onFailure` to observe the failure. `onSuccess` receives a [`MoEngageIdentifyUserResult`](https://moengage.github.io/ios-api-reference/Classes/MoEngageIdentifyUserResult.html) whose `identities` property holds the identifiers the SDK now has for the user:
...
MoEngageSDKAnalytics.sharedInstance.identifyUser(identity: "identifier")
    .onSuccess { result in
        print("User identities: \(result.identities)")
    }
    .onFailure { failure in
        print("Identify user failed: \(failure.reason) - \(failure.message)")
    }
```

```objective-c Objective C wrap
MoEngageIdentifyUserTask *task = [[MoEngageSDKAnalytics sharedInstance] identifyUserWithIdentity:@"identifier" workspaceId:nil];

[task onSuccess:^(id _Nonnull result) {
    MoEngageIdentifyUserResult *identifyResult = (MoEngageIdentifyUserResult *)result;
    NSLog(@"User identities: %@", identifyResult.identities);
}];

[task onFailure:^(MoEngageRequestFailure * _Nonnull failure) {
    NSLog(@"Identify user failed: %@ - %@", failure.reason, failure.message);
}];
```
...
- [Enable/Disable SDK](/developer-guide/ios-sdk/compliance/compliance#enabledisable-sdk)
- [Opt-Out of Data Tracking](/developer-guide/ios-sdk/compliance/compliance#opt-out-of-data-tracking)
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `resetUser()` returns a typed task object ([`MoEngageResetUserTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageResetUserTask.html)) instead of `Void`. On SDK versions below 11.0.0 the equivalent is `resetUser(withCompletionBlock:)`; that variant is deprecated from 11.0.0 in favour of the typed task above.
...
<Note>
  ...
  - Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), these methods return a typed task object ([`MoEngageSetUserAttributeTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageSetUserAttributeTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the ones above compile unchanged. Refer to [Custom User Attributes](#custom-user-attributes) for how to observe the outcome.
</Note>
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026):

  - `setUserAttribute(_:withAttributeName:level:forAppID:)` requires the `value` parameter to conform to `Sendable` for Swift concurrency safety (`Any` → `any Sendable`). The literal values used in these examples (strings, arrays, dictionaries of these) already conform to `Sendable`, so they compile unchanged. Under the Swift 6 language mode or strict concurrency, non-`Sendable` values need to be updated.
  - `setUserAttribute` returns a typed task object ([`MoEngageSetUserAttributeTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageSetUserAttributeTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the ones above compile unchanged.
</Note>

Attach `onSuccess` and `onFailure` to the returned task to observe the outcome. This applies to `setUserAttribute` and to every typed variant, including the default attribute setters such as `setName` and `setEmailID`:
...
MoEngageSDKAnalytics.sharedInstance.setUserAttribute("Bengaluru", withAttributeName: "Current_city")
    .onSuccess { result in
        // The SDK accepted the attribute.
    }
    .onFailure { failure in
        print("Set user attribute failed: \(failure.reason) - \(failure.message)")
    }
```

```objective-c Objective C wrap
MoEngageSetUserAttributeTask *task = [[MoEngageSDKAnalytics sharedInstance] setUserAttribute:@"Bengaluru" withAttributeName:@"Current_city"];
...
[task onFailure:^(MoEngageRequestFailure * _Nonnull failure) {
    NSLog(@"Set user attribute failed: %@ - %@", failure.reason, failure.message);
}];
```

</CodeGroup>

An attribute value that fails validation resolves through `onFailure` with `invalidParameters`. Refer to [Validations and restrictions](#validations-and-restrictions) for the rules and for what happens in `DEBUG` versus release builds.
