...
<Warning>
  Logout is asynchronous. Identify the next user, set user attributes, or track events only after logout completes. The SDK attributes any call made before completion to the previous user, or discards it when it clears the user data.
</Warning>
