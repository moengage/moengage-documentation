...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `showInApp()` returns a typed task object ([`MoEngageShowInAppTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageShowInAppTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the one above compile unchanged.
</Note>

Attach `onSuccess` and `onFailure` to the returned task to find out whether a campaign was displayed. `onSuccess` receives a [`MoEngageShowInAppResult`](https://moengage.github.io/ios-api-reference/Classes/MoEngageShowInAppResult.html) carrying `campaignName`, `campaignId`, and `inAppType`:
...
MoEngageSDKInApp.sharedInstance.showInApp()
    .onSuccess { result in
        print("Displayed campaign: \(result.campaignName)")
    }
...
```objective-c Objective C wrap
MoEngageShowInAppTask *task = [[MoEngageSDKInApp sharedInstance] showInApp];

[task onSuccess:^(id _Nonnull result) {
    MoEngageShowInAppResult *inAppResult = (MoEngageShowInAppResult *)result;
    NSLog(@"Displayed campaign: %@", inAppResult.campaignName);
}];

[task onFailure:^(MoEngageRequestFailure * _Nonnull failure) {
    NSLog(@"In-app not displayed: %@ - %@", failure.reason, failure.message);
}];
```
...
<Warning>
  A failure here usually does not mean something is wrong with your integration. `campaignNotFound` and `anotherCampaignActive` both mean the SDK simply had nothing to display at that moment, which is the normal case most of the time. Do not surface these to your users or treat them as errors.
</Warning>

## InApp Failure Codes

When an in-app request fails, `reason.code` carries a [shared failure code](https://moengage.github.io/ios-api-reference/Classes/MoEngageRequestFailureReason/Code.html) and `moduleCode` carries the precise in-app reason from [`MoEngageInAppRequestFailureReason.ModuleCode`](https://moengage.github.io/ios-api-reference/Classes/MoEngageInAppRequestFailureReason/ModuleCode.html).
...
Downcast `failure.reason` to read the module code:
...
`moduleCode` is `nil` when the failure uses a shared code instead — `sdkNotInitialized` and `networkError` are the most likely. Handlers run on the main thread, and registering a second handler of the same kind replaces the first. Swift callers can `await` the task's `result()` method instead of attaching handlers.
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `MoEngageNudgePosition` migrates to a Swift enum (`.top`, `.bottom`, `.bottomLeft`, `.bottomRight`, `.any`, `.none`). Objective-C callers use the renamed `MoEngageNudgePosition*` constants shown above.
</Note>
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `showNudge` returns a typed task object ([`MoEngageShowNudgeTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageShowNudgeTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the ones above compile unchanged.
</Note>

Attach `onSuccess` and `onFailure` to the returned task to find out whether a nudge was displayed:
...
MoEngageSDKInApp.sharedInstance.showNudge(atPosition: .top)
    .onSuccess { result in
        // A nudge was displayed.
    }
    .onFailure { failure in
        print("Nudge not displayed: \(failure.reason) - \(failure.message)")
    }
```

```objective-c Objective C wrap
MoEngageShowNudgeTask *task = [[MoEngageSDKInApp sharedInstance] showNudgeAtPosition:MoEngageNudgePositionTop];
...
[task onFailure:^(MoEngageRequestFailure * _Nonnull failure) {
    NSLog(@"Nudge not displayed: %@ - %@", failure.reason, failure.message);
}];
```

</CodeGroup>

As with `showInApp`, a `campaignNotFound` failure means there was no eligible nudge to display, not an integration problem. Refer to [InApp Failure Codes](#inapp-failure-codes) for the full list.
...
<Note>
  The completion-block form shown above is deprecated from iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026). Call `getSelfHandledInApp()` without the completion block instead — it returns a typed task object ([`MoEngageGetSelfHandledInAppTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageGetSelfHandledInAppTask.html)) and you attach `onSuccess` to receive the campaign. The same applies to `getSelfHandledInApps`.
</Note>

The task form separates "no campaign available" from a genuine fetch failure, which the completion block could not do:
...
MoEngageSDKInApp.sharedInstance.getSelfHandledInApp()
    .onSuccess { result in
        // Update your UI using the self-handled content.
    }
    .onFailure { failure in
        print("Self-handled in-app unavailable: \(failure.reason) - \(failure.message)")
    }
```

```objective-c Objective C wrap
MoEngageGetSelfHandledInAppTask *task = [[MoEngageSDKInApp sharedInstance] getSelfHandledInApp];
...
[task onFailure:^(MoEngageRequestFailure * _Nonnull failure) {
    NSLog(@"Self-handled in-app unavailable: %@ - %@", failure.reason, failure.message);
}];
```

</CodeGroup>

A `campaignNotFound` failure means no campaign was available, and `metaFetchFailed` means the fetch itself failed. Refer to [InApp Failure Codes](#inapp-failure-codes).
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `selfHandledShown`, `selfHandledClicked`, and `selfHandledDismissed` return a typed task object ([`MoEngageSelfHandledTrackingTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageSelfHandledTrackingTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the ones above compile unchanged.
</Note>

Attach `onSuccess` and `onFailure` to confirm the stat was tracked. Because self-handled campaign stats are yours to report, a silently dropped call here shows up later as under-reported campaign performance:
...
MoEngageSDKInApp.sharedInstance.selfHandledShown(campaignInfo: campaignInfo)
    .onSuccess { result in
        // The impression was tracked.
    }
    .onFailure { failure in
        print("Self-handled stat not tracked: \(failure.reason) - \(failure.message)")
    }
```

```objective-c Objective C wrap
MoEngageSelfHandledTrackingTask *task = [[MoEngageSDKInApp sharedInstance] selfHandledShownWithCampaignInfo:campaignInfo];
...
[task onFailure:^(MoEngageRequestFailure * _Nonnull failure) {
    NSLog(@"Self-handled stat not tracked: %@ - %@", failure.reason, failure.message);
}];
```
