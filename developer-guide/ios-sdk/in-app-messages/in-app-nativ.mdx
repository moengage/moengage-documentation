...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `showInApp()` returns a typed task object (`MoEngageShowInAppTask`) instead of `Void`. Direct calls like the one above compile unchanged. Chaining `.onSuccess { ... }` / `.onFailure { ... }` or calling the async `result()` method reports whether a campaign was displayed. A `campaignNotFound` or `anotherCampaignActive` failure means there was nothing to display, not an integration error. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `showNudge` returns a typed task object (`MoEngageShowNudgeTask`) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe whether a nudge was displayed. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
...
<Note>
  The completion-block form shown above is deprecated from iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026). Call `getSelfHandledInApp()` without the completion block instead — it returns a typed task object ([`MoEngageGetSelfHandledInAppTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageGetSelfHandledInAppTask.html)), and you attach `.onSuccess` to receive the campaign. The same applies to `getSelfHandledInApps`. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `selfHandledShown`, `selfHandledClicked`, and `selfHandledDismissed` return a typed task object (`MoEngageSelfHandledTrackingTask`) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to confirm the stat was tracked. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
