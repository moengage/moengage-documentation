...
<Note>
  The completion-block form shown above is deprecated from iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026). Call `getSelfHandledInApp()` without the completion block instead — it returns a typed task object ([`MoEngageGetSelfHandledInAppTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngageGetSelfHandledInAppTask.html)), and you attach `.onSuccess` to receive the campaign. The same applies to `getSelfHandledInApps`. Refer to [Observing the Outcome of a Call](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#observing-the-outcome-of-a-call).
</Note>
