...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `registerForRemoteNotification`, `registerForRemoteProvisionalNotification`, and `didFailToRegisterForPush()` also return a typed task object (`MoEngagePushTokenTask`) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe the outcome — a failure carries `invalidPushToken` or `pushRegisterFailed`. Refer to [Failure Reason Codes](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#failure-reason-codes).
</Note>
