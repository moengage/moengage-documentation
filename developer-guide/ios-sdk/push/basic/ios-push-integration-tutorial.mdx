...
From SDK version 10.13.0, the SDK manages the app badge count and keeps it in sync with the number of MoEngage notifications in the iOS Notification Center. To manage the badge count in your app instead, add the `MoEngageBadgeUpdateEnabled` key to the root level of your `Info.plist` and set it to `false`. The key defaults to `true`.

<CodeGroup>

```xml Info.plist wrap
<key>MoEngageBadgeUpdateEnabled</key>
<false/>
```
...
<Warning>
  `MoEngageSDKMessaging.sharedInstance.disableBadgeReset(_:)` is deprecated from SDK version 10.13.0. Use the `MoEngageBadgeUpdateEnabled` key in `Info.plist` instead.
</Warning>

<Accordion title="Using an SDK version earlier than 10.13.0?">
  On these versions, call `disableBadgeReset(true)` to stop the SDK from resetting the badge to 0 on app launch. Clicking a notification then decrements the badge by 1, maintaining accurate counts when multiple notifications are present.

  Call `disableBadgeReset(true)` **before** the MoEngage SDK is initialized in `AppDelegate.swift`. Calling it after initialization causes tapping one notification to clear all notifications from the iOS Notification Center.
