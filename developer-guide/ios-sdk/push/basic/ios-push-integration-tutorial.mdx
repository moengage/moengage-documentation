...
Supported image formats also differ by platform. Android displays WebP (`.webp`) images, but iOS supports only [the formats listed by Apple for notification attachments](https://developer.apple.com/documentation/usernotifications/unnotificationattachment#Supported-File-Types). In a campaign that targets both platforms, a `.webp` image is displayed on Android and is not displayed on iOS. Use JPG, PNG, or GIF instead.
