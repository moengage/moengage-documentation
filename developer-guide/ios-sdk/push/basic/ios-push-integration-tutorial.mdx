...
Serve push media over HTTPS so that the NSE can download it. App Transport Security blocks plain HTTP downloads, and the notification then displays without the media.

To serve media over HTTP, add the **App Transport Security Settings** dictionary to the `Info.plist` of your **Notification Service Extension** and set **Allow Arbitrary Loads** to **YES**:
...
<Note>
  Set this in the NSE's `Info.plist`. The app target's `Info.plist` governs only the requests that the app itself makes, so the extension requires its own entry.
</Note>

#### Keep Media Within Apple's Attachment Size Limits

Apple caps the size of a notification attachment, and the cap differs by media type. iOS drops an asset that exceeds the limit and displays the notification without it.
...
Apple sets these limits, not MoEngage. For the current values and the supported file formats for each type, see Apple's [UNNotificationAttachment](https://developer.apple.com/documentation/usernotifications/unnotificationattachment) documentation.
...
Supported image formats also differ by platform. Android renders WebP (`.webp`), while iOS supports only the formats that Apple lists for notification attachments. The same campaign therefore displays the image on Android and omits it on iOS. Use JPG or PNG for campaigns that target both platforms.
