...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `setPushToken(_:)` returns a typed task object ([`MoEngagePushTokenTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngagePushTokenTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the one above compile unchanged. Refer to [Push Registration Callbacks](#push-registration-callbacks) for how to observe the outcome.
</Note>
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `registerForRemoteNotification`, `registerForRemoteProvisionalNotification`, `setUserNotificationCategories`, and `didFailToRegisterForPush()` also return a typed task object ([`MoEngagePushTokenTask`](https://moengage.github.io/ios-api-reference/Classes/MoEngagePushTokenTask.html)) instead of `Void`. The return value is annotated `@discardableResult`, so direct calls like the ones above compile unchanged.
</Note>

## Push Registration Callbacks

Attach `onSuccess` and `onFailure` to any of these tasks to find out whether the device registered with MoEngage. A device that fails to register receives no push notifications, so this is worth logging during integration:
...
MoEngageSDKMessaging.sharedInstance.setPushToken(deviceToken)
    .onSuccess { result in
        // The token reached MoEngage.
    }
    .onFailure { failure in
        print("Push token not registered: \(failure.reason) - \(failure.message)")
    }
```

```objective-c Objective C wrap
MoEngagePushTokenTask *task = [[MoEngageSDKMessaging sharedInstance] setPushToken:deviceToken];
...
[task onFailure:^(MoEngageRequestFailure * _Nonnull failure) {
    NSLog(@"Push token not registered: %@ - %@", failure.reason, failure.message);
}];
```

</CodeGroup>

When a messaging call fails, `reason.code` carries a [shared failure code](https://moengage.github.io/ios-api-reference/Classes/MoEngageRequestFailureReason/Code.html) and `moduleCode` carries the precise reason from [`MoEngageMessagingRequestFailureReason.ModuleCode`](https://moengage.github.io/ios-api-reference/Classes/MoEngageMessagingRequestFailureReason/ModuleCode.html).
...
MoEngageSDKMessaging.sharedInstance.setPushToken(deviceToken)
    .onFailure { failure in
        guard let reason = failure.reason as? MoEngageMessagingRequestFailureReason,
              let rawValue = reason.moduleCode?.intValue,
              let moduleCode = MoEngageMessagingRequestFailureReason.ModuleCode(rawValue: rawValue) else {
            print("Push token not registered: \(failure.reason) - \(failure.message)")
            return
        }
        print("Push token not registered: \(moduleCode) - \(failure.message)")
    }
```
