...
Impression tracking is not limited to rich push. Once the NSE is configured, it tracks impressions for **both text-only and rich push notifications**, in every app state — foreground, background, and killed. A campaign that sends a plain title and body is tracked the same way as one that carries an image or video.
...
The NSE downloads the image, audio, or video before the notification displays. The constraints below apply to that download, whether the media comes from a standard rich push or a push template.

#### Serve Media Over HTTPS

Serve push media over HTTPS. App Transport Security blocks plain HTTP downloads in the NSE, so the asset never attaches and the notification displays without it.

If you must serve media over HTTP, add the **App Transport Security Settings** dictionary to the `Info.plist` of your **Notification Service Extension** and set **Allow Arbitrary Loads** to **YES**:
...
<Note>
  Set this in the NSE's `Info.plist`. Setting it only in the app target's `Info.plist` does not apply to media that the extension downloads.
</Note>

#### Keep Media Within Apple's Attachment Size Limits

Apple caps the size of a notification attachment, and the cap differs by media type. An asset over the limit is not attached, and the notification displays without it.
...
#### When Media Displays for Some Types but Not Others

An image or audio file that renders correctly does not confirm that video will. The download failure is silent: the notification still arrives and displays its title and body, and no error surfaces in the app. When video specifically fails to display, check that it is served over HTTPS and that it is within Apple's size limit for video.

Image format also differs by platform. iOS does not support WebP (`.webp`) for notification attachments, while Android renders it, so the same campaign shows the image on Android and no image on iOS. Use JPG or PNG for campaigns that target both platforms.
...
<AccordionGroup>
  ...
  <Accordion title="Why is rich media (images, GIFs) not showing in my notifications, or why is impression tracking failing?">
    ...
    - **Transport, Format, and Size:** Confirm the media meets the constraints in [Media Requirements](#media-requirements) — served over HTTPS, within Apple's size limit for its type, and not a `.webp` image.
