...
<Note>
  Set this in the NSE's own `Info.plist` — the file inside the extension's folder in Xcode, the group created alongside `NotificationService.swift` — not the app target's. The app target's `Info.plist` governs only the requests that the app itself makes, so the extension requires its own entry.
</Note>
