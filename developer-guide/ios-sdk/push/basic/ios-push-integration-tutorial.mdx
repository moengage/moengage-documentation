...
<Note>
  Set this in the NSE's own `Info.plist`, not the app target's. The NSE's file sits inside the extension's folder in Xcode, the group created alongside `NotificationService.swift`. The app target's `Info.plist` governs only the requests that the app itself makes, so the extension requires its own entry.
</Note>

#### Keep Media Within Apple's Attachment Size Limits

Apple caps notification attachments at 5 MB for audio, 10 MB for images, and 50 MB for video. iOS drops an asset that exceeds its limit and displays the notification without it. Apple sets these limits, not MoEngage. For the current values and the supported file formats, see Apple's [UNNotificationAttachment](https://developer.apple.com/documentation/usernotifications/unnotificationattachment) documentation.

#### Verify Each Media Type Separately

An image or audio file that renders correctly confirms only that media type. The NSE fails silently when a download does not succeed: the notification still arrives and displays its title and body, and the app receives no error. When video alone fails to display, confirm that you serve it over HTTPS and keep it within Apple's size limit for video.
...
<AccordionGroup>
  ...
  <Accordion title="Why is rich media (images, GIFs) not showing in my notifications, or why is impression tracking failing?">
    ...
    - **Transport, Format, and Size:** Confirm that the media meets the [Media Requirements](#media-requirements): served over HTTPS, within Apple's size limit for its type, and in a format that iOS supports.
