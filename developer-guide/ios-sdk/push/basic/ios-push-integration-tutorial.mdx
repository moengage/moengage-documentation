...
keywords: ["Allow Arbitrary Loads", "App Transport Security", "UNNotificationAttachment", "push media size limit", "webp push image", "video not showing"]
---
...
Once you configure the NSE, it tracks impressions for **both text-only and rich push notifications** in every app state: foreground, background, and killed. The SDK tracks a campaign that sends a plain title and body exactly as it tracks one that carries an image or video.
...
The NSE downloads the image, audio, or video before the notification displays. The following requirements apply to that download, whether the media comes from a standard rich push or a push template.

#### Serve Media Over HTTPS

Serve push media over HTTPS so that the NSE can download it. [App Transport Security](https://developer.apple.com/documentation/bundleresources/information-property-list/nsapptransportsecurity) blocks plain HTTP downloads, and the notification then displays without the media.
...
<Note>
  Add the `NSAppTransportSecurity` dictionary to the NSE's `Info.plist`, not to your app target's. In Xcode, the NSE's `Info.plist` is in the extension's folder, next to `NotificationService.swift`. Your app target's `Info.plist` controls only the requests that your app makes, so the extension needs its own entry.
</Note>

#### Keep Media Within Apple's Attachment Size Limits

Apple caps notification attachments at 5 MB for audio, 10 MB for images, and 50 MB for video. iOS drops any asset that exceeds the limit for its media type and displays the notification without the media. Apple sets these limits, not MoEngage. For the current values and the supported file formats, see Apple's [UNNotificationAttachment](https://developer.apple.com/documentation/usernotifications/unnotificationattachment) documentation.

#### Verify Each Media Type Separately

An image or audio file that renders correctly confirms only that media type. The NSE fails silently when a download does not succeed: the notification still arrives and displays its title and body, and the app receives no error. When video alone fails to display, confirm that you serve the video over HTTPS and keep the file within Apple's size limit for video.

Supported image formats also differ by platform. Android renders WebP (`.webp`), while iOS supports only [the formats that Apple lists for notification attachments](https://developer.apple.com/documentation/usernotifications/unnotificationattachment#Supported-File-Types). The same campaign therefore displays the image on Android and omits it on iOS. Use JPG, PNG, or GIF for campaigns that target both platforms.
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `setPushToken(_:)` returns a typed task object instead of `Void`, so you can observe per-call success or failure through `.onSuccess` / `.onFailure` or the async `result()` method. Direct calls like the one above compile unchanged.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Why is rich media (images, GIFs) not showing in my notifications, or why is impression tracking failing?">
    ...
    - **Transport, Format, and Size:** Confirm that the media meets the [Media Requirements](#media-requirements): served over HTTPS, within Apple's size limit for that media type, and in a format that iOS supports.
