...
<Note>
  The completion-block form shown above is deprecated from iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026). Call `logNotificationReceived(withPayload:)` without the completion block instead — it returns a typed task object (`MoEngageLogNotificationTask`), and you attach `.onSuccess` to be notified when the impression is logged. Refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
</Note>
