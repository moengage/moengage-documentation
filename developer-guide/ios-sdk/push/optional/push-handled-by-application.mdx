...
<Note>
  The completion-block form shown above is deprecated from iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026). Call `logNotificationReceived(withPayload:)` without the completion block instead — it returns a typed task object (`MoEngageLogNotificationTask`), and you attach `.onSuccess` to be notified when the impression is logged. Refer to [Observing the Outcome of a Call](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#observing-the-outcome-of-a-call).
</Note>
...
<Note>
  Starting with iOS SDK [11.0.0](/release-notes/sdks/ios#22nd-july-2026), `logNotificationClicked` returns a typed task object (`MoEngageLogNotificationTask`) instead of `Void`. Direct calls like the ones above compile unchanged. Attach `.onSuccess` / `.onFailure` to the returned task to observe the outcome. A `duplicateImpression` failure means the same payload was already tracked in an earlier call, which is not an integration error. Refer to [Failure Reason Codes](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0#failure-reason-codes).
</Note>
