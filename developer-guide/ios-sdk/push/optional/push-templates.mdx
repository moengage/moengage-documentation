...
keywords: ["push template media", "HTTPS", "Allow Arbitrary Loads", "App Transport Security", "UNNotificationAttachment", "video not showing"]
---
...
Templates that carry an image, audio, or video rely on the Notification Service Extension to download the asset before the notification displays. The constraints below apply to that download.

## Serve Media Over HTTPS

Serve push media over HTTPS. App Transport Security blocks plain HTTP downloads in the Notification Service Extension, so the asset never attaches and the notification displays without it.
...
<Note>
  Set this in the Notification Service Extension's `Info.plist`. Setting it only in the app target's `Info.plist` does not apply to media that the extension downloads.
</Note>
