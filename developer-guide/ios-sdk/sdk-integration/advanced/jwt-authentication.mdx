...
<Note>
  Starting with iOS SDK 11.0.0, `MoEngageAuthenticationError.Listener` is `Sendable` and `@MainActor` isolated, and `onError(_:)` is now invoked on the main actor. Conforming types must be safe to use across concurrency domains. The example above is unaffected since it does not capture non-`Sendable` state. See the [iOS SDK release notes](/release-notes/sdks/ios#22nd-july-2026) for details.
</Note>
