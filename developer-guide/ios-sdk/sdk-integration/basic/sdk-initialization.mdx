...
<Danger>
  **Breaking change in iOS SDK 11.0.0:** SDK initialization is now enforced before any public API use across all build configurations. Calling a public API before initializing the SDK now triggers a `fatalError`, including in Release and TestFlight builds, instead of failing silently. Make sure `initializeDefaultTestInstance(_:)` or `initializeDefaultLiveInstance(_:)` runs before any other MoEngage API call. See the [iOS SDK release notes](/release-notes/sdks/ios#22nd-july-2026) for details.
</Danger>

<Note>
  Starting with iOS SDK 11.0.0, `initializeDefaultLiveInstance(_:)` returns a typed task object instead of `Void`. Direct calls like the ones above compile unchanged. To observe the outcome, chain `.onSuccess { ... }` / `.onFailure { ... }` or call the async `result()` method.
</Note>
