...
- **If only the video doesn't display**: Check that the video URL uses HTTPS and that the file is within Apple's attachment size limit for video. The NSE doesn't report an error, so the notification is still delivered with its title and body. Refer to [Media Requirements](/developer-guide/ios-sdk/push/basic/ios-push-integration-tutorial#media-requirements).
- **If the image is in WebP (`.webp`) format**: iOS supports only [the formats listed by Apple for notification attachments](https://developer.apple.com/documentation/usernotifications/unnotificationattachment#Supported-File-Types), which don't include WebP. The same image is displayed on Android but not on iOS. Use JPG, PNG, or GIF.
