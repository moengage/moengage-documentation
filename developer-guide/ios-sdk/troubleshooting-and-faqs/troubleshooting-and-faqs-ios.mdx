...
- **If only video fails while image and audio display**: the video is likely served over HTTP or exceeds Apple's attachment size limit for video. The download fails silently, so the notification still displays its title and body. See [Media Requirements](/developer-guide/ios-sdk/push/basic/ios-push-integration-tutorial#media-requirements).
- **If the image is a `.webp` file**: iOS does not support WebP for notification attachments, though Android renders it. The same campaign shows the image on Android and no image on iOS. Use JPG or PNG.
