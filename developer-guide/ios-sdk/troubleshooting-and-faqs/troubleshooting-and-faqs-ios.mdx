...
- **If only video fails while image and audio display**: the video is likely served over HTTP or exceeds Apple's attachment size limit for video. The download fails silently, so the notification still displays its title and body. See [Media Requirements](/developer-guide/ios-sdk/push/optional/push-templates#media-requirements).
