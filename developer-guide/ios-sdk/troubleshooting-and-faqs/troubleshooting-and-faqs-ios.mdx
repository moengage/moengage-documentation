...
- **If only video fails while image and audio display**: confirm that you serve the video over HTTPS and keep the file within Apple's attachment size limit for video. The NSE fails silently, so the notification still displays its title and body. See [Media Requirements](/developer-guide/ios-sdk/push/basic/ios-push-integration-tutorial#media-requirements).
- **If the image uses the `.webp` format**: Android renders WebP, while iOS supports only [the formats that Apple lists for notification attachments](https://developer.apple.com/documentation/usernotifications/unnotificationattachment#Supported-File-Types). The same campaign therefore displays the image on Android and omits it on iOS. Use JPG, PNG, or GIF.
