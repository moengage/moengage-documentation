...
- **If the image uses the `.webp` format**: Android renders WebP, while iOS supports only the formats that Apple lists for notification attachments. The same campaign therefore displays the image on Android and omits it on iOS. Use JPG or PNG.
