...
To track user attributes and identifiers successfully, ensure that:

- The MoEngage React Native SDK is installed and initialized in your application.
- For workflows that use multiple identifiers, Identity Resolution is enabled in your MoEngage workspace and the relevant identifiers are activated. For configuration steps, see [Configure multiple identifiers](/user-guide/data/user-data/unified-identity-identity-resolution#configure-multiple-identifiers).
...
User identifiers are unique values that persist across multiple sessions and devices, allowing MoEngage to recognize a user as the same individual, even when they switch between different platforms or log in later. This process, known as identity resolution, is critical for maintaining a unified user profile, providing a consistent user experience, and tracking user behavior accurately.
...
Set these identifiers using the `identifyUser()` method.

By default, the `ID` parameter is the identifier used for your workspaces, unless [Identity Resolution](/user-guide/data/user-data/unified-identity-identity-resolution#overview) is enabled and identifiers are activated in your workspace.

**User Attributes:**

User attributes are descriptive information about a user that enhances their profile. They:

- Support segmentation, personalization, and analytics.
- Include details such as name, age, gender, preferences, and purchase history.
- Are set using dedicated methods such as `setUserFirstName()` or `setUserAttribute()`.
- Help you create personalized user experiences. 
...
User attributes and identifiers are central to using MoEngage effectively:

- **Segmentation:**
  - Use attributes to create targeted user groups based on demographics, behavior, and similar factors.
  - Example: Segment users by age or purchase history for specific campaigns.
- **Personalization:**
  - Identifiers ensure a consistent user experience across devices.
  - Attributes enable tailored content such as messages and recommendations.
  - Example: Personalize emails with names and recommend relevant products.
- **Analytics:**
  - Attributes provide context to user actions and behavior data.
  - Analyze conversion rates by segment and feature engagement by demographics.
  - Gain deeper insights for data-driven decisions.

Using attributes and identifiers helps you build more relevant and engaging user experiences.

## Implementing Login/Logout

For SDK versions below 11.2.00, refer to [Setting Unique ID for SDK versions below 11.2.0](/developer-guide/react-native-sdk/data-tracking/setting-unique-id-for-sdk-versions-below-1120).
...
<Note>
  If a key is not specified, the SDK defaults to `uid`, the unique user identifier in MoEngage.
</Note>
...
<Note>
  Before implementing `identifyUser()` with multiple identifiers, you must activate the defined identifiers on the MoEngage dashboard. For configuration steps, see [Configure multiple identifiers](/user-guide/data/user-data/unified-identity-identity-resolution#configure-multiple-identifiers).
</Note>

**Behavior of multiple `identifyUser()` calls**

- When you call `identifyUser()` multiple times, the new identifiers are appended to the existing list rather than replacing them. Here's an example of how this works:
...
// Later call to identifyUser() with the mobile number
ReactMoE.identifyUser({"u_mb": "999999999"});
// The user now has both email and mobile identifiers.
```

</CodeGroup>

- If you call `identifyUser()` with an identifier key that already exists, the new value replaces the existing one. Here's an example of how this works:

<CodeGroup>

```javascript JavaScript
import ReactMoE from 'react-native-moengage';
// First call with the initial email
ReactMoE.identifyUser({"u_em": "abc@xyz.com"});
// Later call to identifyUser() when the user updates their email
ReactMoE.identifyUser({"u_em": "jfk@xyz.com"});
// The user's email is now updated to the most recent value.
```
...
Here, `u_em` and `u_mb` are standard user attributes. To identify a user with more standard user attributes, see [Standard and Custom Attributes](#standard-and-custom-attributes).
...
For detailed instructions on selecting both custom and standard attributes when configuring multiple identifiers, see [Select identifiers](/user-guide/data/user-data/unified-identity-identity-resolution#step-1-select-identifiers).
...
Starting from version 11.x.x of `react-native-moengage`, the default tracking for the custom boolean attribute is `boolean (true/false)` instead of `double (0/1)`. To track the boolean as a double, use `MoEAnalyticsConfig` and pass `true`. By default, this value is `false`, which tracks the boolean as `true`/`false`.
