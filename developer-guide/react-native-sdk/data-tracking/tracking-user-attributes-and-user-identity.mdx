...
To read the identifiers currently set for the user, call `getUserIdentities()`. It resolves to an object of identifier names and their values, or `null` when no identifiers are set for the user.
...
const identities = await ReactMoE.getUserIdentities();
...
<Warning>
  Initialize the SDK before you call this method. If the SDK is not initialized, the promise does not resolve.
</Warning>
