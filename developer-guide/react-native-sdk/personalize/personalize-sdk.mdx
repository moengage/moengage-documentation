...
The returned object is a Promise that holds the `ExperienceCampaignsMetadata` object, containing all necessary metadata for campaign execution. The rejection contains the failure reason and an optional message to identify the specific reason for the request's failure.
...
The returned object is a Promise that holds the `ExperienceCampaignsResult` object containing all necessary metadata for campaign execution. The rejection contains the failure reason and an optional message to identify the specific reason for the request's failure.
