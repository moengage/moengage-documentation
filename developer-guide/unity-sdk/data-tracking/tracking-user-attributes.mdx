...
To read the identifiers currently set for the user, call `MoEngageClient.GetUserIdentities()`. Available from Core 6.0.0, it delivers a dictionary of identifier names and their values to the callback you pass in. The callback receives `null` if the SDK is not initialized.
