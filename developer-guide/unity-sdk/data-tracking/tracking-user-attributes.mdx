...
To read the identifiers currently set for the user, call `MoEngageClient.GetUserIdentities()`. Available from Core 6.0.0, it delivers a dictionary of identifier names and their values to the callback you pass in. The callback receives `null` when no identifiers are set for the user.
...
MoEngageClient.GetUserIdentities(identities => {
    if (identities == null) return;
    foreach (var identity in identities) {
        Debug.Log(identity.Key + " = " + identity.Value);
    }
});
```
...
<Warning>
  Initialize the SDK before you call this method. If the SDK is not initialized, the callback is not invoked at all.
</Warning>
