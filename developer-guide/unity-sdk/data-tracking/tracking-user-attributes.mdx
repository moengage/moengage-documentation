...
Use `MoEAnalyticsHelper.identifyUser()` to set multiple user identities at once instead of calling individual identity methods separately.
...
MoEAnalyticsHelper.IdentifyUser(identities);
```
...
Use `MoEAnalyticsHelper.getUserIdentities()` to retrieve the identities currently set for the user.
...
UserIdentities identities = MoEAnalyticsHelper.GetUserIdentities();
Debug.Log("Unique ID: " + identities.uniqueId);
```
