...
As an alternative to programmatic initialization, you can initialize the SDK using a configuration file. Create a `moengage_config.properties` file in `Assets → Plugins → Android` and specify your workspace ID and data center:

```properties Properties
moengage_workspace_id=YOUR_Workspace_ID
moengage_data_center=DATA_CENTER_X
```

When this file is present, the SDK reads configuration from it at startup. Programmatic initialization takes precedence if both are provided.
