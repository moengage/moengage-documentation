...
Since the TV apps does not have URL and the SDK requires to do a lot of stuff on changing page/screen, it needs to be notified that the page/screen has changed. So you have to call the helper function `Moengage.handlePageChange()` on changing the screen
