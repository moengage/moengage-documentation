...
- **Manual Integration:** Manually configure the SDK parameters and initialization code. For more information, refer to [Option 2: Manual Integration](#option-2-manual-integration).
...
```javascript JavaScript wrap
moengage.setEmailId('abc@xyz.com')
```
...
<Info>
  ...
  `moengage.initialize({appId: 'YOUR_WORKSPACE_ID', useLatest: true});`
</Info>
...
### Does the MoEngage Web SDK support applications that patch native browser APIs?

Yes. If your application uses a framework or library that patches native browser APIs — for example, Angular applications using Zone.js — MoEngage recommends enabling Native Browser API Support to ensure consistent SDK behavior.

For setup instructions, see [Native Browser API Support](/developer-guide/web-sdk/web-sdk-integration/optional-integration/native-browser-api-support).
