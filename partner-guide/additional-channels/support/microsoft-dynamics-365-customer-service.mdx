...
description: "Connect Microsoft Dynamics 365 Customer Service to MoEngage via Power Automate to stream case, SLA, and survey events for contextual campaigns."
---
...
To maintain platform stability, MoEngage limits ingestion of Microsoft Dynamics 365 Customer Service data to a maximum of 500 requests per second (RPS) per workspace. If your workspace exceeds this limit, MoEngage returns an HTTP `429 (Too Many Requests)` status code.

Handle `429` responses in your Power Automate flow by configuring a retry policy on the HTTP action, so events are resubmitted automatically when the limit is reached.

**Retry Exhaustion**: If the HTTP action exhausts all retry attempts, the flow run fails. Resubmit the failed run from the Power Automate run history to replay the request.