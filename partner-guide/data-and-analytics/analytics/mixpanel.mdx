...
| Email Dropped | `MOE_EMAIL_DROP` | Delivery | Email | Tracked when an email is dropped. Email is dropped when it's part of the bounce list, the unsubscribe list, or the spam report list. |
| Email Bounced | `MOE_EMAIL_HARD_BOUNCE` | Delivery | Email | Tracked when our system finds an email is hard bounced. Typically happens when the email address doesn't exist. |
| Email Soft Bounced | `MOE_EMAIL_SOFT_BOUNCE` | Delivery | Email | Tracked when our system finds an email is soft bounced. Typically happens when there is a temporary delivery issue. |
| Email Opened | `MOE_EMAIL_OPEN` | Interaction | Email | Tracked when a user opens an email. |
| Email Clicked | `MOE_EMAIL_CLICK` | Interaction | Email | Tracked when a user clicks on any link in the email. |
| Email Unsubscribed | `MOE_EMAIL_UNSUBSCRIBE` | Interaction | Email | Tracked when a user unsubscribes from receiving emails. |
| Email Spam Complained | `MOE_EMAIL_SPAM` | Interaction | Email | Tracked when a user marks the email as spam. |
...
MoEngage applies an ingestion rate limit and a payload cap to the data that Mixpanel sends into your workspace. When your workspace exceeds the limit, MoEngage returns an HTTP `429 (Too Many Requests)` status code. Refer to Mixpanel's default retry policy to understand how rate limiting affects your data flow.

Mixpanel cohort imports and user or event imports have different limits, so check the figure for the import type you use. For the current limits, refer to [Partner Data Ingestion Rate Limits](/partner-guide/data-and-analytics/ingestion-rate-limits).
