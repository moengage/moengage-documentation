...
These limits apply to imports from Snowflake, Databricks, and BigQuery. Users and events share the same pool, so both entity types draw from one limit.

The limit depends on the type of import:

- A **historical** import fetches all the rows from the configured table or view. The first successful run of any new recurring import is a historical import.
- A **normal** import is every run after that one. These limits match the user limits for file imports.
