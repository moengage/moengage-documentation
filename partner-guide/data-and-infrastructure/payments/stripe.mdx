...
To authenticate requests from Stripe to MoEngage, you need your Workspace ID and Data API Key.

1. In the MoEngage dashboard, navigate to **Settings** \> **Account** \> **APIs**.
...
After you have your endpoint URL and credentials, configure the webhook in your Stripe dashboard.
...
4. Click **Continue**. On the **Choose destination type** step, select **Webhook endpoint**, and then click **Continue**.

   ![Stripe Choose destination type step with Webhook endpoint selected](/images/stripe-destination-type.png)

5. On the **Configure your destination** step, paste the MoEngage endpoint URL you built in Step 1 into the **Endpoint URL** field, enter a descriptive **Destination name**, and then save the destination.

   ![Stripe destination configuration showing the MoEngage endpoint URL and destination name](/images/stripe-configure-destination.png)
...
2. Search for your test user by **Customer ID** (email for checkout and invoice events; Stripe Customer ID for subscription events).
3. Look for the Stripe event names in the user's event history.

Confirm that event records appear with the expected attributes, such as payment amount, currency, and subscription status.
