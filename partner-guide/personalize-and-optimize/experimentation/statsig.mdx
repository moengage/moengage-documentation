...
5. Click **Enable**.
   ...
   ![The Statsig Webhook dialog box showing the webhook signing secret and the integration steps](/images/statsig-webhook-dialog.png)
6. Click the **Event Filtering** tab.
7. Select **Gate Exposures**, **Config Exposures**, **Experiment Exposures**, and **Custom Events**.
8. Click **Save**.

   ![The Event Filtering tab listing the event types you can forward](/images/statsig-event-filtering.png)

The **Webhook** tile carries the **Events** tag. After you enable the webhook, Statsig adds the **Event Filtering** tab to the dialog box.

The dialog box also shows a **Webhook Signing Secret**, which Statsig uses to sign each request. Click **Generate New Key** to create one.
...
Statsig events appear in the event history. Check the following attributes to confirm they arrived correctly:
