...
keywords: "latest android sdk version, current android sdk version, newest android native sdk, android bom version"
rss: true
---
...
* Fixed the auto-dismiss countdown restarting on device rotation. Rotating the device while an In-App was on screen restarted the countdown from the beginning, so campaigns stayed visible past their configured duration, and repeated rotations kept one on screen indefinitely. The countdown now continues from where it left off across orientation changes and dismisses the campaign on schedule.
...
* **Impression and click tracking for self-handled personalized content:** When your app retrieves personalized experiences or offerings through your own backend rather than through the SDK's fetch APIs, you can now report engagement for campaign attribution using `experienceShown()`, `experienceClicked()`, `offeringShown()`, and `offeringClicked()`. Pass the campaign attributes exactly as the dashboard returns them; the `cid` and `experience` keys are mandatory. The SDK queues the event in the background, and the API returns immediately without a completion callback. Batch variants accept multiple items in a single call and validate each entry independently, so a malformed entry does not prevent the SDK from tracking the remaining entries. Content that your app fetches through the SDK's own fetch APIs continues to use the existing typed tracking APIs. For more information, refer [here](/developer-guide/android-sdk/data-tracking/advanced-or-optional/personalize-experience-events-tracking).
...
* **Push to InApp:** Introduced support for triggering and displaying an in-app message upon push notification click. For more information, refer [here](/user-guide/campaigns-and-channels/in-app-message/create/create-in-app-campaign#trigger-criteria).
...
- Refer to the [Migration Guide](/developer-guide/android-sdk/migration/updating-to-12xxx-from-11xxx) for more details.
