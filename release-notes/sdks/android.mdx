...
* **Return MoETask for public APIs:** Public APIs now return a `MoETask` object instead of `void`. This is a breaking change for any code that previously relied on these methods returning nothing. Use the `MoETask` object to track the success or failure of each operation.
