...
<Update label="1st September 2026" description="Core 11.0.0 · Cards 7.0.0 · Inbox 10.0.0 · Geofence 7.0.0 · Personalize 2.0.0">
<CardGroup cols={2}>
<Card title="Plugins">

**Core** <span style={{ color: '#00C896', fontWeight: 'bold' }}>11.0.0</span>

**Cards** <span style={{ color: '#00C896', fontWeight: 'bold' }}>7.0.0</span>

**Inbox** <span style={{ color: '#00C896', fontWeight: 'bold' }}>10.0.0</span>

**Geofence** <span style={{ color: '#00C896', fontWeight: 'bold' }}>7.0.0</span>

**Personalize** <span style={{ color: '#00C896', fontWeight: 'bold' }}>2.0.0</span>
...
- **Updated platform requirements.** The plugin requires Dart 3.6.0 or later and Flutter 3.44.0 or later. Upgrade your toolchain first, otherwise dependency resolution fails.
- **SDK initialization is enforced.** The native SDKs now fail immediately when your app calls any MoEngage API before initialization. Android throws an `IllegalStateException` and iOS raises a `fatalError`. Call `initialise()` before every other MoEngage API.
- Android: The plugin builds with Android Gradle Plugin 9.1.1 and Kotlin 2.3.20. Update your project to a compatible AGP and Kotlin version.
- Android: Compile SDK and Target SDK move to 36. Update `compileSdk` and `targetSdk` in your app's `build.gradle`.
...
- Android: Updated to MoEngage SDK BOM 4.2.0.
- iOS: Updated MoEngage-iOS-SDK to 11.0.0.
- Android: Removed the Remind Later and Snooze notification actions. Campaigns that use these actions no longer display them, and notifications no longer re-post on a snooze timer.
- Android: Deprecated `trackUserPushPreference`. The API works in this release and will be removed in a future version.
- Android: Reduced ANRs during SDK initialization and event handling. Initialization locks now hold the main thread for less time at app start and across foreground and background transitions.
...
- Android: Fixed the storage schema failing to update when upgrading from a native SDK version below 14.09.00.
- Android: Fixed ANRs caused by notification channel setup on app launch. Channel setup no longer blocks the main thread.
- Android: Fixed push notification images rendering only partially. Images now render in full.
- Android: Fixed a push deeplink not starting a new session when `source` is passed as a bundle extra instead of embedded in the deeplink URL. This caused incorrect campaign attribution.
- Android: Fixed HTML nudges disappearing on rotation and other configuration changes when the app does not declare `configChanges` in its manifest.
- Android: Fixed `showInApp()` not displaying a campaign after an earlier campaign was dismissed following an orientation change.
- Android: Fixed video nudges rendering behind the app's own video content on screens that use a `SurfaceView`.
- Android: Fixed HTML nudges overlapping the system navigation bar on some OEMs.
- iOS: Fixed a possible deadlock in scene-based apps when passing URLs to the SDK.
- iOS: Fixed the nudge visibility check to scope correctly by the current screen.
- iOS: Fixed a cropped HTML non-intrusive in-app on device orientation change.
- iOS: Fixed a bottom-positioned nudge rendering above the screen with an unreachable close button when the content exceeded the visible area.
- iOS: Fixed the push received impression being dropped when storage encryption is enabled.
...
- Android: Updated to MoEngage SDK BOM 4.2.0.
- iOS: Updated MoEngage-iOS-SDK to 11.0.0.
- Android: Internal improvements to the Cards UI.

<span style={{ background: 'rgba(212,107,8,0.1)', color: '#D46B08', border: '1px solid #FFE58F', borderRadius: '4px', padding: '2px 8px', fontSize: '12px', fontWeight: '600' }}>Fixes</span>

- Android: Fixed a memory leak on slow or unreliable network connections while the Cards screen is open.
...
<Update label="13th August 2026" description="Core 10.9.0 · Cards 6.6.0 · Inbox 9.6.0 · Geofence 6.6.0 · Personalize 1.1.0">
<CardGroup cols={2}>
<Card title="Plugins">

**Core** <span style={{ color: '#00C896', fontWeight: 'bold' }}>10.9.0</span>

**Cards** <span style={{ color: '#00C896', fontWeight: 'bold' }}>6.6.0</span>

**Inbox** <span style={{ color: '#00C896', fontWeight: 'bold' }}>9.6.0</span>

**Geofence** <span style={{ color: '#00C896', fontWeight: 'bold' }}>6.6.0</span>
...
- Android: Updated to MoEngage SDK BOM 2.3.1
- iOS: Updated MoEngage-iOS-SDK to 10.14.0
- Web: Standardized SDK initialization and API names. For more information, refer to [Standardizing Web SDK APIs for Cross-Platform Consistency](/developer-guide/web-sdk/migration/standardizing-web-sdk-apis-for-cross-platform-consistency).
...
- Fixed an app freeze when fetching inbox messages. Calling `fetchAllMessages()` could block the main thread long enough to trigger an ANR, most noticeably on accounts with a large number of stored messages or on lower-end devices. The fetch now completes without stalling the UI.
