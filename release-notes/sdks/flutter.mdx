...
- Added JWT Authentication support. For more information, refer to [JWT Authentication](/developer-guide/flutter-sdk/sdk-integration/advanced/jwt-authentication).
...
- **SDK initialization is enforced.** The native SDKs now fail immediately when your app calls any other MoEngage API before initialization. Android throws an `IllegalStateException` and iOS raises a `fatalError`. Call `initialise()` before any other MoEngage API.
- Android
  - The plugin builds with Android Gradle Plugin 9.1.1 and Kotlin 2.3.20. Update your project to a compatible AGP and Kotlin version.
  - Compile SDK and Target SDK move to 36. Update `compileSdk` and `targetSdk` in your app's `build.gradle`.
  - Removed the Remind Later and Snooze notification actions. Campaigns that use these actions no longer display them, and notifications no longer re-post on a snooze timer.
...
- Android
  - Updated to MoEngage SDK BOM 4.2.0.
  - Reduced ANRs during SDK initialization and event handling. Initialization locks now hold the main thread for less time at app start and across foreground and background transitions.
- iOS
  - Updated MoEngage-iOS-SDK to 11.0.0.
...
- Android
  - Fixed ANRs caused by notification channel setup on app launch. Channel setup no longer blocks the main thread.
  - Fixed push notification images rendering only partially. Images now render in full.
  - Fixed a push deeplink not starting a new session when `source` is passed as a bundle extra instead of embedded in the deeplink URL. This caused incorrect campaign attribution.
  - Fixed HTML nudges disappearing on rotation and other configuration changes when the app does not declare `configChanges` in its manifest.
  - Fixed `showInApp()` not displaying a campaign after an earlier campaign was dismissed following an orientation change.
  - Fixed video nudges rendering behind the app's own video content on screens that use a `SurfaceView`.
  - Fixed HTML nudges overlapping the system navigation bar on some OEMs.
- iOS
  - Fixed a possible deadlock in scene-based apps when passing URLs to the SDK.
  - Fixed the nudge visibility check to scope correctly by the current screen.
  - Fixed a cropped HTML non-intrusive in-app on device orientation change.
  - Fixed a bottom-positioned nudge rendering above the screen with an unreachable close button when the content exceeded the visible area.
  - Fixed the push received impression being dropped when storage encryption is enabled.
...
- Android
  - Fixed a memory leak on slow or unreliable network connections while the Cards screen is open.
