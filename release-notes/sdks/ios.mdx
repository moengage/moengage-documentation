...
* **Public APIs return typed task objects:** Several APIs that previously returned `Void` now return typed task objects with `onSuccess` and `onFailure` callbacks. Function-reference assignments with an explicit `Void` return type no longer compile.
...
For a step-by-step upgrade path and the full list of affected APIs, refer to [Migration to SDK version 11.0.0](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0). The `onSuccess` and `onFailure` callbacks are documented on each API's own page.
