...
<Danger>
  **Action required before upgrading to 11.0.0:**

  - SDK initialization is now enforced before any public API use. Calling a public API before initializing the SDK now triggers a `fatalError`, so incorrect integration order surfaces immediately instead of failing silently.
  - Several public APIs that previously returned `Void` now return a typed task object. Function-reference assignments with an explicit `Void` return type no longer compile.
  - The public Objective-C enums `MoEngageInAppActionType` and `MoEngageNudgePosition` have been migrated to Swift (see the migration tables below).
</Danger>
...
* Public APIs that previously returned `Void` now return a typed task object (a `MoEngageBaseTask` subclass, for example `MoEngageTrackEventTask`, `MoEngageShowInAppTask`) so you can observe per-call success or failure through `onSuccess` / `onFailure` or the async `result()` method. Modules changed: Core, InApp, Messaging. Direct calls and Objective-C callers compile unchanged, since `@discardableResult` covers ignored return values. Swift function-reference assignments with an explicit `Void` return type no longer compile.
  ...
  ```swift
  // trackEvent (MoEngageSDKAnalytics)
  // Before — compiled
  let trackFn: (String, MoEngageProperties?, String?) -> Void =
      MoEngageSDKAnalytics.sharedInstance.trackEvent

  // After — wrap in an explicit closure
  ...
  // initializeDefaultLiveInstance (MoEngage)
  // Before — compiled
  let initFn: (MoEngageSDKConfig) -> Void = MoEngage.sharedInstance.initializeDefaultLiveInstance

  // After — wrap in an explicit closure
  let initFn: (MoEngageSDKConfig) -> Void = {
      _ = MoEngage.sharedInstance.initializeDefaultLiveInstance($0)
  }
  ```
...
* `showInApp` now returns a typed task object (`MoEngageShowInAppTask`) instead of `Void`, so you can observe per-call success or failure through `onSuccess` / `onFailure` or the async `result()` method. Direct calls and Objective-C callers compile unchanged. Swift function-reference assignments with an explicit `Void` return type no longer compile.
  ...
  ```swift
  // showInApp (MoEngageSDKInApp)
  // Before — compiled
  let showFn: () -> Void = MoEngageSDKInApp.sharedInstance.showInApp

  // After — wrap in an explicit closure
  let showFn: () -> Void = {
      _ = MoEngageSDKInApp.sharedInstance.showInApp()
  }
  ```

* The public Objective-C enums `MoEngageInAppActionType` and `MoEngageNudgePosition` have been migrated to Swift. Update your code to use the Swift equivalents listed below.
...
* Fixed the nudge visibility check to correctly scope by the current screen (MOEN-43868).
* Fixed a cropped HTML non-intrusive in-app on device orientation change (MOEN-41602).
* Fixed a bottom-positioned non-intrusive nudge rendering pushed above the screen with an unreachable close button when the content exceeded the visible area (MOEN-45910).
...
* `setPushToken` now returns a typed task object instead of `Void`, so you can observe per-call success or failure through `onSuccess` / `onFailure` or the async `result()` method. Direct calls and Objective-C callers compile unchanged. Swift function-reference assignments with an explicit `Void` return type no longer compile.
  ...
  ```swift
  // setPushToken (MoEngageSDKMessaging)
  // Before — compiled
  let tokenFn: (Data?) -> Void = MoEngageSDKMessaging.sharedInstance.setPushToken

  // After — wrap in an explicit closure
  let tokenFn: (Data?) -> Void = {
      _ = MoEngageSDKMessaging.sharedInstance.setPushToken($0)
  }
  ```
...
* `presentCardsViewController`, `pushCardsViewController`, and `getCardsViewController` are now annotated `@MainActor`. Objective-C callers are unaffected. Swift callers invoking these methods from a non-main-actor context must now hop to the main actor.
  ...
  ```swift
  // presentCardsViewController
  // Before — compiled anywhere
  func openCards() {
      MoEngageSDKCards.sharedInstance.presentCardsViewController()
  }

  // After — must be on the main actor
  ...
  // …or mark the caller @MainActor so the hop is implicit:
  @MainActor
  func openCards() {
      MoEngageSDKCards.sharedInstance.presentCardsViewController()
  }

  // pushCardsViewController
  // Before — compiled anywhere
  func openCards(in navController: UINavigationController) {
      MoEngageSDKCards.sharedInstance.pushCardsViewController(toNavigationController: navController)
  }

  // After — must be on the main actor
  func openCards(in navController: UINavigationController) async {
      await MainActor.run {
          MoEngageSDKCards.sharedInstance.pushCardsViewController(toNavigationController: navController)
      }
  }
  ...
  // getCardsViewController
  // Before — compiled anywhere
  func openCards() {
      MoEngageSDKCards.sharedInstance.getCardsViewController(withCompletionBlock: { cardsVC in
          // use cardsVC
      })
  }
