...
* **SDK initialization enforced before API use:** The SDK now triggers a `fatalError` across all build configurations (including Release and TestFlight builds) when any public API is invoked before initialization, so incorrect integration order surfaces immediately instead of failing silently.
