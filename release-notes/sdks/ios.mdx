...
<Update label="2nd September 2026" description="SDK 11.1.0">
...
* Fixed duplicate event identifiers during data syncing. An analytics batch identifier could be reused across two sets of events when the app was terminated before pending writes finished. This typically occurred when a notification dismissal was handled while the app was killed. The identifier is now assigned when the batch is sent, so each batch syncs once under its own identifier.

#### Inbox
<span style={{ backgroundColor: 'rgba(212, 56, 13, 0.1)', color: '#D4380D', border: '1px solid #FFCCC7', padding: '2px 10px', borderRadius: '12px', fontSize: '0.85em', fontWeight: 'bold' }}>Breaking Changes</span>

* Inbox APIs now follow Swift's concurrency model. `presentInboxViewController`, `pushInboxViewController`, and `getInboxViewController` are main-actor bound. Swift call sites outside the main actor need to be updated. Objective-C callers are unaffected.
...
* `getInboxViewController` now always invokes its completion block, passing `nil` when no SDK instance is registered for the given Workspace ID.
...
* Fixed rich push media not rendering for dynamic media URLs. Images and other media served from URLs without a recognizable file extension failed to display in the expanded notification. The media type is now resolved from the server response, with the URL extension used as a fallback.
...
* **SDK initialization enforced before API use:** Calling a public API without having called an initialization method at all now triggers a `fatalError` instead of dropping the call, so an incorrect integration order surfaces immediately. This applies to Debug, Release, and TestFlight builds alike. An initialization that runs but does not succeed — for example a wrong Workspace ID or a network failure — does not crash the app; downstream calls report `sdkNotInitialized` through their `onFailure` callback. For the cases where the SDK skips this check, refer to [Initialization Before API Calls](/developer-guide/ios-sdk/sdk-integration/basic/sdk-initialization#initialization-before-api-calls).
* **Public APIs return typed task objects:** Several APIs that previously returned `Void` now return typed task objects with `onSuccess` and `onFailure` callbacks. Function-reference assignments with an explicit `Void` return type no longer compile. For the full API-to-task mapping and the failure reason codes, refer to [API Callbacks and Error Handling](/developer-guide/ios-sdk/sdk-integration/basic/api-callbacks).
...
For a step-by-step upgrade path, refer to [Migration to SDK version 11.0.0](/developer-guide/ios-sdk/migration/migration-to-sdk-version-11-0-0).
