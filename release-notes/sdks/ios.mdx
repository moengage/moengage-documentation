...
<Danger>
  **Action required before upgrading to 11.0.0:**
  ...
  - SDK initialization is now enforced before any public API use. Calling a public API before initializing the SDK now triggers a `fatalError` across all build configurations, including Release and TestFlight builds, so incorrect integration order surfaces immediately instead of failing silently.
