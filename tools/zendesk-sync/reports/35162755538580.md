# Sync report: Create OSM Template API

- **Article**: [35162755538580](https://developers.moengage.com/hc/en-us/articles/35162755538580-Create-OSM-Template-API)
- **YAML**: `api/osm-templates/osm-templates.yaml`
- **Zendesk last updated**: 2026-02-04T07:04:07Z  (draft=False)
- **OpenAPI operation**: `POST /custom-templates/osm`  (operationId: `None`)
...
payload : Payload type should be HTML only. The data type will be a string. No limit restriction on field length For the payload structure, refer here .
...
template_type : string value, three options possible - BANNER, POP_UP, or NUDGE
...
_`meta_info` description differs (similarity 0.52)_
...
Example:
{ "template_name": "test", "template_id": "123456" "template_version": "1", "created_by": "test@ABC.com" }
```
...
_`external_template_id` description differs (similarity 0.47)_
...
_Response code 400 description differs (similarity 0.77)_
...
**OpenAPI**: 
```
Bad Request. Returned when required parameters are missing, invalid, or when a template already exists with the same version, name, or ID.
```


### `response_codes.401.description` — low

_Response code 401 description differs (similarity 0.67)_
...
**OpenAPI**: 
```
Authentication Failure. Returned when authorization parameters are missing or invalid.
```


### `response_codes.415.description` — low

_Response code 415 description differs (similarity 0.82)_
...
**OpenAPI**: 
```
Unsupported Media Type. Returned when the header Content-Type is not provided or is not supported.
```


### `response_codes.429.description` — low

_Response code 429 description differs (similarity 0.39)_
...
**OpenAPI**: 
```
Rate limit breached. Returned when requests per minute exceed the limit or template quota is exceeded.
```
