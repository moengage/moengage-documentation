...
1. [Create a business event](/use-cases/business-events/send-weather-based-contextual-messages#step-1-create-a-business-event)
2. [Create an Email campaign](/use-cases/business-events/send-weather-based-contextual-messages#step-2-create-an-email-campaign)
3. [Trigger the campaign by triggering the business event](/use-cases/business-events/send-weather-based-contextual-messages#step-3-trigger-campaign-by-triggering-business-events)
...
3. Enter the following details:
   ...
   - **User property with email address**: Select _Email (Standard)_.
     <Frame>
       ![2 1](/images/2-1.png)
     </Frame>
...
6. Select the user property **City** and map it with the Business Event Property _City_. So whenever in the business event API, the value in the event property matches the same of the user’s city, all those matched users will receive the email communication.
   <Frame>
     ![Weather 6](/images/weather-6.png)
   </Frame>
   \
   The business event properties are now available in the drop-down lists. The event property **City** can now have a dynamic value each time the business event is triggered, with the value for **City** automatically substituted in the segmentation query.
...
4. Personalize the message by including information such as the user's name. This makes the message more contextual and helps users feel more connected. To do this:
   ...
   2. Select the information from the business event or the user properties.
      <Frame>
