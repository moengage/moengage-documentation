...
4. The model should be as follows:  
   ...
   For size, we will analyze the client’s previous purchase. During the previous purchase, we need to capture the client’s size in a user property via SDK or S2S, depending on the client’s tech availability. The S2S method can be achieved using [connectors](/user-guide/campaigns-and-channels/connectors/create/create-a-connector-campaign). The connector campaign can be essentially an event-triggered campaign that would pass on the size from the event property to the user property via the [User API](/api/user/track-user).  
   In either method, a user property, **Size**, would be created, which would be used to filter the recommendations.
...
3. While defining the message, select the product set generated through the recommendation [you defined](/use-cases/recommendations/how-to-recommend-apparels-by-size-and-availability#create-a-recommendation). Enter “@” and search for the name of the recommendation that you built, in the Push Personalization pop-up.  
