...
3. For the Header, the following are the popular options for fetching the image.
   ...
   - For each user profile, the user property with the last viewed product can be picked.
4. Personalize the message body with customer attributes or event properties such as name, location, and so on (if the template has variables).
