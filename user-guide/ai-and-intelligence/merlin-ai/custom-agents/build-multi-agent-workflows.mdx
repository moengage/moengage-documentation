...
You can add any agent that is both **public** and **Live**, including agents your teammates built. The **Sub Agents** picker lists only these agents, and you can add each one to a roster once.
...
4. To check an agent before you add it, open its **Agent Info** view. Agent Info opens in a new browser tab and shows who created the agent, how many times it has run, and when it last ran.
...
A public agent must never expose a private one, so MoEngage blocks both of these actions:

- Adding a private agent to the roster of a public coordinator.
- Making a coordinator public while a private agent remains on its roster.
...
<AccordionGroup>
  ...
  <Accordion title="Can I add a teammate's private agent to my agent's roster?">
    No. A roster accepts only public, Live agents, though that includes any public agent your teammates published. If your workflow needs what a private agent does, ask its creator to make the agent public.
  </Accordion>
