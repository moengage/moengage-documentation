...
- MoEngage restricts schedule configuration and edits to the agent's **creator**, because every scheduled run executes using the creator's permissions rather than the permissions of whoever views the dashboard when the scheduled run fires.
