...
- **Sub-Agent Activity:** When the agent delegates work to sub-agents, MoEngage displays their activity inline in the same transcript, indented under the coordinating agent's steps, so you follow each hand-off as it happens. Refer to [Build Multi-Agent Workflows](/user-guide/ai-and-intelligence/merlin-ai/custom-agents/build-multi-agent-workflows).
...
| **Run status** | The Run status column displays the current state of the execution (see Run Status Values below). The status values include the following: <br/><br/> • **Idle:** The agent's turn has ended and MoEngage is waiting for your next message. Sessions remain "Idle" because they are always resumable; they are never marked "done." <br/> • **Running:** The agent is actively executing instructions and calling tools. <br/> • **Rescheduling:** The agent hit a temporary limit and is waiting to resume. <br/> • **Terminated:** The run ended before completion and cannot be resumed. |
