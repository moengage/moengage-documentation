...
| **Run status** | The Run status column displays the current state of the execution (see Run Status Values below). The status values include the following: <br/><br/> • **Idle:** The run has completed successfully. Sessions remain "Idle" because they are always resumable; they are never marked "done." <br/> • **Running:** The agent is actively executing instructions and calling tools. <br/> • **Rescheduling:** MoEngage is preparing the next scheduled run. <br/> • **Terminated:** The run ended before completion. |
