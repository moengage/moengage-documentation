...
- If you **log out** or **switch workspaces**, you are prompted to **re-authenticate**. If the connection is interrupted, re-initiate it from your MCP client.
...
- **Independent from the dashboard**: logging in via MCP does not affect your dashboard session, and vice versa.
...
Create, edit, and test-send are supported for **Push and Email**. Search and analytics work for **Push, Email, SMS, and Webhook**; SMS and Webhook are read-only for campaign creation. **WhatsApp is not supported** — see [Other limitations](#other-limitations).
...
| Tool | Description |
| --- | --- |
| `search_campaigns` | Search campaigns by name, status, ID, channel, delivery type, or date range. Include `DRAFT` in the status filter to see drafts. WhatsApp is not a supported channel filter. |
...
<Info>
  Campaign analytics tools enforce a maximum date range of **30 days** per query. Flow analytics tools allow up to **90 days**. The beta behavior, funnel, retention, and session-source analyses use their own, longer windows.
</Info>
...
- **WhatsApp is not supported.** WhatsApp campaigns cannot be created, searched, or analyzed through the MCP. Use the MoEngage dashboard for WhatsApp campaigns.
- **SMS and Webhook** are read-only for creation: searchable and analyzable, but not creatable.
