...
keywords: "MoEngage MCP, MCP server, custom connector, Claude connector, ChatGPT connector, AI assistant, remote MCP, organization connector, segmentation tools, custom segments, segment count, audience estimation, recent query, segment membership, deep validation, PII attributes, behavior analysis, funnel analysis, retention analysis, user analysis, session source analysis"
---
...
<Info>
  Attributes marked as PII are not exposed through the MCP. This includes **Email (Standard)** and **Mobile Number (Standard)**. PII-marked attributes are not available in `get_user_events`, `get_recent_query`, `get_recent_query_users`, or `get_value_suggestions`.
</Info>
...
<Info>
  Attributes marked as PII are not exposed through the MCP. Across all five analyze tools, you cannot use a PII-marked attribute to group by user property or to split by. This includes **Email (Standard)** and **Mobile Number (Standard)**.
</Info>
...
| PII attributes are not exposed | Attributes marked as PII, including **Email (Standard)** and **Mobile Number (Standard)**, are not exposed through the MCP. They are not available in `get_user_events`, `get_recent_query`, `get_recent_query_users`, or `get_value_suggestions`. |
...
- **PII attributes are not available to the analyze tools.** Attributes marked as PII, including **Email (Standard)** and **Mobile Number (Standard)**, are not exposed through the MCP. They cannot be used to group by user property or to split by in `run_behavior_analysis`, `run_funnel_analysis`, `run_retention_analysis`, `run_session_source_analysis`, or `run_user_analysis`.
