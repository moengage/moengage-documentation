...
For per-tool parameters, examples, and caveats, see [MoEngage MCP Server: Segmentation Tools](/user-guide/ai-and-intelligence/merlin-ai/moengage-mcp-segmentation-tools).
...
| `describe_segment_filters` | Turn a segment filter set into a plain-English sentence — confirm intent before creating anything. |
| `deep_validate_segment_filters` | Semantically validate a filter set (attribute existence, data types, operators) before creating a segment or query. |
| `create_recent_query` / `get_recent_query` / `get_recent_query_filters` | Count an audience **without creating a segment**: run an ad-hoc audience query, poll its result, and read back its filters. |
| `is_user_in_segment` | Check in real time whether a specific user is in up to 10 segments. |
