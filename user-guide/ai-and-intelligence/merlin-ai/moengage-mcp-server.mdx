...
<Info>
  The MoEngage AI Connector (MCP server) is available in **DC01, DC02, DC03, and DC04**. Individual tools roll out by data center — see [Tool availability by data center](#tool-availability-by-data-center).
</Info>
...
Tool availability varies by data center and role — see [Tool availability by data center](#tool-availability-by-data-center). If a tool you expect is missing, [refresh the tools list](#refresh-the-tools-list).
...
<Info>
  These tools are newer and currently available on **DC03**. Names and response shapes may still change.
</Info>
...
### Tool availability by data center

The connector is live in DC01–DC04, but individual tools roll out per data center:

- **Campaign build, content, search, and campaign analytics tools** are available across DC01–DC04.
- **Dashboards, flows, and segment tools** are rolling out by data center, starting with DC01 and DC03.
- **Product behavior analytics tools** (`run_behavior_analysis`, `run_funnel_analysis`, `run_retention_analysis`, `discover_analyze_schema`) are currently **DC03 only**.

If you don't see a tool for your data center yet, it is likely still rolling out.
...
- **Segment filters**: segment creation does not yet support **"any of"** (match-any) filter groups. A segment built with an unsupported filter can return zero users — verify the count with `start_segment_count` before you rely on it.
...
<AccordionGroup>
  ...
  <Accordion title="A tool you expected isn't available">
    ...
    1. **Tool not enabled** — enable it in your client's connector settings. See [Enable the tools you need](#enable-the-tools-you-need).
    2. **Not yet on your data center** — some tools roll out per data center. See [Tool availability by data center](#tool-availability-by-data-center).

    Also confirm your MoEngage role has the matching permission (for example, campaign create/manage for build tools).
  </Accordion>
  ...
  <Accordion title="A segment shows zero users">
    Check whether the segment uses an **"any of"** (match-any) filter group, which isn't supported yet, or an otherwise unsupported filter. Rebuild with supported filters and confirm the count with `start_segment_count`.
  </Accordion>
</AccordionGroup>
