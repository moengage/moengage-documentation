...
| `get_value_suggestions` | Gets up to 5,000 distinct stored values (`string`, `double`, or array/object variants) for a user or event property, useful for checking real values before writing an equality filter. For `array_object` attributes, `node_attribute_name` / `node_data_type` resolve values for a child field. |
...
| `find_user_attributes` | Discover user properties matching a natural-language term (for example, "city"). Takes `query` and an optional `limit`; returns the internal `name` and `data_type`, which determines valid filter operators. |
...
| PII and contact attributes are restricted | Attributes marked as PII are not exposed through the MCP and are not available in `get_user_events`, `get_recent_query`, `get_recent_query_users`, or `get_value_suggestions`. **Email (Standard)** and **Mobile Number (Standard)** are masked in these tools even when they are not marked as PII. |
