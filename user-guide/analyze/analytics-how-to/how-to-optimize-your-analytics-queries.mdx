...
### Monthly data scan allowance

Your monthly scan allowance depends on your billing model and analytics package. MTU-based and profile-based billing are mutually exclusive — whichever applies to your workspace, the other is zero.

| Package | MTU billing | Profile billing |
| --- | --- | --- |
| Analytics | MAU (in millions) × 10 TB/month | Profile count (in millions) × 1 TB/month |
| Product Analytics (PA) | MAU (in millions) × 30 TB/month | Profile count (in millions) × 3 TB/month |

Every workspace, including test accounts, has a minimum allowance of 5 TB/month, regardless of the formula above.
...
As your monthly scan usage rises, MoEngage notifies you at the following thresholds:

| Usage | What happens |
| --- | --- |
| 75% | Level 1 warning — email notification and a yellow indicator on the dashboard. |
| 90% | Level 2 warning — your CSM and Engagement Manager are also notified. |
| 95% | Level 3 warning — critical alert with a red dashboard indicator and a prompt to expand your quota. |
| 100% | All analytics queries are blocked for the rest of the billing cycle and return an HTTP 429 response. |

When queries are blocked, Custom Dashboard charts fall back to the last successfully computed result for up to 45 days instead of returning an error.
...
Date range is the single most impactful parameter in any query. The wider the range, the more data MoEngage has to scan. The maximum date range you can select for any analysis is 365 days.
...
The granularity you choose narrows this range further:

- Hourly granularity is available for date ranges of up to 31 days in **Behavior**, **Funnels**, and **Session & Source** analyses.
- **Retention** has its own per-granularity durations. For more information, refer to [Retention Analysis](/user-guide/analyze/analytics/retention/retention-analysis#granularity).
...
- [RFM Segments](/user-guide/segment/create-segments/rfm-segments)
