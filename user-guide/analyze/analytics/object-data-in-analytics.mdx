...
This approach allows you to query complex, related data points in their natural structure (like a 'trip' object containing 'source' and 'destination' details) without needing to flatten all possible variations into separate, top-level event properties.
...
Consider a _SEARCH_BUS_ event, where the event property **trip** is a single object that contains details like its _source_, _destination_, and _journey_date_.
...
This query uses the **With** operator to ensure all conditions are met within the same _trip_ object. It will find _SEARCH_BUS_ events that have a trip where the `journey_date` exists AND the `destination.city` is _Chennai_.
...
- Learn more about defining user groups in [Create Segments](/user-guide/segment/create-segments/rule-based-filter-segments).
