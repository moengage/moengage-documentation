...
3. In the **Events & Filters** section, click the dropdown and then select **App/Site Opened**.  
   ...
   4. Filter the users to whom this analysis may apply. The Behavior analysis is performed on all users having access to the workspace by default. You can also segment users using the **Filter by users** option.  
      For more information, refer [here](/user-guide/segment/create-segments/rule-based-filter-segments).  
...
6. On the **Create alert** pop-up, enter the following details:
   ...
   - **Alert Criteria**:
     ...
     - Select Group by condition (if applied in analysis)
       - **Select Split by** - Select the event property associated with the event and its values. For example, for the event *App/Site Opened*, use the **Split by** filter the *Platform* where Platform is *Android.*
       - **Select Group by user property** - Select the user property associated with the event and its values. For example, for the event *App/Site Opened*, use the **Group by user property** filter on the *last known country* where country is*USA.*
