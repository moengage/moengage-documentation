...
- To analyze and modify the audience size, user properties, or event properties for campaign performance, create a campaign version with the desired changes instead of building a new campaign from scratch.
...
- For Push, Email, or SMS event-triggered campaigns using **With Delay**, the only channels where Campaign Versioning applies to this delivery type, publishing an edit retires the current version. Users who already triggered the event stay tied to that retired version. If their delay condition matures after the retirement, they don't receive the campaign at all. See [What Happens to Users Queued on a Delay Condition When an Event-Triggered Campaign Is Edited?](/user-guide/campaigns-and-channels/campaign-management-and-reports/faqs-campaign-management/what-happens-to-users-queued-on-a-delay-condition-when-an-event-triggered-campaign-is-edited) for details.
</Info>
