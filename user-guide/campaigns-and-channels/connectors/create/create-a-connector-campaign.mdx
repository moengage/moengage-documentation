...
<Info>
  * This setting applies to connector (webhook) campaigns. <!-- The exact field name, UI location, and supported delivery types are subject to confirmation. -->
  * Dedup Key prevents the same request from being sent again when a batch is re-run. Remove Duplicate Users removes duplicate entries from your target audience before sending. Depending on your use case, you may need one or both.
</Info>
