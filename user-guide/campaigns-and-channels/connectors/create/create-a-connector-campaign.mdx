...
Connector campaigns may be re-run in batches as part of standard delivery processing. Set a dedup key to ensure that each qualifying user receives the campaign only once, even if the batch is re-run.

MoEngage recommends always setting a dedup key to prevent duplicate requests, which can increase request volume and load on connected downstream systems.

<Info>
  * This setting applies to connector (webhook) campaigns. <!-- NEEDS CONFIRMATION: exact field name/label, on-screen UI location, and supported delivery types. -->
  * A dedup key prevents the same request from being sent again when a batch is re-run. **Remove Duplicate Users** removes duplicate entries from your target audience before sending. Depending on your use case, you may need one or both.
</Info>
