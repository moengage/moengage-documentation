...
<Warning>
The Drag and Drop editor has a few known rendering, personalization, and editor-specific limitations. Review the [Known Limitations](#known-limitations) section below before building complex templates.
</Warning>
...
Personalized links can be added using the More \>\> Links. This will open a pop-up window where the user can access and select from a list of personalized attributes.
...
<Warning>
Upload images through the file manager (maximum 5 MB each) or reference them with a hosted image URL. Do not paste base64-encoded image data directly into a link or URL field (for example, an `<a href>` value). A large base64 image can exceed the message-size limit during sending and cause the campaign to fail silently, with a Sent count of 0.
</Warning>
...
Display conditions let you show or hide an entire row based on a custom conditional statement, so a single template can render different content for different recipients. For example, showing a coupon-code row only to frequent buyers, or a category-specific promotion only to users who browsed that category.

<Info>
  Display conditions are evaluated using Jinja conditional logic (for example, `{% if %}...{% endif %}` tags) that wraps the row's content. The condition itself is written by you and is not validated by the editor — make sure the Jinja syntax and the attribute or logic you reference are correct before publishing.
</Info>

#### Add a Display Condition to a Row

1. On the editor canvas, select the row you want to apply a condition to.
2. In the properties panel, go to **ROWS > ROW PROPERTIES**.
3. Expand **DYNAMIC CONTENT**. If no condition is applied, this section shows **No display condition applied to this row**.
4. Click **Add condition**. The **Add a new display condition** dialog box opens.
   <img alt="Display_condition" src="/images/display_condition_1.png"  />
5. Enter the following details:
     <img alt="Display_condition_2" src="/images/display_condition_2.png" width="50%"/>

   | Field | Description |
   | --- | --- |
   | **Name** | A name to identify the condition. |
   | **Description** | A brief description of what the condition checks or when it is applied. |
   | **Before** | The opening part of the Jinja conditional statement (for example, an `{% if %}` tag) that is inserted immediately before the row's content. |
   | **After** | The closing part of the Jinja conditional statement (for example, an `{% endif %}` tag) that is inserted immediately after the row's content, closing the block opened in **Before**. |

6. Click **Add**.

The condition is now applied and appears as a **CUSTOM** card under **DYNAMIC CONTENT**, showing its name, description, and the **Before**/**After** values.
      <img alt="Display_condition_3" src="/images/display_condition_3.png"  />
* To modify the condition, click **Edit**.
* To remove the condition from the row, click **Delete**.
...
- A row or background that shrinks only in the Gmail mobile app may be Gmail's own "auto-sizing" feature kicking in, not a template bug — Gmail post-processes the HTML it receives in ways the sender can't fully control. Reduce the risk with `!important` widths, keeping elements under ~600px, and a hidden min-width "bumper" div.
- Don't paste a full standalone `<html>`/`<head>`/`<body>` document into a custom HTML content block — only the inner markup fragment belongs there. For content that needs to look different on mobile vs. desktop, build two literal rows (one hidden on desktop, one hidden on mobile) rather than relying on a single "smart" responsive block.

### Email Clipping

- The editor's displayed file size isn't what Gmail actually evaluates against its clipping limit — a stray trailing style line from the editor's export can be the difference between clipping and not. Don't use the editor's file-size display as your clipping budget check.

### Links & Tracking

- Custom unsubscribe URLs and unsubscribe mode are stored per-sender, not per-account — reconfigure them under Settings → Email → Sender Settings for every new sender added, since a new sender starts blank rather than inheriting an existing one's configuration.
- Deleting or editing a hyperlinked word without also clearing the whitespace around it can leave an invisible "ghost link" embedded in the underlying HTML. When editing or removing a link, clear the surrounding whitespace too, and spot-check the code view for stray anchor tags afterward.

### Personalization

- Complex Jinja for auxiliary data doesn't belong directly inside an HTML attribute like a button's URL field — the button URL picker only exposes user attributes, and quotes inside the expression can conflict with the surrounding attribute syntax. Assign the aux-data expression to a Jinja variable elsewhere in the template, then reference that variable in the button URL field.
- Copy-pasting formatted text from another website can carry over invisible markup that the personalization engine chokes on, causing an "undefined variable" error at send time even though nothing looks wrong in the visual editor. Always paste as plain text when bringing in copy from another source, then reapply formatting and links manually.

### Images

- Only use images uploaded through the editor's own file manager — manually pasted external image URLs can expire without warning, causing images to silently disappear from a live template weeks after setup. Always upload images via the built-in file manager rather than hot-linking externally hosted URLs.

### Editor Behavior Quirks

- The legacy Text block stores manual Enter/blank-line spacing as non-breaking spaces; a platform-wide anti-clipping fix now collapses that spacing away, causing intentional gaps to disappear. The Text block is deprecated — use the Paragraph/List blocks and create spacing structurally via block/row padding or a transparent Divider block.
- A reusable content block should be a bare HTML fragment — wrapping it in its own `<html>`/`<body>` document breaks mobile stacking, centering, and hide-on-device rules once nested inside the parent template.
- Copy-pasting a styled icon or glyph from an external source into a text block can inject hundreds of redundant nested tags and corrupt the template. Paste plain text only and use the built-in special-character picker for symbols and arrows instead of copying styled characters from other apps.
- Repeatedly toggling formatting (like bold) on the same text can generate hundreds of nested tags that break parsing when the campaign is published. If publishing fails after heavy inline editing, delete the affected text element/row and re-add it rather than continuing to toggle formatting on the same node.

### Third-Party / ESP Limitations

- Outlook can distort a correctly-built email specifically when the recipient forwards it, in ways that can't reliably be fixed from the template side. Treat this as a known, low-control Outlook forwarding quirk rather than a template defect worth chasing indefinitely.
