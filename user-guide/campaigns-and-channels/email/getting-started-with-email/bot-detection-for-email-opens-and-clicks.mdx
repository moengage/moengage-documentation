...
<Note>
  Note: While our bot detection is robust, it is impossible to identify 100% of all bot activities. Hence, some sophisticated bots may still be counted as real users.
</Note>

# Prerequisites

* You must have access to the **Settings** menu in your MoEngage dashboard.
* Bot detection is configured exclusively at the **Email** channel level.

# Step 1: Enable Bot Detection

To start filtering bot activity, you must first enable the feature at the channel level:
...
Once enabled, the system reveals two sub-tabs: **Bot opens** and **Bot clicks**. You can configure specific detection rules within each.

# Step 2: Configure Bot Opens

Navigate to the **Bot opens** tab to define how MoEngage identifies and handles bot-driven email opens.

## Detection Rules

* **Detect opens from Apple Mail Privacy Protection**: Apple Mail Privacy Protection (MPP) pre-loads images in incoming emails through Apple's proxy servers as soon as a message arrives. Check this box to automatically identify these proxy-generated opens.
* **Bot user-agent blocklist**: Expand this section to manage the list of user-agent signatures used to detect bots.
  ...
  * **Add bot user-agent**: Click the **+ Add bot user-agent** button to create custom rules. This creates a new input row where you must define the following fields:
    ...
    * **Bot user-agent name**: Type the exact or partial string you want to detect. Note: You must enter a string here before saving, otherwise the system will display a "Please enter a user agent string before saving" error.
    ...
    * **Actions**: While adding a new rule, click the checkmark icon to save and apply the new entry, or click the cross (X) icon to cancel and discard the row. (Once a rule is saved, these icons change to a pencil for editing and a trash can for deleting).
