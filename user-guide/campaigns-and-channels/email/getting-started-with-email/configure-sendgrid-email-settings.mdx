...
9. From the **User Property that stores the user’s email address** drop-down, select **Email (Standard)**.
10. In the "**From" email address(s)** field, add the from address. The domain (the part after @) must be a [validated one on SendGrid](#add-domains-in-sendgrid).  
