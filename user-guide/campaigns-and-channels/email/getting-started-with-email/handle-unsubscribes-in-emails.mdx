...
The subscription types below (Global Unsubscribe, List-Unsubscribe, Subscription Categories) control how a user *opts out*. This is different from **Suppression**, which is the global block-list MoEngage enforces once a user is Unsubscribed, Bounced, or has Complained.

| | Unsubscribe | Suppression |
|---|---|---|
| **Trigger** | User clicks an unsubscribe/list-unsubscribe link, or updates preferences | Bounce, Unsubscribe, or Spam Complaint event |
| **Scope** | Applies only to the specific type triggered (Global, List, or Category) | Global — blocks all non-transactional sends once any one signal is true |
| **Reversibility** | Reversible; user can resubscribe or update preferences anytime | Reversible via CSV upload or API; does not reset automatically |
| **Who controls it** | End user (via email link or preference center) | Brand/admin (via Suppression Updates, API, or ESP sync) |

An unsubscribe is one of the three inputs that can suppress a user; suppression is the resulting block enforced against sends. For details on managing the suppression list directly, refer to [Suppression Updates](/user-guide/campaigns-and-channels/email/getting-started-with-email/suppression-updates).
...
Global unsubscribe allows users to unsubscribe from all non-transactional emails of your brand or business. When a user clicks this link, they can either be unsubscribed directly or be redirected to a landing page to get double confirmation before unsubscribing.
 ...
 Global Unsubscribe only stops non-transactional emails. It does not affect transactional emails or Inform alerts, which continue to be delivered regardless of a user's unsubscribe status.
## List-Unsubscribe
...
<Info>
 Resubscribing a user (via the Resubscribe API or a CSV upload) only reverses the block on MoEngage's side — it does not by itself confirm renewed consent. Before sending marketing emails to a resubscribed user, ensure you have valid consent as required under applicable regulations. See the compliance note in [Suppression Updates](/user-guide/campaigns-and-channels/email/getting-started-with-email/suppression-updates) for more detail.
</Info>
