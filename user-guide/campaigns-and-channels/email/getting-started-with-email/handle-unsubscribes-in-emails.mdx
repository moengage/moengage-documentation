...
| **Scope** | Sets an attribute on the user profile | Blocks promotional/marketing campaigns only. Transactional campaigns (sent from the point channel or via Inform) still reach the user. Bounce is the one exception — it blocks all campaign types, including transactional |
