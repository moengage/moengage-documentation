...
Unsubscribe and suppression are related but distinct concepts. Confusing the two can lead to incorrect suppression management and compliance risk.

**Unsubscribe** is a user action. When a user unsubscribes, MoEngage sets the standard unsubscription attribute on their profile to `True`. This is one of several reasons a user can end up suppressed.
...
- Any of the BUC attributes (Bounce, Unsubscribe, or Complaint) is set to `True`
- Frequency Capping (FC) limits are reached
- Do Not Disturb (DND) settings apply
- The user has opted out of a Subscription Category
- Double Opt-In (DOI) is unconfirmed

| Aspect | Unsubscribe | Suppression |
|---|---|---|
| **Trigger** | User action — the user clicks unsubscribe, setting the Unsubscribe attribute (one of the three BUC attributes) to `True` on their profile | Platform action — occurs when any BUC attribute, FC, DND, Subscription Category, or DOI condition is met |
| **Scope** | Sets an attribute on the user profile | Blocks promotional/marketing campaigns only. Campaigns tagged as transactional, or sent via Inform, still reach the user. Bounce is the one exception — it blocks all campaign types, including transactional |
| **Reversibility** | Reversible — the attribute can be reset via resubscribe flows | Reversible depending on the trigger, but removing the suppression condition does not restore explicit consent (see compliance note below) |
| **Who controls it** | The end user, directly | The platform, automatically, based on profile attributes |

For a full breakdown of how brands can manage suppression (data ecosystem updates vs. email-based updates) and why MoEngage doesn't maintain a native suppression list, see [Unsubscribe vs. Suppression](/user-guide/campaigns-and-channels/email/getting-started-with-email/suppression-updates#unsubscribe-vs-suppression) on the Suppression Updates page.

# Types of Subscription Management in MoEngage Emails

## Global Unsubscribe

Global unsubscribe allows users to unsubscribe from all non-transactional emails of your brand or business. When a user clicks this link, they can either be unsubscribed directly or be redirected to a landing page to get double confirmation before unsubscribing. This doesn't affect transactional emails or those sent via Inform — those continue to reach the user regardless of their global unsubscribe status. Bounce is the only trigger that blocks all campaign types, including transactional.
...
<Tabs>
  <Tab title="MoEngage-Managed">
    ...
    <Warning>
      Paste the href unencoded. If a link contains a MoEngage personalization token, do not URL-encode it. URL-encoding replaces the `{{` delimiter with `%7B%7B` and `$` with `%24`, which decodes to a `{{$` sequence that Jinja cannot parse. When you save or send the campaign, this can produce an error similar to `Error in parsing jinja template format. Error unexpected char u'$' at ...`. Insert the token through the merge tag or token option provided in the editor. For more information, see [Jinja Templating Language](/user-guide/campaigns-and-channels/getting-started/message-personalization/jinja-templating-language#escaping).
    </Warning>
...
<Warning>
  Resubscribing a user in MoEngage or with your email sender only removes the suppression condition, it doesn't restore consent. See the [compliance note](/user-guide/campaigns-and-channels/email/getting-started-with-email/suppression-updates#unsubscribe-vs-suppression) on the Suppression Updates page before resuming sends to a resubscribed user.
</Warning>
