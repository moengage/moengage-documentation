...
<Note>
  Test and preview campaigns do not modify user-profile data. Sending a test does not update the unsubscribe status (`moe_unsubscribe`) or other reachability attributes on the profile — only live campaigns update these fields. To validate an unsubscribe flow, run a live campaign targeting a small internal segment.
</Note>
...
Unsubscribe is a user action: the user clicks unsubscribe, and MoEngage sets the standard unsubscription attribute on their profile to `True`. Suppression is the platform-side outcome that stops non-transactional campaigns from reaching a user, and unsubscribe is only one of the conditions that triggers it.

For the full comparison — what triggers suppression, how brands manage it, and why MoEngage doesn't maintain a native suppression list — see [Unsubscribe vs. Suppression](/user-guide/campaigns-and-channels/email/getting-started-with-email/suppression-updates#unsubscribe-vs-suppression) on the Suppression Updates page.
