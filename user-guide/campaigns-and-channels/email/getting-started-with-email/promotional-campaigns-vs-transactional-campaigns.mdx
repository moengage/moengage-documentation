...
<Info>
The following checks will not be performed for transactional emails: Delivery Controls (FC and DND), Unsubscribe, and Complaints. Subscription Category opt-outs and an unconfirmed Double Opt-In (DOI) status are also ignored for transactional emails — only a hard bounce or an invalid email format blocks a transactional send. Control Groups will also be ignored for transactional emails.
</Info>
