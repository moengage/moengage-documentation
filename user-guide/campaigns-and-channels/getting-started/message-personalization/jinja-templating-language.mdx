...
To insert variables, enclose them in double braces. For example, to insert the user property labeled _FirstName_:
...
**Auxiliary Data** allows you to pass additional, contextual data at the time of sending a campaign, often through an API trigger. This is useful for including transactional details like order IDs, flight numbers, or any other information that is not stored as a user or event property.
...
<Warning>
  Insert MoEngage personalization tokens unencoded in your email HTML. Do not URL-encode a token or an href that contains one. URL-encoding replaces the `{{` delimiter with `%7B%7B` and `$` with `%24`, which decodes to a `{{$` sequence that Jinja cannot parse. When you save or send the campaign, this can produce an error similar to:

  `Error in parsing jinja template format. Error unexpected char u'$' at ...`

  If a link contains a personalization token, insert it through the token or merge tag provided in the editor rather than pasting a manually URL-encoded value into the href.
</Warning>
...
<Note>
  If you're writing Jinja in the rich (Froala) editor for a stylized email or push template, avoid raw `>` / `<` comparison operators — the rich editor HTML-encodes them, which breaks the template with a `TemplateSyntaxError`. Use `!=` instead, or write the Jinja in the custom HTML/source view. See [Rich editor encodes comparison operators](/user-guide/campaigns-and-channels/getting-started/message-personalization/common-personalization-errors-and-faqs#why-does-my-jinja-condition-throw-a-templatesyntaxerror-in-the-rich-editor) for details.
</Note>
...
<Note>
  When looping to generate HTML table rows, the `{% for %}` loop must wrap a complete `<tr>...</tr>` row inside `<tbody>` — not reside directly inside `<table>` or `<tbody>` around individual cells. Placing loop tags in an invalid position causes the email HTML editor to relocate or remove them when the template is saved, breaking the table's output. See [Dynamic tables with Jinja](/user-guide/campaigns-and-channels/getting-started/message-personalization/common-personalization-errors-and-faqs#why-does-my-dynamically-generated-table-render-incorrectly-after-saving) for the correct pattern.
</Note>
...
<Note>
  - Within dateTimeFormatter filter, multiple methods can be used. For example, to display a user property in a particular format and user Timezone, the following code can be used. `{{UserAttribute['First Seen']|dateTimeFormatter(toFormat='%Y-%m-%d %H:%M', timeZone='Asia/Kolkata',tzOffset=UserAttribute['User Time Zone Offset (Mins)'])}}`
