---
title: "Overview"
description: "Personalize MoEngage campaigns with user properties, event data, and dynamic content to improve engagement, open rates, and conversion metrics."
---
...
<Tip>
  - You must have a basic familiarity with MoEngage campaign creation, user properties, and event tracking.
...
2. On the **Data Personalization** tab, in the **Select Attribute** list, click the relevant user property value.
3. For the selected user property, define a fallback value to be used in case the personalization fails for some reason. The following options are supported:
...
For example, personalized emails have higher open rates and conversions because they leverage the email context and are relevant. You can personalize your email campaigns based on user properties, such as name, location, or other attributes (for both general and event-triggered emails).
...
When you want to pass a unique user property to the app and make a decision based on that, the defined key-value pair can be used to pre-fill feedback forms, user surveys, and so on.
...
<Info>
  If the personalized URL fails to find/resolve the user property, the email will not be sent to the user. There is no fallback mechanism for personalized URLs. If you have any issues creating personalized email campaigns, contact your MoEngage Customer Success Manager (CSM) or the Support team.
</Info>
...
<Info>
  For all channels that support personalization, business event property-based personalization is supported only for business event-triggered campaigns.
</Info>
...
2. The attributes of the Business Event mapped to the campaign are shown under the Business Event Property heading, as shown below:
...
Personalization with business events can be a big difference in effort and automation, as you can use one template and send multiple campaigns with it. For example, you can communicate information about three new web series that are being launched with the same campaign. You can use the event properties to personalize the message for each series by using the series name, image, cast, and air date to send the communication to your dynamic segment (that matches the user interest (say, preferred genre) to that of the genre of the series).
...
| ------------------------------------------ | ---- | --- | ----- | -------- | ---------------- | ----------------- | ----- | ---------- | ----------- |
| **User Properties**                        | Yes  | Yes | Yes   | Yes      | Yes              | Yes               | Yes   | Yes        | Yes         |
| **Event Properties**                       | Yes  | Yes | Yes   | Yes      | Yes              | Yes               | Yes   | Yes        | No          |
| **Business Event Properties**              | Yes  | Yes | Yes   | No       | No               | No                | No    | NA         | No          |
