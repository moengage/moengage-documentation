...
<Info>
  You can personalize Content APIs using user properties. Personalization is also possible for event properties when the event is linked to the same campaign.
</Info>
...
3. On the **Add Content API** page, enter the following details:
   ...
   | **Key Value Parameters** | Click the plus sign to add key value for the parameters used in the Content API. Use “@” in the value field to choose from the list of pre-defined user properties. The parameters cannot be personalized using Event properties in the value field. |
4. Click **Save**.
...
4. Click **Done**. The Content API gets added to the message.\
   For example, the Content API dynamically fetches the recommended products for a given user ID in the following screenshot. You can use this feature to engage users with product recommendations and nudge them toward making a purchase. The value parameter can be a dynamic parameter (type @ to add a user property to be passed to the Content API) or a static parameter that can be keyed in the value section.
