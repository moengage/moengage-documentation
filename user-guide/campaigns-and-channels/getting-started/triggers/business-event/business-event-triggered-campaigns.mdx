...
<Note>
  A business event can start a maximum of 10 campaign triggers in 5 minutes and 50 campaign triggers in an hour, measured over a sliding window. If a trigger goes beyond a limit, MoEngage creates the campaign in a stopped state instead of sending it, and the trigger still returns a `200` response. For all business event limits, refer to [Trigger Rate Limits](/user-guide/campaigns-and-channels/getting-started/triggers/business-event/overview-business-events#trigger-rate-limits).
</Note>
