...
<Note>
  A business event can start a maximum of 10 campaign triggers in 5 minutes and 50 campaign triggers in an hour, measured over a sliding window. A trigger beyond a limit still returns a `200` response, but the campaign is created in a stopped state instead of being sent. For all business event limits, refer to [Trigger Rate Limits](/user-guide/campaigns-and-channels/getting-started/triggers/business-event/overview-business-events#trigger-rate-limits).
</Note>
...
<Tabs>
  <Tab title="Push">
    ...
    6. In the **Target audience** section, match user or event properties with the business event's attributes dynamically. This allows you to use a single campaign for many business events.
       ...
       - **User Property**: Match user properties or user properties with event properties. If a user property stores the product ID, you can match it with the business event property containing the product ID for the back-in-stock product.
       ...
       - **User Behavior**: Target audiences who have viewed out-of-stock products in the past 25 days. Match the attributes of products viewed by them with the attributes of products now back in stock using Business Event Properties. The example below shows how to match product names, brands, and item codes of products viewed by users to those now back in stock.
  ...
  <Tab title="SMS, MMS, and RCS">
    ...
    4. On the campaign creation page, add the campaign name, tags, and the user property that stores the mobile number.
    ...
    6. In the **Select audience** section, dynamically match the user's flight information with the delayed flight's information. This allows you to use a single campaign for many business events.
       ...
       You can match user properties, user behavior, and user affinity information with event properties in segmentation. If the user has purchased tickets for a flight, and the flight number and travel information are stored as user properties, you can match this information to the business event properties of the delayed flight, as shown below.
    ...
    8. Create the content for the SMS, MMS, and RCS campaign and add the delivery schedule. You can personalize the message with the business event's attributes. For more information, see [Personalization with Business Event Properties](/user-guide/campaigns-and-channels/sms-mms-and-rcs/sms/personalize-sms-campaign).
    9. In step 3 of campaign creation:
       1. Select the **Start date** and **End date** for the campaign, along with the timezone for delivery.
       2. Add conversion goals as needed.
       3. Add delivery controls to ignore frequency capping and override default throttling limits.
  ...
  <Tab title="Email Campaigns">
    ...
    6. In the **Select audience** section, dynamically match the user's interest genre with the business event property. This allows you to use a single campaign for many business events.
       - **User Property**: Match user properties or user properties with event properties. If the user's preferred genre or the genre of content watched by the user is stored in the user property named **genre**, you can match this to the business event's genre, as shown below.

         <Frame>
           ![User Property(1)](/images/UserProperty\(1\).png)
         </Frame>
       - **User Behavior**: If **genre** is an event property of a **Viewed Content** event (triggered when a user watches content on the OTT platform), and **genre** contains a list of all genres viewed by the user in that session, you can map these attributes as shown below.
    ...
    7. Add the control group (optional) for the campaign, and then move to the next step.
    8. Create the content for the email campaign and add the delivery schedule. You can personalize the subject, preview text, and email content with the business event's attributes. For more information, see [Personalization with Business Event Properties](/user-guide/campaigns-and-channels/email/create/personalize-email-content).
    9. In step 3 of campaign creation:
       1. Select the **Start date** and **End date** for the campaign, along with the timezone for delivery.
       2. Add conversion goals as needed.
       3. Add delivery controls to ignore frequency capping and override default throttling limits.
  ...
  <Tab title="Flows">
    ...
    6. In the **Audience** section, match the user or event properties dynamically with the business event's attributes. This enables you to use a single flow for many business events.

       For example, consider a **Series launched** business event. If a new Thriller series launches, you can map one or more business event properties (such as episode number, episode name, episode rating, or cast) with event or user properties. This allows a single flow to run whenever a business event is triggered for any launched series.
       - **User Property**: Match user properties or user properties with business event properties. If the user's preferred genre or the genre of an episode watched by the user is stored in the user property named **Genre**, you can match this to the business event's genre, as shown below:

         <Frame>
           ![User Property Flows](/images/user-property-flows.png)
         </Frame>
       - **User Behavior**: If the **genre** is an event property of a **Watched episode** event (triggered when a user watches content on the OTT platform), and **Genre** contains a list of all genres viewed by the user in that session, you can map these attributes as shown in the following image.
    ...
    8. Build your flow by adding stages that align with your strategy. You can also personalize the message in the [Action Stages](/user-guide/flows-cross-channel-messaging/create/actions-in-flows/add-actions-to-flows) of the flow with the business event's attributes. For more information, refer to [Personalization with Business Event Properties](/user-guide/flows-cross-channel-messaging/create/actions-in-flows/personalization-in-flows).
...
<Info>
  ...
  - For segmentation to work, the data types of the event properties or user properties used in segmentation must match those of the business event's attributes. If the attributes do not match, segmentation will not run, and the campaigns will not be sent to users.
