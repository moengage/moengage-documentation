...
A trigger that exceeds a limit still returns a `200` response. Instead of sending the campaign it would have run, MoEngage creates that campaign on the dashboard in a stopped state, so check the campaign list when you expect a message that did not go out.

<Note>
  MoEngage is changing this response behavior. After the change, a trigger that exceeds a limit will no longer return `200`.
</Note>
