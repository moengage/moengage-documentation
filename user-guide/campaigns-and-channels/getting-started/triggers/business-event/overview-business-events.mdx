...
A trigger that exceeds a limit still returns a `200` response. The campaign it would have run is created on the dashboard in a stopped state instead of being sent, so check the campaign list when you expect a message that did not go out.
...
MoEngage delivers a business event-triggered campaign on the same timelines as a one-time campaign.
