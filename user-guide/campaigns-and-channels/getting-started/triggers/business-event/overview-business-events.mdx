...
MoEngage does not send the campaign. It creates the campaign on the dashboard in a stopped state instead, and the trigger still returns a `200` response, so a `200` does not confirm that the message went out. Check the campaign list whenever you expect a message that did not arrive.

<Note>
  MoEngage is changing this response behavior. A trigger that exceeds a limit will no longer return a `200` response.
</Note>
