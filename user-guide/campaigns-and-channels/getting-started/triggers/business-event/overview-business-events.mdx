...
A trigger that exceeds a limit still returns a `200` response. Instead of sending the campaign, MoEngage creates it on the dashboard in a stopped state. Check the campaign list when you expect a message that did not go out.
