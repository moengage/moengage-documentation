...
| --- | --- |
| Campaigns | 50 triggers per hour |
...
If you trigger `BackInStock` 25 times within an hour, the campaigns accumulate 50 triggers, which reaches the campaign limit for that window. The flow accumulates 25 triggers, so it passes its limit of 3 per hour after the third trigger. Triggers beyond a limit do not run. Because the window slides, capacity frees up as the earliest triggers fall outside it rather than at a fixed reset time.
