...
- Send updates when someone uploads new videos or posts in preferred channels.
...
Rate limits count the **campaign and flow triggers** that a business event starts, not the number of times you trigger the business event itself. If you attach a business event to several campaigns and flows, it produces one trigger for each of them, and each trigger counts towards its own limit.
...
Suppose you attach a `BackInStock` business event to two campaigns and one flow. Triggering that business event once produces:
...
A business event-triggered flow enters users in bulk and can process millions of users in a single run. Because of this scale, users may take up to 60 minutes to enter the flow after you trigger the business event. Account for this when you plan time-sensitive communication.
