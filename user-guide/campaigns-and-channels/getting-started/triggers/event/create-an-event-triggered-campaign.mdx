...
<Note>
  Internally-generated MoEngage events — events created by MoEngage's backend rather than sent by the SDK or event API (for example, `MOE_USER_REINSTALL`) — are not part of the real-time event stream, so they cannot be used as trigger events for event-triggered campaigns or Flows. These events still appear on the user profile and in segmentation, so you can still use them in segments. MoEngage events received through the SDK or event API (for example, `MOE_APP_OPENED`) can be used as trigger events.
</Note>
