...
<Info>
  If you edit and publish the campaign after users are already queued on a **With Delay** condition (whether the delay uses a fixed time value or an event attribute), [campaign versioning](/user-guide/campaigns-and-channels/campaign-management-and-reports/campaign-versioning) retires the old version. Users queued against the retired version are not sent the campaign when their delay condition matures — they receive neither the previous content nor the updated content. This does not apply to campaigns that trigger **Immediately**.
</Info>
