...
<Note>
  If the trigger event often coincides with a **user merge** — for example, login, registration, or confirmation events where an anonymous user becomes a known user right after the trigger — select **With Delay** (for example, around 10 minutes). This gives the merge time to complete before the message is evaluated, so the campaign is not missed for users who are merged immediately after the trigger event.
</Note>
