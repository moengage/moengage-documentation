...
For **On session start** and **On screen load**, you can show the message immediately or after a configured wait using the **After Delay** trigger time — for example, to avoid interrupting the splash screen.
...
![The Filter users by section of an In-App campaign, showing user property filter conditions.](/images/moengage_7e1d90.png)
...
- **Real-time segments**: Segments built on user properties, user events, event properties, and event duration from the last 30 days are evaluated in real time. Users start seeing the message as soon as the campaign is published, and the eligible-user list stays current. Use these for time-sensitive promotions and announcements. For more information, refer to [Real-Time Segment Evaluation](/user-guide/segment/advanced-concepts/real-time-segment-evaluation).
...
<Info>
  - **SDK version**: Android BOM 1.3.0 or later, and iOS 10.10.0 or later.
  - **Supported segments**: Available only for User Property–based segments and Real-Time Segment Evaluation (RTSE) segments. The option is hidden for segment types where real-time changes do not apply, such as static segments.
...
- **Ignore frequency capping**: Lets this campaign bypass the account-level [frequency-capping](#frequency-capping) limit.
...
For the full configuration, refer to [Frequency Capping](/user-guide/settings/channels/delivery-controls/frequency-capping). Support for In-App frequency capping requires Android BOM 1.4.0 or later and iOS 10.10.0 or later.
...
- The SDK is not integrated or initialized correctly for the In-App channel. Follow the [integration steps](#sdk-integration) for your platform.
...
Some In-App features require a minimum MoEngage SDK version. Users must be on an app build that includes that version for the feature to work on their device. Android versions are the [BOM](/developer-guide/android-sdk/sdk-integration/basic-integration/Install-Using-BOM) (Bill of Materials) version; features that predate BOM-based versioning are supported by every BOM version and are marked **All versions**. iOS versions are the package version. Hybrid platforms (React Native, Flutter, Cordova, Capacitor) may have their own minimums.

| Feature | Android BOM | iOS |
| --- | --- | --- |
| On session start / On screen load triggers (native only) | All versions | 6.06.0 or later |
| After Delay trigger time | All versions | 4.11.1 or later |
| On push click | 2.3.0 or later | 10.14.0 or later |
| Multiple triggers within an event (AND/OR conditions) | All versions | 9.16.1 or later |
| Nudges (`showNudge()`) | All versions | 9.16.1 or later |
| Re-evaluate campaign eligibility | 1.3.0 or later | 10.10.0 or later |
| Frequency capping | 1.4.0 or later | 10.10.0 or later |

## Delivery Flow Diagram

The diagram below shows how an In-App message travels from setup in the dashboard to display on a user's device. It expands the four delivery stages into five phases — configure, fetch, trigger, select, and display — showing trigger and selection as separate steps. The points where the flow stops are the same points to check when a message does not display; the reason for each stop is recorded in the campaign's delivery funnel.
...
flowchart TD
    ...
    subgraph DISPLAY["5 · Display and track"]
        direction TB
        E1("Fetch campaign assets and<br/>personalization data") --> E2{"Rendered<br/>successfully?"}
