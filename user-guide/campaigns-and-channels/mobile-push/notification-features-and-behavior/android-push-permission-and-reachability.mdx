...
keywords: ["Android push permission", "Android 13 notification permission", "POST_NOTIFICATIONS", "opt-in rate", "push reachability", "track push permission", "permission trends", "opt-in methods", "improve opt-in rates"]
---
...
To enable Android to push notification Opt-in/Opt-out tracking, upgrade to MoEngage Core SDK version 12.3.01 and above. MoEngage SDK tracks the device's opt-in status by default. The opt-in status of the user’s devices will be can be tracked by the “Reachability Push Android” user property.
...
5. In the **Behavior Options**, choose the Analysis type as _Unique users_, Group by user property as _Reachability Push Android_, and the Duration as _last 2 months_.
