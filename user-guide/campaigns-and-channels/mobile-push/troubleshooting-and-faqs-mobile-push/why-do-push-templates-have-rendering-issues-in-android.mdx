...
- Maintain the aspect ratio recommended for the template you're using. The ratio differs by template, and within a template it differs depending on whether the notification includes buttons — for example, 2.9:1 / 2.8:1 for basic notifications and 1.4:1 / 1.3:1 for the Image banner with text overlay template. The campaign builder shows the recommended ratio in the helper text below the image field, and the full list by template is in [Push Templates](/user-guide/campaigns-and-channels/mobile-push/create/push-templates).\
  Cropping is minimal when the image is close to the recommended aspect ratio. However, you might notice some cropping on a few devices despite images following this aspect ratio. This is because of the following reasons:
