...
3. To capture user input, map the input field from the **Lead Gen** template to a user property. In the second step of campaign creation, click the input widget on the template, and then configure the user property where this input will be saved.

![Screenshot showing how to map input widget to user properties.](/images/moengage_15304f.png)
...
By default, MoEngage captures the **First Name** and **Email Address** user properties. You can add more input fields by modifying the template content.
...
You can view the captured data in MoEngage user properties. MoEngage tracks the **MOE\_RESPONSE\_SUBMITTED** event for every submission from an **SMS Sign-Up** On-site Message.
