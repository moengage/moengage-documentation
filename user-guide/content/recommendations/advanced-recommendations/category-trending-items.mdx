...
Popular Items rank the items with the highest overall interaction counts across your catalog. It is not personalized, so every user receives the same crowd-level list.

MoEngage serves Popular Items as the default fallback for every Advanced Recommendations model. If a model has too little data, MoEngage falls back to Popular Items, which means new users and users with little interaction history still receive recommendations. You do not configure this fallback, and MoEngage serves it automatically.

For more information, refer to [Overview of Advanced Recommendations](/user-guide/content/recommendations/advanced-recommendations/overview-advanced-recommendations).
...
Trending Items results can differ for each user based on their recent activity, so MoEngage doesn't auto-populate them. After you create the recommendation, MoEngage takes about 2 hours to process it.

After the processing is complete, before using this recommendation in a campaign, test it against a real user, a random sample, or a specific item, and fine-tune its filters live. For more information, refer to [Test a Recommendation](/user-guide/content/recommendations/getting-started/test-a-recommendation).
