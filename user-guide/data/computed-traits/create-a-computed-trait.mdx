...
1. **Computed trait name:** Enter a unique name for your computation. This name will appear as a new user property across the MoEngage UI (for example, "Lifetime Value").
...
1. **Select the Type:** In the **Type** section, choose the desired type of computation. You can create traits based on:
   ...
   - **Aggregation:** Aggregate values of an event property (for example, Total order value).
   - **First/Last Value:** Identify the first or last value of an event property.
...
2. **Set up the Configuration:** In the **Configuration** section, based on the type of computation that you selected, do the following:
   ...
   - For an **Aggregation** computation, select the event and the time period. Then, choose the aggregation function. The following functions are available: **Sum**, Min, Max, Average, Median, Distinct count, and **True if event exists**. After selecting the function, select the event property you want to compute.
     ![Screenshot 2025-08-19 at 1.49.48 PM.png](/images/moengage_150954.png)
   - For a **First/Last Value** computation, select the event and time period. Then, choose whether to compute the **First value** or **Last value** of a specific event property.
...
1. In the **Schedule section**, select when the computation will execute.
   - In the **One-time** section, select **Run at next schedule** to run the computation once at the next scheduled time.\
     or
   - In the **Periodic section**, select the computation to run repeatedly on a **Weekly** or **Monthly** basis to keep the user property updated. You can adjust the schedule from the **Settings** page.
...
- - **View:** See the detailed configuration of a computation on a new page. Alternatively, you can access the detail page by clicking the trait name.
  ...
  - **Pause/Resume**: Pause the computation, or resume a paused computation. Pausing a computation releases its slot in the active-computation limit.
...
<Note>
  You cannot delete a computed trait. To stop a trait from running, pause it.

  To change the logic of a trait that has already run, create a new trait with the updated configuration and pause the original. Recomputing a trait with different logic would change the values that live segments and campaigns already depend on.
</Note>

<Info>
  - You can have a maximum of ten active computations at any given time. This includes computations with Scheduled, Processing, or Active statuses. Paused computations do not count toward this limit.
  - A workspace can hold up to 50 computations in total, including paused ones.
  - Computed trait values are applied to user properties at a processing speed of up to 1 million users per hour.
</Info>

# Use Your Computed Traits

Once your computation has run successfully, the new trait will be available as a standard user property throughout MoEngage.
...
<Note>
  In a workspace with multiple projects, a computed trait is created as a global user property. It appears under **User property** for every project, regardless of which project's events you built it from. For more information, refer to [Computed Traits in a Multi-Project Portfolio](/user-guide/data/computed-traits/computed-traits-overview#computed-traits-in-a-multi-project-portfolio).
</Note>
