...
Know more about [List of derived User properties here](/user-guide/data/user-data/derived-user-attributes).
...
| Viewed Web Page | Tracked when a user visits a web page. Select page URL as an event property to find the number of users visiting a particular page or use it to set up a Drop-off capture" Event Triggered Web Push. | Web, TV |
...
| Offering Clicked | Tracked when a user clicks an offering. | Android, iOS, Web |
| Offering Shown | Tracked when an offering is shown to the user. | Android, iOS, Web |
...
Billable Standard Events are predefined events in MoEngage that are tracked by the platform at your request. Unlike other derived events (automatically tracked) or custom events (sent by you via SDK or integrations), Billable Standard Events are generated when you ask MoEngage to capture certain specific events/actions. Each occurrence of these events contributes to your billed event count.

Since Billable Standard Events are tracked on demand, you need to create a support ticket to enable one or more of the Billable Standard Events. Note that data will be available only from the date the event is enabled as per your request.

## Event Properties

Source-wise list of event properties:
...
<Info>
  - Some MoEngage-tracked events or system events (e.g., App/Site opened) have device information tracked as [event properties](/user-guide/data/event-data/derived-events-and-attributes#event-attributes). Such event properties are available for analysis and segmentation.
...
<Note>
  ...
  **Available for Segmentation**
  ...
  The following device attributes are copied into user properties and are available in segmentation as a user property:

  - **Advertising Identifier** (iOS/Windows): Available as the "Advertising Identifier" user property
  ...
  **Not Available for Segmentation**
  ...
  All other device attributes listed below are not available as user properties in segmentation:
