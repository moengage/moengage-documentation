...
| Subscribed to Push | Tracked when a user subscribes to Push Notifications.  Note: *This event will not be generated for devices what when permission is granted by default. Eg. Android 12 and below devices* | Android, iOS, Web |
| Unsubscribed to Push | Tracked when a user changes push permission for a device from Subscribed to Unsubscribed  Note: *This event will not be generated for devices where the permission is denied by default.* | Android, iOS, Web |
...
<Note>
 The device attributes listed below are collected by MoEngage SDKs at the device level and are tied to a specific device instance, not to the user's profile.
 ...
 The following device attributes are copied into user attributes and are available in segmentation as a user property:
  ...
  - **moe_gaid** (Android) — Google Advertising ID
  - **Browser Details (moe_w_ds)** — For web users, includes platform, browser, and manufacturer
  - **Mobile User (moe_mweb)** — Whether the user accessed on mobile web
 ...
 All other device attributes listed below are not available as user attributes in segmentation:
  ...
  - **iOS:** IDFV, os_ver, device_tz, model, model_name
  - **Web/MWeb:** os, os_platform, os_ver, model, device_ts, device_tz, push_id
  - **TV:** os_key, moe_os_type, unique_id, push_id, status, OS_VERSION, MODEL, app_version
</Note>
