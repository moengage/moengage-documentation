...
MoEngage delivers event data to Databricks in two ways. Both methods produce the same table schema and refresh hourly, so choose based on how you want the data to reach your workspace:

| | Direct Export | OpenShare (Zero-Copy Sharing) |
| --- | --- | --- |
| **How data arrives** | MoEngage writes events into a table in your schema. | Your workspace queries MoEngage-hosted data live through the Delta Sharing protocol. |
| **Credentials MoEngage needs** | `WRITE` access to your Databricks catalog and schema. | None. You provide a sharing identifier, which is not a secret. |
| **IP allowlisting** | Required if your system enforces it. | Not required. |
| **Where data is stored** | In your own catalog and schema. | In a dedicated MoEngage-hosted catalog that you mount. |
| **Requires Unity Catalog** | No. | Yes. |
| **Setup time** | Self-serve on the App Marketplace. | Up to seven business days after you request enablement. |
...
<Tabs>
  ...
  <Tab title="OpenShare (Zero-Copy Sharing)">
    ...
    - Exports to Databricks are part of the Streams add-on. Contact your dedicated MoEngage CSM to enable them for your account.
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Direct Export">
    ...
    **Step 2: Add a New Databricks Export on the MoEngage App Marketplace**
    ...
    MoEngage starts appending events to the newly created table every hour.
  </Tab>
  <Tab title="OpenShare (Zero-Copy Sharing)">
    ...
    **Step 2: Accept the Share**
    ...
    After MoEngage provisions the share, which usually takes up to seven business days, your MoEngage representative sends you the name of your data share. To accept the share, perform the following steps:
    ...
    **Step 3: Configure Your Export**
    ...
    4. Follow the steps in the **Direct Export** tab of [Set Up Your Export](/user-guide/data/exports/events/data-warehouse/databricks-exports#set-up-your-export), starting from Step 2, to select your events and attributes.

    <Note>
      OpenShare provisions the connection for you, so MoEngage does not prompt you for service principal credentials or write access.
    </Note>
