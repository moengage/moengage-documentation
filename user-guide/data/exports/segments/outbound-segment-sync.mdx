...
In the **Segment** \> [Manage Segments page](/user-guide/segment/create-segments/manage-segments), select a Custom Segment and click **Sync Segment** in the action drop-down list.
...
| Resync All Users | 1. Choosing to allow resync all users while [setting up an Integration](#step-2-add-an-integration) will enable the action on each synced segment. <br />2. In case of any issue with a particular sync, your destination might hold incorrect users in the segment. To maintain the accurate state of the users in a segment, you have the option of resyncing all users in the segment. <br />3. Ensure that the destination can drop all users in the segment and allow the resync of the entire segment once again. |
...
Follow the details [above](#step-3-set-up-a-segment-sync) to sync your segments to your destination app.
...
Sync configurations such as the synced user property, sync schedule, and notification preferences are also available on the detail page.
