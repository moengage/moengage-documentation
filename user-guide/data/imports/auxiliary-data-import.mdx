...
A failed run is retried automatically up to a maximum retry count. Once retries are exhausted, the run is marked **EXHAUSTED** and requires a manual re-trigger. The schedule itself continues independently — a failed run does not block future scheduled runs.
