...
<Info>
  Very large single imports can fail to complete — for example, the run may time out or its schedule may expire — because of payload-size and processing-time limits. This is most likely on the first (historical) sync, which pulls in all matching rows. If you have a large dataset, split it into multiple smaller imports (for example, by date range) instead of importing everything in a single sync.
</Info>
