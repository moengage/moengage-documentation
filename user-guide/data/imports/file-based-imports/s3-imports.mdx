...
<Accordion title="A file failed with 'Applicable rate limit breached'. Will the periodic import reprocess it automatically?">
  No. When a file exhausts its processing retries and fails with the _Applicable rate limit breached_ status, MoEngage cannot replay that file on its own, so the periodic schedule does not reprocess it automatically. This retry limit protects the ingestion pipeline from being overloaded. To reprocess the affected files, rename them and re-upload them to your S3 folder so that MoEngage treats them as new files. To reduce the chance of hitting rate limits, stagger large batches instead of scheduling many files at once (see [Best Practices](/user-guide/data/imports/overview/overview-imports#best-practices)).
</Accordion>
