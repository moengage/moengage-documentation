...
<Info>
  **Information**
  ...
  - Very large RSA keys, such as 4096-bit, are not supported and fail during decryption with an error similar to `public key decryption failed: Cannot allocate memory`. Generate your key pair with a smaller RSA key size.
