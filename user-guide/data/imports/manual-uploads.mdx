...
description: "Upload a CSV directly in the MoEngage dashboard to import users or events. One-time uploads with no periodic scheduling."
...
<Tabs>
  <Tab title="Users">
    ...
    **Entry point:** **Segment** > **User Imports** > **Import Users**.
    ...
    Map each CSV column to a MoEngage user property (or create a new one). You see the top 3 rows (excluding the header) and all columns.
    ...
    <Info>
      Column names are case-sensitive and must match the attribute name on the dashboard exactly. Do not use `moe_` as a prefix for events, event properties, or user properties, it is a reserved system prefix, and using it may result in periodic blacklisting without prior communication.
    </Info>
  ...
  <Tab title="Events">
    ...
    **Entry point:** **Data** > **Data Imports** > **Events** tab > **Import** > **Events** > **Upload CSV**.
    ...
    3. **Select file**: Drag-and-drop or upload a CSV (less than 150 MB). Download a sample file to see the expected format. Each column represents an event property.
    ...
    **Map Columns**: Map the remaining CSV columns to MoEngage event properties:
