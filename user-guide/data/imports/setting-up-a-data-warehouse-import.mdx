...
<Accordion title="Deep dive: entity behavior, timezone pitfalls, and a worked example">
  ...
  **Events** — two separate mappable columns:
  ...
  `u_204` still does not import — `09:30` is earlier than every future `last_run_time`, so the miss does not self-correct. Because events are immutable once imported, there is no update-in-place to fix `u_204` later — recovery requires a one-time import (see below).
