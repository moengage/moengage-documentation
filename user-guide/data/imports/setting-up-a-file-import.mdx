...
For a fresh import, MoEngage provides a sample manifest file you can use as a reference. If you duplicate an existing import, its mapping file is pre-attached and available to download. If a manifest maps a column to an attribute that doesn't exist in MoEngage, the mapping is left blank — create the attribute from the UI first, then map the column to it.
