...
keywords: "file import, Amazon S3, SFTP, CSV, JSON, NDJSON, mapping file, manifest, naming conventions, file size limit, rate limit, PGP"
---
...
<Tabs>
  <Tab title="Amazon S3">
    1. In the **S3 bucket name** box, enter the name of the bucket that holds your files.
    2. In the **S3 access key** box, enter the Access Key ID that MoEngage uses to connect to the bucket.
    3. In the **S3 secret key** box, enter the Secret Access Key for that Access Key ID.
    4. In the **Region** list, select the region your bucket is hosted in. (Optional)
    5. In the **Folder path** box, enter the path of the folder that holds your files, without a trailing slash.
    6. To import PGP-encrypted files, select **Is your file encrypted?** and provide your private **Decryption key**, optional public **Signing key**, and **Key passphrase**.

    <img src="/images/s3-configure-connection-folder.png" alt="S3 bucket name, access key, secret key, region, folder path, and encryption fields" />
  </Tab>
  <Tab title="SFTP">
    ...
    <img src="/images/sftp-configure-connection-folder.png" alt="SFTP connection, folder URL, and encryption fields" />
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Amazon S3">
    - **Given S3 Access Key is not valid**: Verify your **S3 access key** and **S3 secret key**.
