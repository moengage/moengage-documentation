...
<Tabs>
  ...
  <Tab title="Bulk">
    By selecting bulk, you can fetch a maximum of 500 user properties in a single request, significantly reducing the number of requests made to collect each customer data individually.
    ...
    - **Parameters:** Type or paste the relevant "URL parameters," "Headers," and/or "Body" details for the API as the key-value pairs.
      <Info>
        You cannot personalize the URL path parameters and headers in bulk requests. You can only personalize user properties in the request body with the key-value pairs as follows: Customer hash: `{{UserAttribute['ID']}}` value, which is mandatory in input, and the same key will be returned in the output.
      </Info>
    ...
    3. Customer hash is a key, and 456 `{{UserAttribute['ID']}}` is the value. You can have different key-value pairs, for example: Customer key and Customer ID.\
       **Sample Response:** You will receive a response from the backend, as shown below:
       ...
       ```
       The data will be returned with the same key (user property) you requested.
       <Info>
         You can have multiple key sets but can make a personalized request based on user properties from only one key set.
       </Info>
  </Tab>
</Tabs>
