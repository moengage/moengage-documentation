...
<Tabs>
  <Tab title="Data APIs">
    ...
    **The following Bulk Import API consists of payloads for both users and events.**
    ...
    - Where possible, send the full set of configured identifiers in each request rather than a partial subset, to avoid ambiguous matches.
    - Whether Profile retention rules (Step 2) are configured to reflect which data source (online vs. offline, primary system vs. secondary) should win in a conflict.
    - You have a process to handle API error responses for merge conflicts rather than silently retrying.
    - You are aware that merges triggered this way are irreversible — test in a staging workspace before rolling out to production.
    ...
    This behavior is identical across every Data API (Track User, Create Event, Bulk Import) and across all other ingestion paths — SDKs, file imports, and native integrations such as Shopify. The merge is driven by your Identity Resolution configuration, not by which API or source the data arrived on.
...
# FAQs
<AccordionGroup>
  <Accordion title="Some user attributes and events seem to be lost during merge. Is it possible?">
    Yes.
