...
<Tabs>
  <Tab title="Data APIs">
    ...
    **The following Bulk Import API consists of payloads for both users and events.**
    ...
    - Profile retention rules (Step 2) are configured to reflect which data source (online vs. offline, primary system vs. secondary) should win in a conflict.
    - You have a process to handle API error responses for merge conflicts (see above) rather than silently retrying.
