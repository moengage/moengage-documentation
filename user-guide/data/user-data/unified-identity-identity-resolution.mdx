...
The two mechanisms have different rules: this feature is opt-in, irreversible once activated, and caps transitive merges at 100 events; the anonymous-to-registered merge is always-on, and transfers all events with no cap.
</Tip>
...
<Tabs>
  <Tab title="On Data">
    ...
    -Events: events from the merged profile transfer to the retained profile with one exception: for transitive multi-ID merges (a user merging into a profile that has itself already been merged), only the most recent 100 events of the deleted merged profile carry over. Earlier events in that chain are not preserved.
    <Info>
  ...
  <Tab title="On Segments">
    - Active segments: When users are merged, the total number of users in active segments may decreaseas as duplicate profiles collapse into one.
  ...
  <Tab title="On Campaigns">
    ...
    - Only Data APIs (TrackUser, CreateEvent, BulkImport API) supports multiple identifiers. Other APIs in MoEngage may still requires ID.
