...
description: "Remove a value from a custom user attribute entirely, restoring it to a null or non-existent state for accurate segmentation and data hygiene."
---
...
- This feature applies only to custom user attributes, not for standard and derived user attributes.
...
|-----|-----------|----------|--------------|
| type | String | Yes | Denotes the matching logic used to identify attributes for removal. Supported values are:<br />`equal`: Unsets the attribute that matches the exact name provided in `attr`.<br />`prefix`: Unsets all attributes that start with the string provided in `attr`.<br />`contains`: Unsets all attributes that contain the string provided in `attr`. |
| attr | String | Yes | The target string used for matching. This acts as the specific attribute name (for `equal`) or the search pattern (for `prefix`/`contains`) |
| case_sensitive | Boolean | Yes | Determines if capitalization matters during the matching process. Supported values are:<br />`true`<br />`false` |
