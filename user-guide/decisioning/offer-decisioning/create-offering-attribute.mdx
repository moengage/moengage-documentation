...
3. **Choose the Attribute Type**:
   ...
   - **Dynamic Value**: You map the attribute to specific user profile attributes from your connected CDP. The score is determined at the time of decision, based on the individual customer's profile.
     - Add between 1 and 20 user properties from your customer profile.
     - Assign a score to each user property mapping. The system uses these scores when a customer's profile matches.
     - The same user property cannot appear more than once in the mapping list.
...
| User Properties (Dynamic Value) | Min: 1<br />Max: 20 |
