...
A Dynamic value attribute is a scoring parameter where the score is not fixed at the time of creating the offering — it is calculated fresh for each individual customer at the moment of decisioning. The engine looks at that customer's real-time profile in MoEngage, checks specific user properties you have pre-configured, and awards a score based on what it finds.
...
When you define a Dynamic Value attribute, you map it to one or more user profile attributes and assign a score to each mapping. At decisioning time, the engine checks those user properties in order and awards the score for the first match it finds.
