...
When users move into a version from an earlier one, a badge appears above each stage they entered directly. Hover over the badge to see how many users arrived at that stage and which version they came from. The stage's own entry count also splits into the users who entered this version normally and those who arrived from a previous version, which explains a sudden rise in a stage's numbers that the entry stage does not account for.
