...
When you move active users to a new version, both tabs also report the movement. **Exits** lists a **Version migration** reason, which counts the trips that exited because of cross-version migration. Expand it to see the stage type and stage name each user left from. **Drops** lists the users who had no defined path to move to in the new version.
...
<Note>
  Changing the conversion goal is a common reason to create a new version, and MoEngage still sums the converted and revenue figures across versions when that happens. Where the event on the primary conversion goal differs between versions, MoEngage shows a callout on the All Versions view so that you know the total covers more than one definition.
</Note>
