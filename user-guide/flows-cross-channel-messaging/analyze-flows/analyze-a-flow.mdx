...
When you select a specific version, a **Cross-version movement** table appears below the flow performance chart. It lists how many users moved into the version you are viewing and which version each of them came from. The table has its own date range, separate from the one on **Flow performance**, so changing the dates there does not change this table.
...
When you move active users to a new version, both tabs also report the movement. **Drops** lists a **Versioning** reason for users who had no defined path to move to in the new version, and **Exits** reports the users who left the version because they moved to a newer one.
