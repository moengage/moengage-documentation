...
![Flow canvas showing the entry rules, each stage's entered, current, drops and exits counts, and the branch and exit counts below the conditional split](/images/flows-canvas-view-stage-stats.gif)
...
When users move into a version from an earlier one, a badge appears in the top-right corner of each stage they entered directly. Hover over the badge to see how many users arrived there and which versions they came from.

The stage's **Entered** count also carries a second line showing the split, such as `0 + 2 (from V10, V6)`. The first number counts the users who reached the stage from the stage above it, and the second counts those who arrived from an earlier version. This explains a rise in a stage's numbers that the entry stage does not account for.

![Hovering the badge on a stage to see how many users entered it directly from an earlier version](/images/flows-versioning-canvas-cross-version-badge.gif)
...
<Note>
  ...
  - Moving to a new version of the flow, when you publish a new version and choose to move the active users into it
...
- **Drops** - This tab lists reasons for all drop-offs, an explanation for each drop-off reason, the total drop-offs due to this reason, and the percentage of all drop-offs. When you expand a drop-off reason, you will see the stage type and stage name from which users were dropped off. When you move active users to a new version, this tab also lists a **Version migration failed** reason, which counts the trips dropped because the cross-version migration did not complete, such as when no defined path existed for the user in the new version. ![Drops tab of the Drops and Exits Breakdown showing the Version migration failed reason expanded to the stage users dropped from](/images/flows-versioning-drops-version-migration.png)
- **Exits** - This tab lists reasons for all exits, an explanation for each exit reason, the total exits due to this reason, and the percentage of all exits. When you expand an exit reason, you will see the stage type and stage name from which users exited.\
  This tab also shows Exit on conversion. You can see stats of users who exited the flow on achieving the conversion goal defined in the flow. When you move active users to a new version, this tab also lists a **Version migration** reason, which counts the trips that exited because of cross-version migration. Expand it to see the stage type and stage name each user left from. <img src="/images/moengage_83f95a.png" alt="9.png" />

![Exits tab showing the Version migration reason expanded to the stage users left from](/images/flows-versioning-exits-version-migration.png)

### Cross-Version Movement

When you select a specific version, a **Cross-version movement** table appears below the flow performance chart. **Moved from** names the version each group of users came from, and **No. of users moved** counts them. The table has its own date range, separate from the one on **Flow performance**, so changing the dates there does not change this table.

![Cross-version movement table showing the version users moved from and how many moved](/images/flows-versioning-cross-version-movement.png)
...
This section helps you analyze the aggregate performance of every channel used in the flow, together with the performance of each action campaign within it. MoEngage previously showed this as two separate sections, **Engagement Trends** and **Campaign Stats**.

### Channel Performance

Select a channel from the tabs to see how it performed, or select **All Channels** to compare every channel used in the flow. You can view the data as a table, a line chart, or a bar chart, and switch the chart between **CVR**, **CTR**, and **Delivery rate**. Use **Date Range** and **Tracking Goal** to scope the view, and **Compare With** to set a second period to compare against.

When you select a single channel, **CVR** and **CTR** for the tracking goal appear above the chart, and the chart plots that channel's own metrics, such as attempts, messages sent successfully, conversions, and conversion events. When you select **All Channels**, the chart compares every channel in the flow on the metric you selected. In the table view, each row is a date, with columns for user entries, successful sends, delivered, opened, adjusted open, clicked, CTR, and conversions.

![Channel and Campaign Performance section showing the channel tabs, the Channel Performance chart with CVR, CTR and Delivery rate options, and the Campaign Performance table below it](/images/flows-channel-campaign-performance.gif)

This section is also available when you select **All Versions**, which sums each metric across every version of the flow. Use it to judge the total cost and return of the flow without exporting each version and adding the numbers yourself. The All Versions view adds a **Converted trips**, **Converted users**, and **Conversions** column for each channel.

<Note>
  Changing the conversion goal is one of the reasons to create a new version, and MoEngage still sums the converted and revenue figures across versions when that happens. Where the event on the primary conversion goal differs between versions, MoEngage shows a callout on the All Versions view so that you know the total covers more than one definition.
</Note>
...
<Tabs>
  ...
  <Tab title="Email">
    ...
    | Conversion (CVR) | Conversions represent the number of unique users who have completed the conversion event. CVR is the percentage ratio between Conversions and Unique Impressions.  <br />The formula for CVR is (Unique Conversions/Unique Opens)\*100 |
    | Revenue | Revenue represents the sum of the total order value of conversion events. |
  </Tab>
  <Tab title="SMS">
    | Metric | Description |
...
<Note>
  You cannot add the metrics in each row of the **Campaign Performance** table and compare it with flow-level metrics because this table shows each campaign's performance and thus will have overlapped attribution across campaigns. For example, the sum of unique conversions can be different from converted trips and conversions at the overall flow level. For more information, refer [here](/user-guide/flows-cross-channel-messaging/faqs/analyzing-flows).
</Note>

| Metrics | Description |
| --- | --- |
| **Campaign Name** | The name of the action campaign in the flow. |
| **Channel** | The channel the campaign sends through. |
...
| **Total Sents** | The count of messages that were successfully sent eventually. |
| **Delivered** | Tracked for SMS. The Total Delivered for a campaign is shown here. Also the Delivered rate is shown. |
| **Opened** | Tracked for email. The Unique opens is shown here. Also the Open rate is shown in case of emails.  <br />You can also view the Adjusted opens here. To know more, refer to Adjusted Opens under [Channel Performance](#channel-performance). |
...
| **Successfully Synced** | Tracked for Facebook and Google Audience. The Total Synced for a campaign is shown here. |
...
| **Conversions** | The unique conversions and the CVR are shown for the campaigns here. MoEngage names this column after the goal you select in **Tracking Goal**, for example `Conversions: Goal 1`. |

<Note>
  The current flow version's channel and campaign performance is shown by default. Use the version selector to view the performance of other versions of the flow.
</Note>