...
- _Trip end reason_ - this field indicates why the user exited the flow. A user could have exited the flow because of one of the following reasons:
  ...
  | Move to new version | The user was exited from this version because a new version was published and the active users were moved into it. |
  | Dropped off as the path was no longer defined | The user was exited because the new version had no matching stage for them to move to. |
- _Trip end stage_ - this field indicates at which the user exited the flow.

When a user moves between versions, each version they were in appears as a separate entry, numbered in the order they happened. Select the entry for the older version to see the trip ending at the stage they left from, and select the entry for the newer version to see the trip starting at the stage they arrived in, rather than at the entry stage.

On the canvas, an icon above that stage marks where the user came into this version, and names the version they came from. The trip summary repeats this as a note, and clicking the note recenters the canvas on that stage.

**Locate user** — This option lets you view where the user is currently in the flow.

**Locate trip end stage** — This option lets you view where the user exited the flow. Flows typically have numerous nodes, multiple branches, and splits. The **Locate trip end stage** helps you search for the stage where the user exited the flow without the hassle of manually searching the canvas.
