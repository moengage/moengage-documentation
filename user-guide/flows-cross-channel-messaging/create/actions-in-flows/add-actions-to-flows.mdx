...
<Tabs>
  <Tab title="Push">
    ...
    Marketers can set the new push message (personalize-able on User properties) or import the message from an existing campaign using the campaign ID of that campaign\*\*.\*\*
  ...
  <Tab title="RCS">
    ...
    - **User property that stores user's mobile number:** Select the specific attribute that stores the user's mobile number.
    - **Select sender**: Select the authorized sender.
    - **Select template:** Select a custom template for the message.
    - **Personalization**: You can personalize fields such as Title, Body, and Suggestion using user properties like `{{Name}}` or `{{Link}}`.
