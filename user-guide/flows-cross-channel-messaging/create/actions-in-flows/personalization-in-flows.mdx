...
<Note>
Event attribute personalization in a campaign stage works only when the qualifying event occurred after the user entered the flow.
When a Has Done Event node is configured with a lookback window (for example, in the past 12 hours), it can match events the user performed before entering the flow. In this case, the condition evaluates correctly and the user moves to the Yes path — but the event's attributes are not available for personalization. Any attribute tokens in your campaign message will resolve to their fallback values.
</Note>
