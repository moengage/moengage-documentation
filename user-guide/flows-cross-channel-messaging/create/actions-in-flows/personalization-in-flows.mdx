...
<Warning>
Event attribute personalization in a **Has Done Event** node only works for events that occur **after** the user enters the flow. If the qualifying event occurred before the user entered the flow — for example, matched through a lookback window — the event attributes will be empty and personalization will not resolve. In this case, only the fallback value is used.
</Warning>
