...
3. From the resulting pop up, use one of the following options to create a flow:
   ...
   - [Using a flow template](#option-2-create-a-flow-using-a-flow-template) or
...
<Tabs>
  <Tab title="On Event Trigger">
    ...
    - In **THEN enter the user** section,
      ...
      2. Select **With Delay** to allow the user to enter the flow at a delayed time interval by providing the following details:
         1. Type the time value and select minutes, hours, or days. The entered value can range between 5 minutes and 30 days (both inclusive).
  ...
  <Tab title="At fixed time">
    ...
    <Note>
      Time-based flows enter users in bulk. Each instance or run of a time-based flow can enter up to 10 million users.
    </Note>

    <Info>
      **How the first send is calculated**

      For Daily, Weekly, and Monthly recurrence, the first send is the earliest occurrence of your chosen pattern that falls on or after your Start Date and Send Time. If the pattern's matching day already fell earlier in that cycle, before your Start Date, MoEngage skips it and moves the first send to the next matching cycle. It won't send retroactively, and it won't send on the Start Date itself unless the Start Date happens to match the pattern.

      Confirm the computed date using the confirmation message shown once you finish configuring the schedule:

      ![Confirmation line showing the computed first send date for a Monthly periodic flow](/images/periodic-flow-confirmation-send-date.png)

      Check this line before publishing. It always reflects your current configuration.
    </Info>
    ...
    | **Weekly** | Follow these steps to allow the user to enter the flow at a Weekly frequency starting from a specified time and date:  <img alt="Flow Weekly.png" src="/images/moengage_2f5e78.png" /> <ol><li>Select the time zone.</li><li>Select the start date from the calendar.</li><li>Type or use the up and down arrows to set the start time in hours and minutes.</li><li>Click am or pm.</li><li>Select the frequency at which the user is allowed to enter the flow using **Repeat every** and **Repeat on** option. <ul><li>Select the days of the week on which the user is allowed to enter the flow every week in **Repeaton**;and</li><li>the frequency in weeks for which the flow should allow users to enter from **Repeat every**. For example, choosing Monday and Friday in the 'Repeat on' section, and 3 in the 'Repeat every' section means that the campaigns will be sent 3 weeks once on Monday and Friday of the week.</li><li>Example: If your Start Date falls on a Wednesday but you select Monday and Friday under **Repeat on**, the first send occurs on the next Monday or Friday after the Start Date. Because the **Repeat every** interval is counted from that first matching day rather than from the Start Date, this first gap can be shorter than a full interval.</li></ul></li><li>Do one of the following to define the End time and date at which the system will stop new entries to the flow: <ul><li>Click **Never** to ensure the user can always enter the flow.</li><li>Click **On** and select the end date from the calendar. **Note**: Ensure that the end date is greater than the start date.</li><li>Click **After** and select the number of occurrences after which the campaign ends.</li></ul></li></ol> |
    | **Monthly** | Follow these steps to allow the user to enter the flow at a Monthly frequency starting from a specified time and date:  <img alt="Flows Monthly Days.png" src="/images/moengage_1ed91a.png" /><img alt="Flows Monthly Weeks.png" src="/images/moengage_26ccd5.png" /> <ol><li>Select the time zone.</li><li>Select the start date from the calendar.</li><li>Type or use the up and down arrows to set the start time in hours and minutes.</li><li>Click am or pm.</li><li>Select the frequency at which the user is allowed to enter the flow using '**Repeat on**' and '**Repeat every**'. <ul><li>**Days of month**: Select the days of the month on which the user is allowed to enter the flow and the frequency in months from **Repeat every** option. <ul><li>For example, choosing '7' in the calendar in the 'Repeat on' section and '3' months in 'Repeat every' means that the user will be allowed to enter the flow on the seventh of a month once every three months. Note: Multiple days can be selected in a month.</li><li>Example: If your Start Date is July 22 and you select the 21st under **Repeat on** with **Repeat every** set to 1 month, the 21st for July has already passed by the Start Date. The first send moves to August 21.</li><li>Note: If multiple days are selected in a month (for example, 5, 15, and 25) and the Start Date is July 22, the first send is July 25, the next selected day remaining in that month, rather than skipping ahead to August.</li></ul></li><li>**Days of week**: Select the days of the week on which the user is allowed to enter the flow and the frequency in months from **Repeat every** option. <ul><li>With this feature, users can be allowed to enter the flow on the first Mondays of every month or second Tuesdays and Wednesdays of every month and so on.</li><li>Example: If you select **First Monday** under Repeat on and the Start Date falls after that month's first Monday, the first send skips to the first Monday of the next eligible month, based on your Repeat every frequency.</li></ul></li></ul></li><li>Do one of the following to define the End time and date at which the system will stop new entries to the flow: <ul><li>Click **Never** to ensure the user can always enter the flow.</li><li>Click **On** and select the end date from the calendar. **Note**: Ensure that the end date is greater than the start date.</li><li>Click **After** and select the number of occurrences after which the campaign ends.</li></ul></li></ol> |
  </Tab>
  ...
  <Tab title="On business event trigger">
    <Note>
      Business-event-triggered flows are subject to the following limits:

      - A workspace can trigger up to **5 activations per hour** and **20 per day**. To raise these caps, contact Support (higher caps are priced at a 5× cost multiplier).
      - Up to **10 million users** can enter the flow in a single activation.
    </Note>
...
<Tabs>
  ...
  <Tab title="Filter by User Property">
    ...
    3. The default **Constant value** helps you complete the filter with specific static values. The **User property** (applicable for all types of flows) or **Business event property** (only for Business Event Triggered flows) option, as shown below, allows you to define the conditions using a different user property or business event property.\
...
<Warning>
  Canvas performance slows down once a flow has more than 500 stages.
</Warning>
