...
- Upon saving a version, you'll get the following option on the top of the page above the Flows canvas:
  ...
  | Publish | Let's you publish the draft version and [retire](/user-guide/flows-cross-channel-messaging/create/create-flows/status-of-flows#retiring-a-flow) the currently active version of the Flow.  <br />**Note:** If the previous version is in [paused status](/user-guide/flows-cross-channel-messaging/create/create-flows/status-of-flows#pause%2Fresume), publishing the draft version will stop the previous version. |
...
The User Entry Limit carries over across Flow versions. It doesn't reset when a new version publishes. A user's past entries count against whatever limit the current version defines, so they can only enter the new version enough times to reach that limit.

For example, say a Flow's User Entry Limit is 2 per year in Version 1, and a user has already entered twice:

- Version 2 keeps the limit at 2: the user can't enter for the rest of the year, since they've already used up the limit carried over from Version 1.
- Version 2 raises the limit to 3: the user can enter once more, since only 2 of the 3 entries have been used.
- Version 2 lowers the limit to 1: the user can't enter at all, since their entry count already exceeds the new limit.
