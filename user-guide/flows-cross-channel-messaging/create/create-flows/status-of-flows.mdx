...
This confirmation appears when no earlier version of the flow still has active trips. When an earlier version does, MoEngage shows a different confirmation that stops every version of the flow at once. For more information, refer to [Stopping a Flow That Has Active Older Versions](/user-guide/flows-cross-channel-messaging/create/create-flows/editing-a-flow#stopping-a-flow-that-has-active-older-versions).
