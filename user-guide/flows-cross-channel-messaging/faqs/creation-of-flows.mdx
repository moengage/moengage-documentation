...
Periodic (scheduled) flows have an upper limit on the size of the entry segment. If the target audience exceeds this limit, the affected users silently do not enter the flow, and no error is shown on the dashboard. If users are not entering a scheduled flow that targets a very large audience, reduce the segment size or split the audience into smaller segments across separate flows.