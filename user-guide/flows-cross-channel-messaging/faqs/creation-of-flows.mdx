...
- You can personalize the message with user properties and event properties.
...
## Why didn't my periodic flow send its first campaign on the Start Date I chose?

For Daily, Weekly, and Monthly flows, MoEngage calculates the first send as the earliest occurrence of your recurrence pattern on or after your Start Date and Send Time. It's not always the Start Date itself. If the day you selected already occurred earlier in that cycle, MoEngage moves the first send to the next matching cycle instead of sending late.

For example, a Monthly flow with a Start Date of July 22 that repeats on the 21st of every month won't send on July 21, since that date already passed. It sends on August 21 instead.

This is expected behavior. See [How do I confirm exactly when my flow's first campaign will send?](#how-do-i-confirm-exactly-when-my-flows-first-campaign-will-send) before publishing to verify the computed date.

## How do I confirm exactly when my flow's first campaign will send?

Once you've configured a Daily, Weekly, or Monthly schedule under **At fixed time**, MoEngage displays a confirmation line at the bottom of the scheduling panel, for example, *"Campaign will be next sent on Sat Aug 22 2026 11:30 PM."* This line accounts for your Start Date, Send Time, and recurrence settings, so it always shows the actual date and time MoEngage will send.

![Confirmation line showing the computed first send date for a periodic flow](/images/periodic-flow-confirmation-send-date.png)

Check this before you publish. It's the fastest way to catch a scheduling mismatch before it reaches your users.

## Why did my weekly flow's first send happen sooner or later than I expected?

The first send of a Weekly flow is the next occurrence of a day selected under **Repeat on** on or after your Start Date. That gap doesn't have to match your **Repeat every** cadence.

Say your Start Date falls on a Wednesday and you've selected Monday and Friday under Repeat on. The first send lands on whichever of those two days comes next, even if that's only a day or two away. The "Repeat every N weeks" cadence only kicks in starting from that first send. It has no bearing on how soon the first send happens.

## Why are users not entering my scheduled (periodic) flow with a large audience?

Periodic (scheduled) flows enter up to 10 million users in a single run. When the target audience is larger than 10 million, the users above that limit do not enter the flow.

To reach a larger audience, split it into segments of up to 10 million users each and run them across separate flows. For more information about the limit, refer to [Create a Flow](/user-guide/flows-cross-channel-messaging/create/create-flows/create-a-flow).
