...
<Warning>
  These rules reduce nesting depth, but they repeat terms — `D` appears three times in the result above — so the query gets longer as it gets flatter. A segment query is serialized before it runs and cannot exceed 256 KB (262,144 characters), which is a separate limit from the 6-level nesting limit. When you flatten a definition that already references several custom segments or filters with long value lists, run the query afterwards to confirm it still returns a count. See [Why Does My Segment Query Fail With a Query Length Error?](/user-guide/segment/faqs-segment/why-does-my-segment-query-fail-with-a-query-length-error)
</Warning>
