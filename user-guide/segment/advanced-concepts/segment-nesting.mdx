...
Marketers usually segment users based on multiple criteria such as User Property rules (User Properties), User activity rules (Event Properties), User Affinity rules, and Custom Segments. Most of the time, a combination of multiple criteria using AND or OR conditions is used to create a complex user segment.

For better understanding, let’s say A, B, C, and D represent the ‘segment blocks’, which are events & user properties with all their filters. For example, A can be ‘users who are male’, B can be ‘users who added to cart: wallet in the last 3 days, and so on. You can always create segments like A AND B AND C AND D and A OR B OR C OR D by combining segment blocks using AND/OR operators.
...
The examples mentioned above have 1 level of nesting. As in, for the complex segment (A AND B) OR (C AND D). In the above example A, B, C, and D have simple definitions using user properties or event properties. Those are not segments, those are simple user properties or event properties.
...
H is an user property “Install status is True”
...
**Definition of segment:** F is a custom segment made up of event property “Best time to send Email is between 10 Hrs and 13 Hrs. This will create a custom segment F of nesting level 1.
...
2. Another level - Using nested filter an event property is used that increases the nesting level to 2
...
**Definition of segment**: F is a custom segment made up of user or event properties J, K, L, M and F is used as a custom segment in Segment E’s definition
...
K is an user property “Best time to send Email is between 10 and 13”

L is an user property “SPAM is false”

M is an user property “Mobile user is True”
...
2. Another level - Using nested filter an event property is used that increases the nesting level to 4
...
<Info>
  [File Segments](/user-guide/segment/create-segments/file-segments) are not counted towards the nesting Limits.
</Info>
...
Here is an example of collapsing a multi-nested segment into a single level of nesting. Start with segment blocks `A` and `B` combined as `A AND B`, then expand each block that is itself a segment:

1. `A = C OR D`, so the segment becomes `(C OR D) AND B`.
2. `C = E AND F`, so the segment becomes `((E AND F) OR D) AND B`, which simplifies to `(E OR D) AND (F OR D) AND B`.
3. `E = G AND H`, so the segment becomes `((G AND H) OR D) AND (F OR D) AND B`.

Applying the logical rules above, this simplifies to `(G OR D) AND (H OR D) AND (F OR D) AND B` — a query with a nesting level of 1. Any segment definition can be broken down into a single level of nesting this way.