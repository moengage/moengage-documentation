...
description: "Speed up segment creation with value suggestions that auto-populate event and user property values as you build queries on the MoEngage dashboard."
---

Value Suggestions is a small but very useful feature for MoEngage dashboard users. Value suggestions are the exact values present in an event property or user property, which are shown to the users while creating segments or performing any analysis, so users do not have to remember long attribute names and can directly select the value from the suggested ones.
...
- For event properties, data becomes available in the value suggestions one day after they are received. Similarly, for user properties, data becomes available in the value suggestions two days after they are received.
...
- ***MoEngage does not show value suggestions for:***  
  - **Naming Conflicts** - Custom events or user properties/properties whose names conflict with MoEngage reserved names will not be considered for generating value suggestions, e.g., *appVersion, sdkVersion, os, and user\_id.*
