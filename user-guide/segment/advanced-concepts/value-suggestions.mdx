...
- MoEngage generates value suggestions daily at 12:30 AM UTC based on the previous day's data. For backdated ingestion (data occurring in the past but uploaded to MoEngage today), suggestions are derived from the four most recent days of that specific event, provided the data is no older than 60 days.
  ...
  <Warning>
  MoEngage does not generate or retroactively update value suggestions for previous-day data received after the daily run completes. Backdated suggestions come from a separate pass over older data, which does not revisit days the daily run has already processed. This affects recurring daily imports, such as SFTP imports, that arrive after the run.
  </Warning>
