---
title: "Rule-Based (Filter) Segment"
description: "Step-by-step guide to creating user segments in MoEngage using event filters, user properties, and behavioral rules for targeted campaign delivery."
---
...
The MoEngage dashboard enables growth teams to create multiple types of segments, including [File Segments](/user-guide/segment/create-segments/file-segments), Rule-Based Segments, and Custom Segments. [Learn more about segment types.](/user-guide/segment/getting-started/types-of-segments) You can also build and count these segments conversationally through an AI assistant using the [MoEngage MCP segmentation tools](/user-guide/ai-and-intelligence/merlin-ai/moengage-mcp-server#segments).

This document explains Rule-Based Segments in detail and how to combine them with other segment types.

<Tip>
  **Prerequisites**

  Rule-based segments are built from user property data and event data sent from your app or website. Before creating rule-based segments, ensure that MoEngage SDK integration and event tracking are complete. [More details on integration in our documentation.](/developer-guide/introduction)

  If you want to create a segment without SDK integration, use [File Segments](/user-guide/segment/create-segments/file-segments) or [Warehouse Segments](/user-guide/segment/create-segments/warehouse-segments) instead.
</Tip>

# Rule Based Segments

On the MoEngage dashboard, growth teams can create rules and criteria to define a user segment. The rules and criteria can be applied to user properties/properties, events & event properties, and a combination of the two.
...
Events are activities done by users such as App Open, Product Purchase, etc. These events also have event properties such as app version, platform name, product category, product price, etc. Event properties are tracked with events and are specific to the selected event.
...
4. Add Event property with a filter condition
...
<Info>
  - MoEngage generates a few default user properties, events, and event properties. [A list of all derived events and attributes can be found in this article.](/user-guide/data/event-data/derived-events-and-attributes)
...
[User affinity is explained in detail in this article.](/user-guide/segment/create-segments/affinity-segments)
...
- Add Event property with a filter condition and provide the filter value/s to create a rule.
...
[Custom Segments are explained in detail in this article](/user-guide/segment/create-segments/manage-segments).
...
<Tip>
  For advanced techniques such as nesting segment blocks within each other, aggregation operations, and object data type filtering, refer to the [Advanced Segmentation](/user-guide/segment/advanced-concepts/segment-nesting) section.
</Tip>
...
- **Export users** option to export users and desired User Properties/properties of the Segment. More details in this [article](/user-guide/analyze/analytics/user-data-exports).
