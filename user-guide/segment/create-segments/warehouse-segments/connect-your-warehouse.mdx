...
Before you can build a warehouse segment, connect MoEngage to your data warehouse and grant it read access to the tables you want to query. This is a one-time setup task for your data or engineering team.
...
{/* TODO: Confirm whether BigQuery and Redshift connections are now self-serve through App Marketplace. The spec assumes App Marketplace for every warehouse; the steps below still describe the email handoff. If self-serve shipped, both tabs need rewriting. */}
...
- [Warehouse Schemas](/user-guide/settings/data-management/warehouse-schemas) — map your tables so marketers can build segments from dropdown filters.
