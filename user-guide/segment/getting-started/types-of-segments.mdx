...
| --- | --- | --- |
| [File](/user-guide/segment/segment-operations/file-segments) | A static list of users uploaded as a CSV file. | You have a fixed, known list of users — for example, a migration list or an export from an offline database. |
| [Rule-based (filter)](/user-guide/segment/segment-operations/rule-based-filter-segments) | A dynamic group defined by conditions on user attributes, events, and event attributes. | You want membership to update automatically as user data and behavior change. |
| [Affinity](/user-guide/segment/segment-operations/affinity-segments) | A group based on behavioral patterns and preferences, such as how often or how consistently users take an action. | You want to target users by the strength of a behavior — for example, users who book rides at a certain hour at least 40% of the time. |
| [Custom](/user-guide/segment/segment-operations/manage-segments) | A saved, named segment query you can reuse across campaigns, flows, and analytics. | You reuse the same audience logic in multiple places, or you combine file, rule-based, and other custom segments. |
| [RFM](/user-guide/segment/segment-operations/rfm-segments) | Automatic buckets — such as Loyal, Promising, and At Risk — based on the recency, frequency, and monetary value of transactions. | You want to group users by transaction behavior without building rules manually. |
| [Warehouse](/user-guide/segment/segment-operations/warehouse-segments) | An audience built directly from your data warehouse (BigQuery, Snowflake, or Redshift) without copying data into MoEngage. | Your segmentation depends on scores or data that live only in your warehouse, or you have not integrated the MoEngage SDK yet. |
