...
description: "Create audience segments from your data warehouse in MoEngage without moving data. Launch targeted campaigns using warehouse data in a few clicks."
---
...
# Create a Warehouse segment

## Step 1: Set up your data source

Big Query Snowflake Databricks Redshift
...
2. Click the **SHARING** dropdown and then select **Permissions.**  
    ...
    <img  src="/images/moengage_6314e3.png"  />
3. Add the service account's email address (found in the JSON key file) as a member with the "BigQuery Data Viewer" role. <img src="/images/moengage_d9bc93.png" />
...
5. Click **Save** to grant access to the API.  
    <img  src="/images/moengage_996567.png"  />
...
2. **Snowflake Account Identifier**
   - It is always found at the beginning of your Snowflake URL, for example; (https://ACCOUNT\_IDENTIFIER.snowflakecomputing.com).
...
1. [Add a service principal in Databricks](/user-guide/segment/create-segments/warehouse-segments)
2. [Assign a service principal to Databricks workspace](/user-guide/segment/create-segments/warehouse-segments)
3. [Provide data reader access to the service principal](/user-guide/segment/create-segments/warehouse-segments)
4. [Provide compute permissions on SQL warehouses to service principal](/user-guide/segment/create-segments/warehouse-segments)
5. [Create a personal access token (PAT)](/user-guide/segment/create-segments/warehouse-segments)
6. [Fetch the HTTP path and Hostname](/user-guide/segment/create-segments/warehouse-segments)
7. [Connect Databricks on the App Marketplace](/user-guide/segment/create-segments/warehouse-segments)
...
<img alt="Screenshot 2025-02-25 at 5.37.34 PM.png" src="/images/moengage_8e7bfb.png" />
...
<img src="/images/moengage_e929fc.png" />
...
5. Click **Done**. <img alt="Screenshot 2025-02-25 at 6.03.21 PM.png" src="/images/moengage_4f1d44.png" />
...
<img alt="image (1).png" src="/images/moengage_fe54e9.png" />
...
2. On the App Marketplace page, search for **Databricks**. <img src="/images/moengage_e2ec5b.png" />
...
5. Enter the following details:  
   ...
   | **Catalog** | Yes | Type the Databricks Catalog name to which MoEngage will have access. |
    ...
    <img src="/images/moengage_9fc001.png" />
...
<Info>
If you have created a Redshift connection earlier, make sure you have the [current permissions](/user-guide/segment/create-segments/warehouse-segments) listed below.
</Info>
...
2. In the **Find Services** search box, search for IAM. You will be directed to the IAM dashboard.  
    <img src="/images/moengage_f80d3b.png" />
3. In the IAM dashboard, select the **Roles** tab.
4. Click **Create role.**
5. Select **AWS account**.
6. Under the **Select trusted entity** section, select **Custom trust policy. <img src="/images/moengage_dde296.png" />**
...
10. ctvtvgd4t Add the permissions mentioned below:
    ```
    redshift\_finegrained (customer-managed policy)
    ```
    ...
    ```
11. You can optionally restrict access to specific db-groups and users by selecting those options in the policy editor.   
     <img src="/images/moengage_3a38ef.png" />
12. Click **Next.**
13. In the **Add tags** section, click the Add new tag button to add the tags required for this IAM role. <img src="/images/moengage_c5e8c5.png" />
14. Click on **Next** \> **Review.**
15. In **Role name** box, enter the name for your role (for example, moengage-redshift-data-access-role").  
     <img src="/images/moengage_c2d5ec.png" />
16. Click on **Create IAM role**.  
     <img src="/images/moengage_73a8d5.png" />
...
- **ARN** (Amazon Resource Name):
  ...
  - Copy the ARN for the IAM role.
    ...
    <img src="/images/moengage_6d0501.png" />
...
- **Redshift Cluster Identifier name:** It is recommended that separate connections be created for each provisioned Redshift cluster.
  ...
  ```
  #{
  "role\_arn" : "arn:aws:iam::5822XXXX730:role/Segmentation\_Redshift",
  "aws\_regions" : "ap-south-1",
  "cluster\_identifier" : "redshift-segmentation-cluster",
  "default\_db" : "dev",
  }
  ```
...
1. On the sidebar menu in MoEngage, hover over the **Segment** menu item <img alt="Screenshot 2025-06-16 at 3.25.14 PM.png"  src="/images/moengage_10177f.png"  />  . The **Segment** menu appears.
2. Click **Warehouse segment**.  
    <img alt="Screenshot 2025-06-17 at 2.15.43 PM.png"  src="/images/moengage_9b41d7.png"  />
3. Here, you can choose the data warehouse connection which was set up in the previous step, from the **Source** dropdown on the top.
4. Once a source is chosen, the data warehouse **schema** - database, tables, and columns, will be populated on the left panel. From here, you can search for any database, table, or column that you want to use in your query.
5. You can enter your SQL query in the **SQL editor** on the right. You can perform any operations, such as joining multiple tables, sub-queries, aliases, etc., as required to return your optimal results.
6. Click on **Run Query** to run the SQL query in your data warehouse. You should be able to see a new row added in the Query results section below. <img alt="2024-11-26_17-24-56.gif" src="/images/moengage_b97fa4.png" />
7. The **Query results** section provides a list of recently run warehouse queries with query metrics such as Run time, Data scanned, and the number of rows of data returned by a specific query from your data warehouse. These metrics are useful for gauging the performance of your data warehouse queries and can be used to optimize your queries while you write them on MoEngage.
...
8. The Query results section also provides the status of each warehouse query run from MoEngage. The status can be successful, failed, or processed.
9. Once a query has run successfully, you should be able to expand the row from the Query results section and see the sample results of each query. We show up to 100 rows and 10 columns of data for each query.
10. Each query result also has a set of actions that can be accessed from the 3-dot menu on each row.
    ...
    - **Download results:**You can download the results of a query using the download results action. The downloaded file will contain a maximum of 100 rows and 100 columns of data <img src="/images/moengage_74c04f.png" />
11. **Preview Table:**Now you can one-click preview a few sample rows from any table listed in the schema section. On hovering over a table name in the schema, you should be able to see an eye icon, clicking on this will run a sample query on the table providing up to 10 rows of data as a preview.
...
<img alt="Screenshot 2024-06-26 at 3.37.51 PM.png" src="/images/moengage_c25d77.png" />
...
<img src="/images/moengage_a07516.png" />
...
<img alt="nandhini scr rec2.gif" src="/images/moengage_3de8f4.png" />
