...
<Tabs>
  <Tab title="Billable MTUs">
    ...
    MTU counts every unique user profile that performs at least one event in the calendar month, including anonymous profiles and background or system-triggered events, not only users who opened your app. This is why your MTU can be higher than MAU (Monthly Active Users), a separate dashboard metric that only counts users who opened your app or site and isn't part of the billing formula.

    | Metric |  Details | Example |
    | --- | --- | --- |
    | Total MTU | Mobile MTU + Web MTU + TV MTU. A user active on more than one platform is counted separately on each. | 10,000 mobile-only users + 4,000 web-only users + 1,000 users active on both (counted once per platform, contributing 2,000) = 16,000 Total MTU |
    | FUP | The average number of events allowed per user per month, set in your contract. To find your FUP, refer to [Organization Details](/user-guide/settings/account/billing/organization-details). | Typically 50–500 events per user |
