...
Your **Entitlements** are the products and services included in your contract. The Subscription tab shows which products are active, their subscription start dates, and expiration dates.
...
<Steps>
  ...
  <Step title="Open Subscription Tab">
    Click the **Subscription** tab.
  </Step>
  <Step title="Review Your Products">
    You'll see a table listing all active products (Email, SMS, WhatsApp, Flows, Segments, etc.) with their active dates and expiration dates.
  </Step>
  <Step title="Download for Record-Keeping">
    Click **Download CSV** to export your entitlements for your records or to share with your finance team.
  </Step>
</Steps>
...
| Product | Description |
| --- | --- |
| **Email** | Send email campaigns and flows; billed per email sent. |
| **SMS** | Send SMS campaigns; billed per SMS segment. |
| **WhatsApp** | Send WhatsApp messages; billed per message sent. |
| **Push Notifications** | Send push notifications; included in platform pricing. |
| **Flows** | Build and run customer journeys; included in all plans. |
| **Segments** | Create audience segments; included in all plans. |
| **Data Exports** | Export data to external destinations; usage is billed separately depending on plan. |
| **Open Analytics** | Run queries on your data; billed per GB scanned. |

If an entitlement is expiring soon, contact your Account Manager to renew or amend your contract.

## Create Billing Alerts

Create custom alerts that notify you when metrics approach or exceed contracted limits. Adjust campaigns, throttle activity, or plan upgrades before charges are incurred.

### Create a Custom Alert

<Steps>
  <Step title="Open Alert Management">
    Click **+ Manage Alert** in the upper-right corner of the Usage page.
  ...
  <Step title="Enter the general details">
    - **Alert name**: Enter a unique name (e.g., "Email Approaching 80%", "SMS Overage Warning").
    - **Create Alerts On**: Select **Billing**.
    - **Send Alert On**: Choose **Email**, **In-app**, or both.
    - (Optional) Click **+ Add alert destination** to configure Slack, MS Teams, or Webhooks. For more information, refer to [Alert Destinations](/user-guide/settings/reports-and-alerts/alert-management/getting-started/create-an-alert#alert-destinations).
  </Step>
  <Step title="Define the alert condition">
    - **Metric**: Select the billing metric to monitor (e.g., Emails Sent, SMS Sent, WhatsApp Messages, Total Monthly Active Users).
    - **Slab level**: Choose which slab to compare against:
      - **Base slab**: Your primary contracted volume.
      - **Slab 2** or **Slab 3** (if applicable): Higher tiers.
      - **Overage slab**: When consumption exceeds all contracted slabs.
    - **Threshold logic**: Select **Is greater than** (or **Is less than** for minimum thresholds).
    - **Numerical limit**: Enter the threshold value (e.g., "80000" for 80,000 emails).
  </Step>
  <Step title="Add recipients">
    In the **Members** section, select team members who should receive notifications.
  </Step>
...
- **Evaluation**: MoEngage scans your usage daily (typically at 12:00 AM UTC) and compares accumulated consumption against your threshold.
- **Notification**: You receive a notification the first time your usage crosses the threshold. Repeated notifications occur only when usage drops below the threshold and crosses it again.
- **Latency**: There's a 24-hour delay between when usage occurs and when it's available for alert evaluation.

### Best Practices

Start with alerts for high-risk metrics:

- **Emails Sent**: High volume and cost; set alerts at 70–80% of base slab.
- **SMS Sent**: Frequently variable; set alerts at 50%, 80%, and 100%.
- **WhatsApp Messages**: Often budget-constrained; set early warnings at 70%.
- **Platform MTU**: Set alerts for platform metrics you track closely.

Avoid too many alerts (more than 5–10) to prevent notification fatigue. Share alerts with finance, growth managers, and account admins.

For metrics with different evaluation periods (monthly vs. annual), adjust alert thresholds accordingly. View your [Contracted Volumes Dashboard](/user-guide/settings/account/billing/contracted-volumes-dashboard) to identify each metric's evaluation schedule.

<Info>
  - Edit and delete alerts at any time. For more information, refer to [Managing Alerts](/user-guide/settings/reports-and-alerts/alert-management/manage-alerts/managing-alerts).
  - Creating alerts requires the **Alert Manager > Manage alerts** permission. For more information, refer to [Access Roles](/user-guide/settings/account/team-management/access-roles).
</Info>

## FAQs

<Accordion title="Why don't my Email or SMS dashboard numbers match my bill?">
  The email volumes shown in your workspace dashboard reflect sends from that specific workspace only. Billing for email is calculated at the account level across all workspaces that use the same MoEngage Sender. If the same Sender (for example, a shared "from" email address or domain, or Sendgrid Subuser) is configured across multiple workspaces, the total sends from all those workspaces are aggregated for billing purposes. Your billed email volume is higher than what any single workspace dashboard shows. For the complete billing information, refer to the Usage page and select all relevant workspaces.
