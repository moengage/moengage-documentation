...
<Warning>
  New rules are saved in **Unpublished** status. The API Firewall remains inactive until you publish the rules. This allows you to add all required IP addresses before enforcement begins.
</Warning>
...
The rule status changes to **Active**. MoEngage now accepts Data API requests only from IP addresses that match a published rule. Requests from all other addresses receive a `403 Forbidden` response.
...
| **Last modified by** | Email address of the user who last modified the rule. |
