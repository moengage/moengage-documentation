...
<Tabs>
  ...
  <Tab title="Configure JumpCloud SSO">
    To set up SSO with JumpCloud, perform the following steps:

    1. Navigate to the **JumpCloud Admin Portal**.
    2. On the left navigation menu, click **Access** \> **SSO Applications**.
    3. Click **\+ Add New Application**, search for **Custom SAML App**, select it, and click **Next**.
    4. Select **Manage Single Sign-On (SSO)**, click the **Configure SSO with SAML** radio button, and click **Next**.
    5. In the **Display Label** box, type a name for your application, and click **Save Application**.
    6. Click **Configure Application**. You are taken to the SAML settings page for your application.
    7. On the **Configure SSO** dialog box in the MoEngage dashboard, in the **Identity Provider** list, select **JumpCloud**, and copy the **Single sign-on URL** and **Audience URI (SP Entity ID)**.
    8. On the SAML settings page in JumpCloud, enter the following details:
       | Field | Required | Description |
       | --- | --- | --- |
       | ACS URL | Yes | This URL receives the SAML assertion (the authentication response) from JumpCloud to MoEngage. Paste the **Single sign-on URL** copied from the MoEngage dashboard. |
       | SP Entity ID (Audience) | Yes | This URL informs JumpCloud about the specific application (MoEngage) for which the authentication assertion is intended. Paste the **Audience URI** copied from the MoEngage dashboard. |
       | SAMLSubject NameID | Yes | Select email. |
       | SAMLSubject NameID Format | Yes | Select **Email Address**. |
    9. Click **Save**.
    10. Under **JumpCloud Metadata**, click **Export Metadata** to download the SAML metadata XML file.
    11. On the **Configure SSO** dialog box in the MoEngage dashboard, click **Upload** to upload the downloaded XML file.
    12. By default, the **Update the same config for Test Environment** check box is selected. When it is selected, the configuration is applied to both the test and live environments. Clear the check box if you want the configuration to be applied only to the live environment.
    13. Click **Configure**. The Alert pop-up window will appear, prompting you to review the instructions properly.
    14. Select the **Send email to the users of this workspace** check box to notify all users about your workspace's Single Sign-On (SSO) enablement.
    15. Click **Done**. The SSO configuration is now successfully completed. You can see the JumpCloud SSO on the **Login** tab.
    16. In the JumpCloud Admin Portal, activate the application, and assign the users or groups who need access to MoEngage.
  </Tab>
