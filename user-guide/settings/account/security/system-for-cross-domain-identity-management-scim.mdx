...
<Tabs>
  <Tab title="Okta">
    ...
    ### Step 6.2.3: Update Okta User Property
  ...
  <Tab title="JumpCloud">
    ...
    <Info>
      MoEngage does not require JumpCloud to provision group objects. JumpCloud provisions users individually, and role assignment is handled using a custom attribute, as described in [Step 4](/user-guide/settings/account/security/system-for-cross-domain-identity-management-scim#step-4-map-the-role-attribute).
    </Info>

    ## Step 4: Map the Role Attribute

    Grant JumpCloud a way to pass the MoEngage role to each user during provisioning by adding a custom attribute to the user and mapping it to MoEngage's role extension.
    ...
    ### Step 4.1: Add a Custom Attribute to the User

    Add this custom attribute when you invite a new user, or to an existing user. To add it to an existing user, perform the following steps:
    ...
    3. On the **Details** tab, scroll to **Custom Attributes**, and click **add new custom attribute**.
       <Frame>
         ![User custom attribute](/images/usercustomattribute.gif)
       </Frame>
    ...
    3. Scroll to the bottom of the **Optional Mappings** table, and click **\+Add Attribute**.
       <Frame>
         ![Attribute mapping](/images/attributemapping.gif)
       </Frame>
    4. In **Mapping Type**, click **Direct Mapping (JSON Path)**. In **Source**, click **Custom User Property**, and replace the default value with the exact attribute name you used in [Step 4.1](/user-guide/settings/account/security/system-for-cross-domain-identity-management-scim#step-4-1-add-a-custom-attribute-to-the-user), for example, role.
    5. In the destination attribute box, type urn:ietf:params:scim:schemas:extension:moengage:2.0:User.role.\
       **Note**: Use a period before `role`, not a colon. JumpCloud reads `role` as a property of the `moengage:2.0:User` extension, rather than as a flat attribute name — unlike the syntax used for other IdPs.
    6. Click **Update** to apply the mapping and push the updated attribute to existing users.
    ...
    - **Role Accuracy**: Confirm that the specific role sent from JumpCloud (based on the custom attribute mapping) matches the assigned role in the workspace.
      <Frame>
        ![Userinvitation2](/images/userinvitation2.png)
      </Frame>
  </Tab>
